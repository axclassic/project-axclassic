...
<tr>
        ...
        <td class="bordNewsHautCentre" width="561">&nbsp;</td>
  ...
  $dbhz=new DBHZ();
   $dbhz->open();
	  $color = '<font color=#ffffff size=2><B><i>';
       $query = "SELECT character_.name, character_.level, character_.zonename, account.status FROM character_, account 
		WHERE account.id = character_.account_id AND character_.ID > '1822' AND character_.level > '4' 
		AND account.status = '0' ORDER BY character_.level DESC LIMIT 15";
...
echo '<br><br>';
      for ($i=0; $i <$num_results; $i++)
      {
         ...
         echo('<B><center><font size=2><i><font face="Times New Roman, Times, serif">'.$row['name'].'  </center></font></i><br><br>');
$HTML = <<< EOF
<table style="text-align: left; width: 80%; margin-left: auto; margin-right: auto;"border="0" cellpadding="0" cellspacing="0">
 <tbody>
  <tr>
   <td style="vertical-align: top;"><font size=1><font color="#336600">Current Level: <font color="#660033">$row[level] </P></font>
    </td>
   <td style="vertical-align: top; white-space: nowrap; text-align: right;"><font size=1><font color="#336600">Current Zone:<font color="#660033"><i> $row[zonename]</P></i></font>
    </td>
   </tr>
  </tbody>
</table>
