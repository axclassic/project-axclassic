#############
#NPC: Bryan_McGee
#Quest Name: Rogue Redemption
#Quest by: 
#Revised: Olhadon
#NPCs Involved: 
#Items Involved: Falyn Farreach, Conium Darkblade
#zone: highpass
...
sub EVENT_SAY { 
  if ($text=~/hail/i) {
    ...
    # checking if Beef ID:5055 is spawned.
    my $npccheck = $entity_list->GetMobByNpcTypeID(5055);
    if ($npccheck != 0) {
      #event_signal for Beef to chat.
      quest::signalwith(5055, 1, 2);
    }
  }
...
#END of FILE Zone:highpass  ID:5056 -- Bryan_McGee 
