...
sub EVENT_ITEM {
    if(plugin::check_handin(\%itemcount, 8723 => 1) && ($EventStage == 2)) {
        #Tiny Metal Quill
        quest::say("");
        quest::summonitem(0);
        #Copper x 8, Silver x 4, Gold x 3, Platinum x 12
        quest::givecash(8,4,3,12);
        quest::depop();
    }
    else {
