...
sub EVENT_SAY {
  ...
  if(($text=~/BattleRez/i) && ($ulevel >= 50)){
   ...
   elsif(defined ($EpicBox)) {
       quest::say("I see you have already quested your Epic...");
       quest::say("Take this Scroll to the Spell Scriber Anita Thrall");
       quest::summonitem(628);  # BattleRez Scroll
       return;
   }
   else{
      quest::say("Prove you have the Cleric Epic 1.0 by holding it and telling me [I have the Epic].");
   }
}
  elsif($text=~/I have the Epic/i) {
    if(plugin::check_hasitem($client, 5532)) { # Cleric Epic 1.0 Item ID
	quest::say("Excellent!");
	quest::say("Take this Scroll to the Spell Scriber Anita Thrall");
	quest::summonitem(628);  # BattleRez Scroll
	return;
        ...
        quest::say("You have more work to do....");
	return;
    }
  }
}
