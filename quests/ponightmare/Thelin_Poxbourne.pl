...
my $ready = quest::saylink("ready", 1);
  ...
  quest::flagclient(1510,2);
  }
  if ($text=~/dagger/i){
  quest:flagcheck(1510,2);
...
quest::flagclient(1510,3);
  }  
  if ($text=~/help/i){
  quest:flagcheck(1510,3);
