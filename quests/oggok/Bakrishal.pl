...
my $strong = quest::saylink("strong", 1);
  if($text=~/hail/i){
    quest::say("Whats do yuz wants? Yuz come to mez to lern bout speshul armorz? If'n yuz ar a $Beastlord, yuz kan lern to kraft yur own armor, yuz just needz da recipe.");
  }
  if(($text=~/Beastlord/i) && $class eq "Beastlord"){
