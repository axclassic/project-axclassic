sub EVENT_SAY {
my $watch = quest::saylink("watch reports", 1);
my $AandB = quest::saylink("Anehan and Behroe", 1);
my $LandQ = quest::saylink("Leanon and Quinon", 1);
if($text =~ /Hail/i) {
    quest::say("Why, hello there, $name! I am Jusean, loyal member of the League of Antonican Bards. You look like the reliable sort, maybe you could help me out for a bit, huh? It's about time for our field agents to turn in their $watch, and I need someone to go pick them up for me."); 
 }
 if ($text=~/watch reports/i) {
   quest::say("We always have someone stationed on watch duty at the two main entries to the city to keep an eye on what is happening around Qeynos. We have $AndB down at the docks, and $LandQ working the North Gate.");
 }
