sub EVENT_SAY {
   ...
   if($text=~/loyal servant/i && $faction == 1) { #require ally faction
      quest::say("I see now, yes, you are a loyal servant. I will translocate you into my master's lair when you are [ready].");
   }
   if($text=~/loyal servant/i && $faction != 1) { # less than ally faction
