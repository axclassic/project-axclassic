...
my $join = quest::saylink("join", 1);
 ...
 #else{
   #quest::say("Glad to see you again $name! You may pass.");
   #quest::movepc(337,479.9,-668.5,21.5); #Broodlands Norrath Keepers Camp
  # }
   #}
   if($text=~/worthy/i) {
 quest::say("We are in a race against time, the Dark Reign and thier associates are also recruiting people to try and turn the inhabitants of the various areas to the dark.");
 quest::say("If you are willing to $join the Norrath Keepers I send you to the Keepers camp in the Broodlands.");
 #quest::say("If you allow me to examine the item that makes you think you are worthy to assist us in this endevor, I can send you to the Norrath Keepers camp in the Broodlands.");
 }
 if($text=~/join/i) {
 quest::say("Very well $name, you may enter the Broodlands.");
 quest::movepc(337,479.9,-668.5,21.5); #Broodlands Norrath Keepers Camp
 }
 }
 #sub EVENT_ITEM {
 #if($ulevel >= 21 && plugin::check_handin(\%itemcount, 52347 => 1)) { ##Fatestealer 1.5 Epic
#quest::say("Well done explorer, you may enter the Broodlands."); 
