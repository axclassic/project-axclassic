...
my $Storms = quest::saylink("Storms", 1);
  if ($text=~/Hail/i){
   ...
   ##need zone to work to finish this quest##
   }
   ...
   quest::say("Someone of your vast experiance is just what I need. I have two missions that you could help The keepers of Norrath with.");
   quest::say("Kessdona's $Perch, $Reflections of Silver and An End to the $Storms. Which would you like to attempt?")
   }
   if ($ulevel >= 60 && $text=~/Perch/i){
   ...
   if ($ulevel >= 60 && $text=~/Reflections/i){
...
sub EVENT_ITEM { 
  if ($ulevel <= 50 && $ulevel >= 41 && plugin::check_handin(\%itemcount, 57201 => 1)) {
...
elsif ($ulevel >= 60 && plugin::check_handin(\%itemcount, 120142 => 1)) { 
