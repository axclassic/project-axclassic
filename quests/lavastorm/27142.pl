...
sub EVENT_ITEM {
   if ($ulevel <= 50 && $ulevel >= 41 && plugin::check_handin(\%itemcount, 120175 => 4)) { 
   quest::say("Good work $name. Hopefully that will slow the invasion.");
   quest::exp(20000000);
   quest::ding();
   quest::givecash(0,0,0,40);
   }
   elsif ($ulevel <= 50 && $ulevel >= 41 && plugin::check_handin(\%itemcount, 120174 => 4)) { 
   quest::say("Good work $name. Hopefully that will slow the invasion.");
   quest::exp(20000000);
   quest::ding();
   quest::givecash(0,0,0,40);
   }
   elsif ($ulevel <= 50 && $ulevel >= 41 && plugin::check_handin(\%itemcount, 55210 => 4)) { 
