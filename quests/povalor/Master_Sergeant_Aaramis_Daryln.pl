...
my $the = quest::saylink("the", 1);
my $cause = quest::saylink("cause", 1);
my $Aerin = quest::saylink("Aerin", 1);
my $glass = quest::saylink("glass", 1);
my $Dar = quest::saylink("Dar", 1);
my $company = quest::saylink("company", 1);
my $mission = quest::saylink("mission", 1);
  if(defined($qglobals{pov_key_quest}) && ($qglobals{pov_key_quest} == 1)) {
    if($text=~/hail/i) {
	   quest::say("You don't look like you could be much of a help to me $name, but since you were sent by Paralin perhaps you can $aid $the $cause.");
	   }
	if($text=~/aid/i) {
    ...
    if($text=~/mission/i) {
      quest::say("Not too fast $name. You'll have to speak to the Captain about that. I'm not at liberty to divulge that information at this time. That information is classified. I can tell you, but then I'd have to kill you.");
	  quest::emote('  laughs');
      quest::say("Go to Captain Ryglot and he'll be able to fill you in with all the details.");
