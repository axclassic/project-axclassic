sub EVENT_SAY {
  my $stupid = quest::saylink("stupid", 1);
my $brave = quest::saylink("brave", 1);
my $company = quest::saylink("company", 1);
my $squad = quest::saylink("squad", 1);
my $razorfiend = quest::saylink("razorfiend", 1);
...
my $prove = quest::saylink("prove", 1);
  if(defined $qglobals{"Aerinkey"} == 1) {
    if($text=~/hail/i ){
      quest::say("You're either $brave, or you're $stupid.");
    }
    if($text=~/stupid/i){
      quest::say("As I suspected, Be gone $name!");
    }
    if($text=~/brave/i) {
