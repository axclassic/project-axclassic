...
sub EVENT_HP {
  my @clist = $entity_list->GetClientList();
        foreach my $c (@clist){
  if($hpevent == 80){
   $c->Message(15, "You hear a strange buzzing around your head, and feel as though something is creeping toward you.");
  ...
  if($hpevent == 70){
   $c->Message(15, "You hear a buzzing in the distance.");
  ...
  sub EVENT_COMBAT { 
    my @clist = $entity_list->GetClientList();
        foreach my $c (@clist){
            $c->Message(15, "You would have made fine additions to my army, were you not so willful. Prepare yourself for power beyond your greatest nightmares.");
...
sub EVENT_SIGNAL {
   ...
   $c->Message(15,"The world shifts around you as the riftseeker's are consumed by their magic.");
