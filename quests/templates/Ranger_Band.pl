...
## Update 07/01/2020 @ line 20 patrik ##
########################################
## Further Notes:
...
my $minCharID  = 7711; # 5829 Ladder info
...
sub EVENT_ITEM {
    ...
    if($platinum > 0) {
        ...
        if(($itemcount{ $item1} == 1) || ($itemcount{ $item2} == 1)||($itemcount{ $item3} == 1) || ($itemcount{ $item4} == 1) ) {
    ...
    if(( ($copper > 0 )&& ($silver > 0 )&& ($gold > 0 )) || (($copper > 0 )&& ($silver > 0 ))|| ( ($copper > 0 )&&  ($gold > 0 )) || ( ($copper > 0 )&& ($gold > 0 ))|| ( ($silver > 0 )&&  ($gold > 0 ))) {
