sub EVENT_SAY { 
my $great = quest::saylink("great", 1);
my $whip = quest::saylink("whip", 1);
my $tassel = quest::saylink("tassel", 1);
my $lock = quest::saylink("lock", 1);
 if ($text=~/Hail/i){
	quest::say("I am looking for a $great sorcerer. Are you such a person?");
 }
 if (($text=~/great/i) && plugin::check_hasitem($client, 4266) && ($ulevel => 35)) {
	quest::say("Are we now? Well then take this. See if you can finish this project that I started so many years ago. It still requires a $whip, a $tassel, and a $lock."); 
    quest::summonitem(17195);
	quest::say("Put within what once was there. Go and find these items and return to me with what you have already learned and I shall reward you.");	
 }
 if ($text=~/whip/i){
