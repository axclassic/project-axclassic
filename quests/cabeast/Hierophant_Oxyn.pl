...
my $daily = quest::saylink("daily", 1);
my $crabs = quest::saylink("crabs", 1);
my $damaged = quest::saylink("damaged", 1);
  ...
  if(($text=~/temple/i) && ($faction <= 5) && ($class == "Shaman")) {
  ...
  if(($text=~/daily/i) && ($faction <= 5) && ($class == "Shaman")) {
  ...
  if($text=~/crabs/i) { #Cursed Wafers quest
  ...
  if($text=~/liquid/i) { #Crusaders of Greenmist (Greenmist Quest 8/8)
...
my $cudgel = quest::saylink("cudgel", 1);
my $other = quest::saylink("other", 1);
my $saviour = quest::saylink("saviour", 1);
  ...
  #Cursed Wafers turn in
    ...
    quest::say("We appreciate your service. Take a few copper for your deed as well as some of our cursed waters. They will provide you with nourishment."); 
    quest::faction(282, 10); ##Scaled Mystics##
    quest::faction(193, 10); ##Legion of Cabilis##
    quest::summonitem(12406, 2); ##Cursed Wafers##
	quest::say("As for future tasks, we are searching for a few $lost skulls and i am sure you are searching for your iron $cudgel of the clairvoyant And i also hear that the furscales are in need of some broodlings to do some manual labor. Tell them Oxyn sent you.");
  }
  ##Shaman Skull Quest No.1 turn in - Check for  Logrin Skull, Morgl Skull, and Waz Skull hand in along with Iron Cudgel of the Petitioner##
    ...
    quest::say("Excellent, you have retrieved the three skulls of the ancients for us.  There are yet more skulls that I must ask you to retrieve before I can assist you in furthering your progress to becoming a clairvoyant. I will tell you with whom you need to speak to to continue the path of clairvoyance upon bringing me the $other skulls.");
    quest::faction(282, 10); ##Scaled Mystics##
    quest::faction(193, 10); ##Legion of Cabilis##
