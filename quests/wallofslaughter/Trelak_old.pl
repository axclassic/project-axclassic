...
sub EVENT_SAY{
 if(($faction >=0) && (($class eq 'Warrior') | ($class eq 'Bard') | ($class eq 'Paladin') | ($class eq 'Shadowknight') | ($class eq 'Bard') | ($class eq 'Cleric'))){ #must be ally (1101) to do this quest
    if($text=~/hail/i){
     quest::me("Greetings stranger. What can I, Trelak do for you? Surely you are the strangers that I have heard whispers about . . . the ones that have truck back against our enemies? Perhaps we can aid each other? I am an armor smith that has crafted many fine pieces for the more prestigious officers in the dragorn militia. Many of these pieces have unique magical qualities that I crafted into them. If you are [willing] to recover them, I can recapture most of this magic and create new pieces for you.");
     }
    elsif($text=~/I am willing/i){
...
sub EVENT_ITEM{
 if(($faction >=0) && (($class eq 'Warrior') | ($class eq 'Bard') | ($class eq 'Paladin') | ($class eq 'Shadowknight') | ($class eq 'Bard') | ($class eq 'Cleric'))){ #must be ally (1101) to do this quest
	...
	quest::ding(); quest::exp(10000);
     ...
     elsif(plugin::check_handin(\%itemcount, 51479 => 1, 51486 => 2)){ #sleeves
   ...
   elsif(plugin::check_handin(\%itemcount, 51480 => 1, 51487 => 2)){ #bracers
    ...
    elsif(plugin::check_handin(\%itemcount, 51481 => 1, 51488 => 2)){ #gloves
    ...
    elsif(plugin::check_handin(\%itemcount, 51476 => 1, 51483 => 3)){ #breastplate
       quest::emote("takes the items from you before handing you your reward."); #made up
       quest::ding(); quest::exp(10000);
	 if($class eq 'Warrior'){
   ...
   elsif(plugin::check_handin(\%itemcount, 51477 => 1, 51484 => 3)){ #leggings
      quest::emote("takes the items from you before handing you your reward."); #made up
      quest::ding(); quest::exp(10000);
	if($class eq 'Warrior'){
  ...
  elsif(plugin::check_handin(\%itemcount, 51478 => 1, 51485 => 2)){ #boots
     quest::emote("takes the items from you before handing you your reward."); #made up
     quest::ding(); quest::exp(10000);
	if($class eq 'Warrior'){
