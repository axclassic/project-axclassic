# #Verrell_Cogswin (34049)
# Dragon Scale Conversion

my $recipe = quest::saylink("recipe", 1);
my $redscale = quest::saylink("red scale", 1);
my $goodnews = quest::saylink("good news", 1);
my $badnews = quest::saylink("bad news", 1);
my $catch = quest::saylink("catch", 1);
my $adhesive = quest::saylink("adhesive", 1);
my $gluepowder = quest::saylink("glue powder", 1);
my $DeepBarleyBrew = quest::saylink("Deep Barley Brew", 1);

sub EVENT_SAY {
    if($text=~/hail/i) {
        quest::say("Sorry, but I am a very tired gnome. I am returning from a trip to Halas but this desert heat has worn me out and for some reason my gate spell keeps failing. If only I had some water then I could make it to the wizard portal and wait for my brother Pellillo to pick me up.");
    }
    if(defined($qglobals{Green_Scale_Quest}) && ($qglobals{Green_Scale_Quest} == 1)) {
        if($text=~/recipe/i) {
            quest::say("My old shaman friend Shamus Felligan has been working on some way to change the color of items. He was very close to completing it but for some reason everytime he applied the mixture to something it would change the color temporarily. So he asked me to come down and help him. After a few months we realized what the problem was and we're now capable of changing things from red to green and from green to gray. I kept a sample for myself so I could turn my friend's red drake green as a surprise, but I would like to test it on a [ $redscale ] first.");
        }
        if($text=~/red scale/i) {
    ...
    if(defined($qglobals{Green_Scale_Quest}) && ($qglobals{Green_Scale_Quest} == 2)) {
        ...
        if($text=~/adhesive/i) {
            quest::say("In order to make this special adhesive you will need to combine some [ $gluepowder ] and some [ $DeepBarleyBrew ] in this container with this special vial. Return the result to me and I should be able to fix the scale, but please try to hurry I have to meet my brother Pellillo Cogswin at the wizard platform so he can take me home.");
...
sub EVENT_ITEM {
    ...
    elsif(plugin::check_handin(\%itemcount, 11622 => 1)) {
        #Red Dragon Scales
        if(defined($qglobals{Green_Scale_Quest}) && ($qglobals{Green_Scale_Quest} == 1)) {
            quest::emote("pulls a vial out of his pocket and places the red scale on the ground before pouring some of the vials contents on the scale. The scale begins to bubble and fizz as the liquid covers it entirely. Verrell looks at you and smiles before looking back at the scale, gasping and snatching the scale up from the ground.");
            quest::say("Hmmm . . well I have some [ $goodnews ] and a lot of [ $badnews ] which do you want to hear first?");
            quest::setglobal("Green_Scale_Quest",2,0,"F");
        }
    }
    elsif(plugin::check_handin(\%itemcount, 67386 => 1)) {
        #Thick Adhesive
    ...
    elsif(plugin::check_handin(\%itemcount, 67384 => 1)) {
        #Grey Colored Scales
