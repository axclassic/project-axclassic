#####################################
##The_Undead_Construct 4 of 5 bosses.
##accessing plane of time B
##associated with doorid 12
...
#If player manages to kill the final blow
	if (!defined $qglobals{$name."boss4"}){
		quest::setglobal($name."boss4", 1, 7, "M60");
		#$client->Message(15, "boss4 not defined, so you get a point!");	
	} else {
		#$client->Message(15, "boss4 qglobal already up and running.");
	}
...
#Checking to see if qglobals are already defined or not, if not assign to client(s)
		if (!defined $qglobals{$clientlist."boss4"}) {
			quest::setglobal($clientlist."boss4", 1, 7, "M60");
		}
