...
sub EVENT_SAY {
my $Gomoz = quest::saylink("Gomoz", 1);
my $hand = quest::saylink("hand", 1);
 if($text=~/hail/i){
 ...
 if($text=~/hand/i){
  quest::emote('hands you an old tattered sack.');
  quest::say("This shall be your proof of allegiance. Prove that you are strong with an intact mandible of a skulking brutling and the claws of a burynai excavator. Prove that you are a monk of fist and tail with a sash of an exiled human monk and the sparring gloves of a goblin pit fighter. Combined, these items make the sack full. This will earn my trust and the hand.");
  quest::summonitem(17036);
 }
 if($text=~/assist/i){
  quest::emote('pours the pebbles into his pocket and hands the box to you.');
  quest::say("Take my box. Fill and combine within my box the following, a radiant meteorite, frontier fool's gold, petrified redwood, Rile's sand coin, sulfur stone, fragment of tektite, plains pebble, jade magma, heart of ice and an unholy coldstone. Return the full tin box to me and I shall reward you with a shackle of bronze.");
  quest::summonitem(17039);
 }
 if($text=~/student/i){
  quest::emote('hands you what appears to be the base of a key.');
  quest::say("This was delivered to me by a dying student. He collapsed just after handing it to me. He was accompanying a student of mine named Veltar. They were questing for the legendary Whistling Fists. The key is made of a metal I have never felt or heard of. Find him.");
 ...
 if($text=~/test/i){
  ...
  my $z = $npc->GetZ();
  my $h = $npc->GetHeading();
  quest::spawn2(96325,0,0,$x,$y,$z,$h);
  quest::depop();
 }
 if($text=~/whistling/i){
...
my $student = quest::saylink("student", 1);
 if(plugin::check_handin(\%itemcount, 12815 => 1)){
  quest::emote('bows before you and takes a wrinkled, mummified hand from a pouch at his side.');
  quest::say("Take the hand. Let it rest within Cabilis and be reunited with the left.");
  ...
  quest::emote('opens the box to reveal a variety of pebbles.');
  quest::say("How grand!! My pebbles!! Now I can continue with my rock garden!! Hmmm. You look familiar. Hey!! I bet you can help me complete my garden!! Would you please $assist old Rinmark in completing his rock garden?");
  quest::exp(30000);
  quest::ding();
  quest::faction(317,20);
  quest::faction(193,10);
  ...
  quest::emote('graciously accepts the tin box filled with precious rocks and gems.');
  quest::say("Thank you greatly, young $name. For your deed, I bestow upon you the shackle of bronze!! You have completed the third rung. Alas, there is no time for celebration. I need your help in rescuing a $student of mine.");
