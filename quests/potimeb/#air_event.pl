...
sub EVENT_SPAWN {
    ...
    quest::ze(15, "Congdar spawned 4 An_Air_Phoenix_Noble.");
} 

sub EVENT_SIGNAL {
	if($signal == 14058) {
        quest::ze(15, "Congdar air_event signaled phase1_air_npcevent $phase1_air_npcevent of 12.");
		$phase1_air_npcevent = $phase1_air_npcevent + 1;
	}

	if($phase1_air_npcevent >= 13) {
        quest::ze(15, "Congdar phase1_air_npcevent 12 of 12 so spawn Neimon_of_Air and 4 Servitor_of_Xegony (223995).");
	...
	if($signal == 14050) {
        quest::ze(15, "Congdar Neimon_of_Air dead signalling phase_trigger, bye.");
