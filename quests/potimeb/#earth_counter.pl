...
sub EVENT_SPAWN {
    ...
    quest::ze(15, "Congdar spawned 6 An_Unholy_Rock_Fiend, 3 An_Elemental_Stonefist.");
}

sub EVENT_SIGNAL { #like earth_event.pl starts a counter for when all are dead, spawns the next group!
	if($signal == 14028) {
        quest::ze(15, "Congdar earth_counter signaled phase2_earth_npccounter $phase2_earth_npccounter of 9.");
	...
	if($phase2_earth_npccounter >= 10) {
        quest::ze(15, "Congdar phase2_earth_npccounter 9 of 9 so signal phase2_trigger, bye.");
