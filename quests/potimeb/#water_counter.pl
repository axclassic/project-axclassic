...
sub EVENT_SPAWN { #This spawns 1st mob group for Phase 2
    ...
    quest::ze(15, "Congdar pawning 3 A_Watercrafted_Hunter, 3 Champion_of_Korascian, 2 A_Deepwater_Assassin and A_Watercrafted_Hunter."); 
}

sub EVENT_SIGNAL {
    #like water_event.pl starts a counter for when all are dead, spawns the next group!
	if($signal == 14058) {
        quest::ze(15, "Congdar water_counter signaled phase2_water_npccounter is $phase2_water_npccounter out of 9."); 
        #This signal are from these mobs upon death!
		$phase2_water_npccounter = $phase2_water_npccounter + 1;
	}	

    if($phase2_water_npccounter >= 10) {
        quest::ze(15, "Congdar phase2_water_npccounter 9 of 9 so signal phase2_trigger.");
