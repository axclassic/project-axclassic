...
sub EVENT_SPAWN { #This spawns 1st mob group for Phase 2
    ...
    quest::ze(15, "Congdar spawned 5 A_Ferocious_War_Boar, 3 A_Fire_Etched_Doombringer, and A_Smoldering_Elemental.");
}

sub EVENT_SIGNAL {
    #like fire_event.pl starts a counter for when all are dead, spawns the next group!
	if($signal == 14068) {
        quest::ze(15, "Congdar fire_counter signaled phase2_fire_npccounter $phase2_fire_npccounter of 9.");
        #This signal are from these mobs upon death!
		$phase2_fire_npccounter = $phase2_fire_npccounter + 1;
	}	
	if($phase2_fire_npccounter >= 10) {
