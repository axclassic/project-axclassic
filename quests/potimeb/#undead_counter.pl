...
sub EVENT_SPAWN {
    ...
    quest::ze(15, "Congdar spawned 9 an_undead_guardian_.");
}

sub EVENT_SIGNAL {
    #like air_event.pl starts a counter for when all are dead, spawns the next group!
	if($signal == 14048) {
        quest::ze(15, "Congdar undead_counter signaled phase2_undead_npccounter $phase2_undead_npccounter of 9.");
        #This signal are from these mobs upon death!
		$phase2_undead_npccounter = $phase2_undead_npccounter + 1;
	}	
	if($phase2_undead_npccounter >= 10) {
        quest::ze(15, "Congdar phase2_undead_npccounter 9 of 9 so signal phase2_trigger, bye.");
