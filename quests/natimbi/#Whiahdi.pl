...
sub EVENT_SAY {
    if(plugin::check_hasitem($client,15784)) {
...
sub EVENT_ITEM {
    if((defined($qglobals{Soulwhisper}) && ($qglobals{Soulwhisper} == 2)) && plugin::check_handin(\%itemcount, 11174 => 1)) {
        #Whiahdi's Earthly Possessions
    ...
    elsif((defined($qglobals{Soulwhisper}) && ($qglobals{Soulwhisper} == 3)) && plugin::check_handin(\%itemcount, 55855 => 1)) {
        #Tarlang's Head
        ...
        $client->SetTitleSuffix("Deathspeaker", 1);
        $client->AddAAPoints(5);
    }
    elsif(plugin::check_handin(\%itemcount, 55940 => 1, 55932 => 1, 47100 => 1, 62581 => 1)) {
        #Staff Piece Number Two, Staff Piece Number Three, Globe of Discordant Engergy, Soulwhisper
        ...
        $client->SetTitleSuffix("Grand Occultist", 1);
