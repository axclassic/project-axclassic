sub EVENT_SAY {
my $smith = quest::saylink("smith", 1);
my $white = quest::saylink("white", 1);
my $red = quest::saylink("red", 1);
my $fiancee = quest::saylink("fiancee", 1);
my $ring = quest::saylink("ring", 1);
my $present = quest::saylink("present", 1);
my $red = quest::saylink("red", 1);
  if($text=~/hail/i){
  ...
  if(($text=~/ring/i){
   #White Dragonscale Cloak 
   quest::say("Only the best will do. I will not propose to her with anything less than a Platinum Trio Ring.");
  }
  if($text=~/present/i){
   #White Dragonscale Cloak 
   quest::say("Well, she is quite fond of jewelry. Bring to me a Marquise Cut Alexandrite and a Square Cut Black Sapphire. I will fashion the necklace myself, that will complement her eyes and beard so exquisitely.");
  }
  if($text=~/red/i){
   #Red Dragonscale Armor (START)
   quest::say("So you have a red dragon scale, eh? Well, I am wanting very much to return to my fiancee and propose to her. If you could aid me in quickly returning to her with a bit of spare change, we could have a deal. Heck, for the armor, a red dragonscale, a vial of swirling smoke and 1000 platinum coins is a bargain!");
  }
}

sub EVENT_ITEM {
  #Handin:
  if(plugin::check_handin(\%itemcount, 9240 => 1, 37902 => 1, 37903 => 1, 37710 => 1)){
