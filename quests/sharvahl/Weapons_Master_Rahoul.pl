...
sub EVENT_SAY { 
    ...
    if($text=~/willing/i) {
        quest::say("First, we will need you to contribute some of your own funds in order to obtain this particular piece. It is in a form of a statuette that will grant you an audience with Muada. I have used some rare components to make these that will cost you in the order of 2000 platinum pieces. Yes, it's a high cost, but we must keep our coffers healthy. You will need to create your own leather [adornments] however. When you have given me the required funds, I will give you the beastlord's statuette to work with.");
    }
    if($text=~/adornments/i) {
...
sub EVENT_ITEM {
    ...
    if(plugin::check_handin(\%itemcount, 9053 =>2, 9054 =>1, 9052 =>1 )) {
        quest::say("Ah, I see you have been so kind as to help us. Muada would like you take your assistance one step further. If you are [willing], of course.");
    }
    if($platinum >= 2000) {
