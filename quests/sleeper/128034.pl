...
sub EVENT_SAY {
my $adventuring = quest::saylink("adventuring", 1);
my $fancy = quest::saylink("fancy", 1);
my $larger = quest::saylink("larger", 1);
my $stories = quest::saylink("stories", 1);
my $short = quest::saylink("short", 1);
my $knowledge = quest::saylink("knowledge", 1);
my $couple = quest::saylink("couple", 1);
  if($text=~/hail/i && defined $qglobals{"Sleeperawake"}){
   quest::say("There is nothing more I can tell you $name, perhaps you should speak with Jobana Du'Ata");
   }
 ...
 elsif($text=~/fancy/i && !defined $qglobals{"Sleeperawake"}){
 ...
 elsif($text=~/adventuring/i && !defined $qglobals{"Sleeperawake"}){
  quest::say("Excellent, we can always use more of your type around, to perpetuate the life, if nothing else. Take care through this den though, as there are a great many things that would just as soon see you as a floor decoration than a wandering buccaneer, especially the four Warders at the end of it. From my own experience, I have found that the $larger golems seem to be in charge and may instruct the others to do their bidding, which may or may not include the destruction of any invaders.");
  }
 elsif($text=~/larger/i && !defined $qglobals{"Sleeperawake"}){
 ...
 elsif($text=~/stories/i && !defined $qglobals{"Sleeperawake"}){
  quest::say("There is a story of a $short fellow who came through here looking for grand adventures and seeking grand treasures, but all he found was grand pain and suffering. There is a lost love story of a $couple who came here in search of a friend, only to be sucked into the traps that litter the grounds beyond. Theirs is a most sad tale, wrought with pain and suffering, and whose story isn't yet completed. Then. . . then there is your story. Your story is what you make of it, and I hope for your sake you choose the pages of your adventure well. Look to the halls of this debilitating dungeon for answers you seek and perhaps you will find more than you were searching for.");
  }
 elsif($text=~/short/i && !defined $qglobals{"Sleeperawake"}){
 ...
 elsif($text=~/knowledge/i && !defined $qglobals{"Sleeperawake"}){
 ...
 elsif($text=~/couple/i && !defined $qglobals{"Sleeperawake"}){
