...
my $Skull = quest::saylink("Skull", 1);
my $Dagger = quest::saylink("Dagger", 1);
my $Coronet = quest::saylink("Coronet", 1);
my $Blue = quest::saylink("Blue", 1);
my $Black = quest::saylink("Black", 1);
my $highest = quest::saylink("highest", 1);
...
if ($text=~/headbands/i){quest::say("The Silent Fist Clan gives out colored Headbands for various deeds completed by our students. Phin gives out the White, Yellow, and Orange Headbands. I give out the $Red, $Purple, and $Blue Headbands.");
}
if ($text=~/red/i){quest::say("Ah, you think you are skilled enough to earn the red headband of the Silent Fist Clan, $name ? Hmmm.. Well, if you can help us find Brother $Dareb, you would certainly deserve the honor of wearing the red headband.");
}
if ($text=~/Dareb/i){quest::say("Yes, brother Dareb was last heard from over two weeks ago. He was out in the Plains of South Karana, and then just disappeared. We fear that those vile gnolls fleeing Splitpaw may have captured him or worse. We sent brother Shen to go search the Karanas and look for Dareb over three days ago, but haven't heard from him, either. I fear the worst for both of them, especially if the $Splitpaws are involved. Please, $name , find and return our lost brothers to us. If they are not alive, bring back their remains so we may give them a proper burial.. and, if it is that evil Ghanex Drah's doing, bring me his head as well, for the honor of the clan. You do this for us, $name , and turn in your orange headband, and I will reward you with the red headband of the Silent Fist. Good luck.");
}
...
if ($text=~/Coronet/i){quest::say("The Coronet of Buried Blood is an ancient headdress which is said to have belonged to the High Priest of the Plexant Temple. The Plexant were known to be devout followers of Innoruuk and the coronet is supposed to have been blessed by the Prince of Hate himself. Apparently, the coronet is now in the hands of a clan of goblins that dwell in the caverns of Permafrost.");
}
if ($text=~/blue/i){quest::say("Sorry, we are currently out of blue headbands. Please check back later.");
}
if ($text=~/black/i){quest::say("The black headband is one of the highest honors of our guild.. Lu'Sun only gives those out for great acts of heroism and devotion to the clan.");
}
if ($text=~/highest/i){quest::say("Ahhh, Lu'Sun is a master of many skills. He has traveled all of Norrath, studying various techniques and disciplines. Now, he runs this small guild and passes on his knowledge to those willing to devote their lives to our cause."); }
}
