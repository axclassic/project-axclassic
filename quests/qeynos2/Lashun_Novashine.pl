...
sub EVENT_SAY {
  if($text=~/hail/i) {
    quest::say("Well met, $name. My name is Lashun Novashine and I am a humble priest of Rodcet Nife, the Prime Healer. I wish to spread His word to every corner of Norrath. My job gets more difficult each day with so many so willing to take lives rather than preserve them. Is it [healing] that you require?");
  }
  if($text=~/healing/i) {
  quest::emote("smiles warmly.");
  quest::say("Rodcet Nife requests a small donation to enlighten you with healing. If you wish healing, please bring me two bone chips. If you need curing, a small donation of 2 gold pieces would bestow a fine blessing.");
  }
}
