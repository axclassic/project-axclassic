sub EVENT_SAY {
    if($text=~/hail/i) {
        if(defined($qglobals{Wayfarer}) && ($qglobals{Wayfarer} == 4)) {
            #Player has received Wayfarers Brotherhood Emblem
            if(!plugin::check_hasitem($client, 40999)) {
...
sub EVENT_ITEM {
    if(plugin::check_handin(\%itemcount, 41000 => 1)) {
        #Adventurer's Stone
        if(defined($qglobals{Wayfarer}) && ($qglobals{Wayfarer} == 3)) {
            #Player has maxed out Adventurer's stone and is ready to speak with Chaenz
