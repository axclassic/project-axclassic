...
sub EVENT_SIGNAL {
	if($signal == 1) {
		...
		if($counter == 9) {
			...
			quest::spawn2(154147,0,0,329.00,-305.00,-7.00,260); # 
			quest::spawn2(154147,0,0,356.00,-305.00,-7.00,260); #
			quest::spawn2(quest::ChooseRandom(154155, 154163),0,0,403.10,-239.98,37.06,130); # Fabled/Warder of death
		}

		if($counter == 17) {
			quest::spawn2(quest::ChooseRandom(154154, 154162),0,0,345.00,-277.00,-6.00,260); # Fabled/Warder of Life
		}
	...
	if($signal == 3) {
		# Succed
		quest::spawn2(quest::ChooseRandom(154145, 154161),0,0,940.00,-610.00,-41.00,0); # Spawn Fabled/Kahti Sha
		quest::signal(154145,100); # Signal Khati Sha to Shout
		quest::signal(154161,100); # Signal Khati Sha to Shout
