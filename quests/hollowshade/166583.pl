## Normal condition Spawner ##
sub EVENT_SPAWN{
my $a = quest::ChooseRandom(166413,166426,166445,166457);
my $b = quest::ChooseRandom(166414,166427,166446,166458);
my $c = quest::ChooseRandom(166415,166428,166447,166459);
my $d = quest::ChooseRandom(166416,166429,166448,166460);
my $e = quest::ChooseRandom(166417,166430,166449,166461);
my $f = quest::ChooseRandom(166418,166431,166450,166462);
my $g = quest::ChooseRandom(166419,166432,166451,166463);
my $h = quest::ChooseRandom(166420,166433,166452,166464);
my $i = quest::ChooseRandom(166408,166409,166410);
my $j = quest::ChooseRandom(166470,166471);
my $k = quest::ChooseRandom(166421,166453,166465);
my $l = quest::ChooseRandom(166491,166434,166454,166489);
my $m = quest::ChooseRandom(166422,166435,166455,166466);
my $n = quest::ChooseRandom(166423,166436,166456);
my $o = quest::ChooseRandom(166424,166437,166254);
my $p = quest::ChooseRandom(166474,166477);
my $q = quest::ChooseRandom(166475,166478);
my $r = quest::ChooseRandom(166480,166481);
my $s = quest::ChooseRandom(166659,166479);
my $t = quest::ChooseRandom(166602,166603,166604);
my $u = quest::ChooseRandom(166655,166656,166657,166658);
my $v = quest::ChooseRandom(166605,166606,166607,166608);
my $w = quest::ChooseRandom(166609,166610,166611);
my $x = quest::ChooseRandom(166612,166613,166614);
my $y = quest::ChooseRandom(166615,166616,166617,166618);
my $z = quest::ChooseRandom(166619,166620,166621,166622);
my $aa = quest::ChooseRandom(166623,166624,126625,126626);
my $ab = quest::ChooseRandom(166627,166628,166629,166630);
my $ac = quest::ChooseRandom(166631,166632,166633);
my $ad = quest::ChooseRandom(166634,166635,166636);
my $af = quest::ChooseRandom(166640,166641,166642);
my $ag = quest::ChooseRandom(166643,166644,166645);
my $ah = quest::ChooseRandom(166646,166647,166648);
my $dn = quest::ChooseRandom(166177,166777,166829);
my $do = quest::ChooseRandom(166830,166831);
my $dp = quest::ChooseRandom(166832,166833);
my $dq = quest::ChooseRandom(166834,166835);
my $dr = quest::ChooseRandom(166836,166837);
my $ds = quest::ChooseRandom(166838,166839);
my $dt = quest::ChooseRandom(166840,166841);
my $du = quest::ChooseRandom(166842,166843);
my $dv = quest::ChooseRandom(166844,166845);
my $dw = quest::ChooseRandom(166846,166847);
my $dx = quest::ChooseRandom(166848,166849);
my $dy = quest::ChooseRandom(166850,166851);
my $dz = quest::ChooseRandom(166852,166853);
my $ea = quest::ChooseRandom(166854,166855);
my $eb = quest::ChooseRandom(166856,166857);
my $ec = quest::ChooseRandom(166858,166859);
my $ed = quest::ChooseRandom(166860,166861);
my $ef = quest::ChooseRandom(166862,166863);
my $eg = quest::ChooseRandom(166864,166865);
   quest::depopall(166311);
   quest::depopall(166406);
   quest::depopall(166575);
   quest::depopall(166574);
   quest::depopall(166403);
   quest::depopall(166139);
   quest::depopall(166668);
   quest::depopall(166256);
   quest::depop(166210);
   quest::depop(166209);
   quest::depop(166175);
   quest::depop(166211);
   quest::depop(166170);
   quest::depop(166777);
   quest::depop(166829);
   quest::depop(166830);
   quest::depop(166831);
   quest::depop(166832);
   quest::depop(166833);
   quest::depop(166834);
   quest::depop(166835);
   quest::depop(166836);
   quest::depop(166837);
   quest::depop(166838);
   quest::depop(166839);
   quest::depop(166840);
   quest::depop(166841);
   quest::depop(166842);
   quest::depop(166843);
   quest::depop(166844);
   quest::depop(166845);
   quest::depop(166846);
   quest::depop(166847);
   quest::depop(166848);
   quest::depop(166849);
   quest::depop(166850);
   quest::depop(166851);
   quest::depop(166852);
   quest::depop(166853);
   quest::depop(166854);
   quest::depop(166855);
   quest::depop(166856);
   quest::depop(166857);
   quest::depop(166858);
   quest::depop(166859);
   quest::depop(166860);
   quest::depop(166861);
   quest::depop(166862);
   quest::depop(166863);
   quest::depop(166864);
   quest::depop(166865);
   quest::depop(166866);
   quest::depop(166867);
   quest::depop(166597);
   quest::depop(166407);
   quest::depop(166315);
   quest::depop(166405);
   quest::depop(166576);
   quest::depop(166316);
   quest::depop(166577);
   quest::depop(166662);
   quest::depop(166661);
   quest::depop(166663);
   quest::depop(166722);
   quest::depop(166721);
   quest::depop(166723);
   quest::depop(166487);
   quest::depop(166585);
   quest::depop(166586);
   quest::depop(166587);
   quest::depop(166588);
   quest::depop(166408);
   quest::depop(166409);
   quest::depop(166410);
   quest::depop(166411);
   quest::depop(166412);
   quest::depop(166413);
   quest::depop(166414);
   quest::depop(166415);
   quest::depopall(166416);
   quest::depop(166417);
   quest::depop(166418);
   quest::depop(166419);
   quest::depop(166420);
   quest::depop(166421);
   quest::depop(166422);
   quest::depop(166423);
   quest::depop(166424);
   quest::depop(166425);
   quest::depop(166426);
   quest::depop(166427);
   quest::depop(166428);
   quest::depop(166429);
   quest::depop(166430);
   quest::depop(166431);
   quest::depop(166487);
   quest::depop(166585);
   quest::depop(166586);
   quest::depop(166587);
   quest::depop(166588);
   quest::depop(166432);
   quest::depop(166433);
   quest::depop(166434);
   quest::depop(166435);
   quest::depop(166436);
   quest::depop(166437);
   quest::depop(166438);
   quest::depop(166439);
   quest::depop(166440);
   quest::depop(166441);
   quest::depop(166442);
   quest::depop(166443);
   quest::depop(166444);
   quest::depop(166445);
   quest::depop(166446);
   quest::depop(166447);
   quest::depop(166448);
   quest::depop(166449);
   quest::depop(166450);
   quest::depop(166451);
   quest::depop(166452);
   quest::depop(166453);
   quest::depop(166454);
   quest::depop(166455);
   quest::depop(166456);
   quest::depop(166457);
   quest::depop(166458);
   quest::depop(166459);
   quest::depop(166460);
   quest::depop(166461);
   quest::depop(166462);
   quest::depop(166463);
   quest::depop(166464);
   quest::depop(166465);
   quest::depop(166466);
   quest::depop(166467);
   quest::depop(166468);
   quest::depop(166469);
   quest::depop(166470);
   quest::depop(166471);
   quest::depop(166474);
   quest::depop(166475);
   quest::depop(166477);
   quest::depop(166478);
   quest::depop(166479);
   quest::depop(166586);
   quest::depop(166587);
   quest::depop(166588);
   quest::depop(166487);
   quest::depop(166492);
   quest::depop(166493);
   quest::depop(166659);
   quest::depop(166660);
   quest::depop(166665);
   quest::depop(166321);
   quest::depop(166322);
   quest::depop(166323);
   quest::depop(166324);
   quest::depop(166325);
   quest::depop(166326);
   quest::depop(166327);
   quest::depop(166328);
   quest::depop(166329);
   quest::depop(166330);
   quest::depop(166331);
   quest::depop(166332);
   quest::depop(166333);
   quest::depop(166334);
   quest::depop(166335);
   quest::depop(166336);
   quest::depop(166337);
   quest::depop(166338);
   quest::depop(166339);
   quest::depop(166340);
   quest::depop(166341);
   quest::depop(166342);
   quest::depop(166343);
   quest::depop(166344);
   quest::depop(166345);
   quest::depop(166346);
   quest::depop(166347);
   quest::depop(166348);
   quest::depop(166349);
   quest::depop(166350);
   quest::depop(166351);
   quest::depop(166352);
   quest::depop(166355);
   quest::depop(166356);
   quest::depop(166357);
   quest::depop(166358);
   quest::depop(166359);
   quest::depop(166360);
   quest::depop(166361);
   quest::depop(166362);
   quest::depop(166363);
   quest::depop(166364);
   quest::depop(166365);
   quest::depop(166366);
   quest::depop(166367);
   quest::depop(166368);
   quest::depop(166369);
   quest::depop(166370);
   quest::depop(166371);
   quest::depop(166372);
   quest::depop(166373);
   quest::depop(166374);
   quest::depop(166375);
   quest::depop(166376);
   quest::depop(166377);
   quest::depop(166378);
   quest::depop(166379);
   quest::depop(166268);
   quest::depop(166380);
   quest::depop(166381);
   quest::depop(166382);
   quest::depop(166383);
   quest::depop(166320);
   quest::depop(166390);
   quest::depop(166391);
   quest::depop(166392);
   quest::depop(166393);
   quest::depop(166394);
   quest::depop(166395);
   quest::depop(166260);
   quest::depop(166396);
   quest::depop(166397);
   quest::depop(166398);
   quest::depop(166399);
   quest::depop(166400);
   quest::depop(166401);
   quest::depop(166402);
   quest::depop(166404);
   quest::depop(166602);
   quest::depop(166603);
   quest::depop(166604);
   quest::depop(166605);
   quest::depop(166606);
   quest::depop(166607);
   quest::depop(166608);
   quest::depop(166609);
   quest::depop(166610);
   quest::depop(166611);
   quest::depop(166612);
   quest::depop(166613);
   quest::depop(166614);
   quest::depop(166615);
   quest::depop(166616);
   quest::depop(166617);
   quest::depop(166618);
   quest::depop(166619);
   quest::depop(166620);
   quest::depop(166602);
   quest::depop(166602);
   quest::depop(166621);
   quest::depop(166622);
   quest::depop(166623);
   quest::depop(166624);
   quest::depop(166625);
   quest::depop(166626);
   quest::depop(166627);
   quest::depop(166628);
   quest::depop(166629);
   quest::depop(166630);
   quest::depop(166631);
   quest::depop(166632);
   quest::depop(166633);
   quest::depop(166634);
   quest::depop(166635);
   quest::depop(166636);
   quest::depop(166637);
   quest::depop(166638);
   quest::depop(166639);
   quest::depop(166640);
   quest::depop(166641);
   quest::depop(166642);
   quest::depop(166643);
   quest::depop(166644);
   quest::depop(166645);
   quest::depop(166646);
   quest::depop(166647);
   quest::depop(166648);
   quest::depop(166650);
   quest::depop(166651);
   quest::depop(166652);
   quest::depop(166653);
   quest::depop(166654);
   quest::depop(166655);
   quest::depop(166656);
   quest::depop(166657);
   quest::depop(166658);
   quest::depop(166719);
   quest::depop(166669);
   quest::depopall(166670);
   quest::depop(166671);
   quest::depop(166672);
   quest::depop(166673);
   quest::depop(166674);
   quest::depop(166675);
   quest::depop(166676);
   quest::depop(166677);
   quest::depop(166678);
   quest::depop(166679);
   quest::depop(166680);
   quest::depop(166681);
   quest::depop(166682);
   quest::depop(166683);
   quest::depop(166684);
   quest::depop(166685);
   quest::depop(166686);
   quest::depop(166687);
   quest::depop(166688);
   quest::depop(166689);
   quest::depop(166690);
   quest::depop(166691);
   quest::depop(166692);
   quest::depop(166693);
   quest::depop(166694);
   quest::depop(126695);
   quest::depop(126696);
   quest::depop(166697);
   quest::depop(166698);
   quest::depop(166699);
   quest::depop(166700);
   quest::depop(166701);
   quest::depop(166702);
   quest::depop(166703);
   quest::depop(166704);
   quest::depop(166705);
   quest::depop(166706);
   quest::depop(166707);
   quest::depop(166708);
   quest::depop(166709);
   quest::depop(166710);
   quest::depop(166711);
   quest::depop(166712);
   quest::depop(166713);
   quest::depop(166714);
   quest::depop(166715);
   quest::depop(166716);
   quest::depop(166717);
   quest::depop(166718);
   quest::depop(166661);
   quest::depop(166662);
   quest::depop(166663);
   quest::depop(166233);
   quest::depop(166601);
   quest::depop(166600);
   quest::depop(166509);
   quest::depop(166561);
   quest::depop(166496);
   quest::depop(166528);
   quest::depop(166510);
   quest::depop(166529);
   quest::depop(166497);
   quest::depop(166538);
   quest::depop(166511);
   quest::depop(166530);
   quest::depop(166498);
   quest::depop(166512);
   quest::depop(166531);
   quest::depop(166499);
   quest::depop(166513);
   quest::depop(166531);
   quest::depop(166514);
   quest::depop(166500);
   quest::depop(166515);
   quest::depop(166534);
   quest::depop(166562);
   quest::depop(166501);
   quest::depop(166516);
   quest::depop(166535);
   quest::depop(166517);
   quest::depop(166502);
   quest::depop(166518);
   quest::depop(166698);
   quest::depop(166729);
   quest::depop(166503);
   quest::depop(166627);
   quest::depop(166526);
   quest::depop(166504);
   quest::depop(166519);
   quest::depop(166539);   
   quest::depop(166506);
   quest::depop(166520);
   quest::depop(166532);
   quest::depop(166507);
   quest::depop(166725);
   quest::depop(166540);
   quest::depop(166541);
   quest::depop(166542);
   quest::depop(166724);
   quest::depop(166726);
   quest::depop(166521);
   quest::depop(166540);
   quest::depop(166508); 
   quest::depop(166734); 
   quest::depop(166735);
   quest::depop(166736);
   quest::depop(166737);
   quest::depop(166738);
   quest::depop(166739);
   quest::depop(166740);
   quest::depop(166741);
   quest::depop(166742);
   quest::depop(166743);
   quest::depop(166744);
   quest::depop(166745);
   quest::depop(166746);
   quest::depop(166747);
   quest::depop(166748);
   quest::depop(166749);
   quest::depop(166750);
   quest::depop(166751);
   quest::depop(166752);
   quest::depop(166753);
   quest::depop(166754);
   quest::depop(166755);
   quest::depop(166756);
   quest::depop(166757);
   quest::depop(166758);
   quest::depop(166759);
   quest::depop(166760);
   quest::depop(166761);
   quest::depop(166762);
   quest::depop(166763);
   quest::depop(166764);
   quest::depop(166765);
   quest::depop(166766);
   quest::depop(166767);
   quest::depop(166768);
   quest::depop(166769);
   quest::depop(166728);
   quest::depop(166729);
   quest::depop(166730);
   quest::depop(166731);
   quest::depop(166732);
   quest::depop(166733);
   quest::depop(166791);
   quest::depop(166254);
   quest::depop(166210);
   quest::depop(166209);
   quest::depop(166175);
   quest::depop(166211);
   quest::depop(166170);
   quest::depop(166777);
   quest::depop(166829);
   quest::depop(166830);
   quest::depop(166831);
   quest::depop(166832);
   quest::depop(166833);
   quest::depop(166834);
   quest::depop(166835);
   quest::depop(166836);
   quest::depop(166837);
   quest::depop(166838);
   quest::depop(166839);
   quest::depop(166840);
   quest::depop(166841);
   quest::depop(166842);
   quest::depop(166843);
   quest::depop(166844);
   quest::depop(166845);
   quest::depop(166846);
   quest::depop(166847);
   quest::depop(166848);
   quest::depop(166849);
   quest::depop(166850);
   quest::depop(166851);
   quest::depop(166852);
   quest::depop(166853);
   quest::depop(166854);
   quest::depop(166855);
   quest::depop(166856);
   quest::depop(166857);
   quest::depop(166858);
   quest::depop(166859);
   quest::depop(166860);
   quest::depop(166861);
   quest::depop(166862);
   quest::depop(166863);
   quest::depop(166864);
   quest::depop(166865);
   quest::depop(166866);
   quest::depop(166867);
   quest::depop(166870);
   quest::depop(166784);
   quest::depop(166871);
   quest::depop(166872);
   quest::depop(166873);
   quest::depop(166874);
   quest::depop(166875);
   quest::depop(166876);
   quest::depop(166877);
   quest::depop(166878);
   quest::depop(166879);
   quest::depop(166880);
   quest::depop(166881);
   quest::depop(166882);
   quest::depop(166883);
   quest::depop(166884);
   quest::depop(166885);
   quest::depop(166886);
   quest::depop(166887);
   quest::depop(166888);
   quest::depop(166889);
   quest::depop(166890);
   quest::depop(166891);
   quest::depop(166892);
   quest::depop(166893);
   quest::depop(166894);
   quest::depop(166895);
   quest::depop(166896);
   quest::depop(166897);
   quest::depop(166898);
   quest::depop(166899);
   quest::depop(166900);
   quest::depop(166901);
   quest::depop(166902);
   quest::depop(166903);
   quest::depop(166904);
   quest::depop(166905);
   quest::depop(166906);
   quest::depop(166907);
   quest::depop(166868);
   quest::depop(166869);
   quest::depop(166147);
   quest::depop(166156);
   quest::depop(166811);
   quest::depop(166908);
   quest::depop(166909);
   quest::depop(166910);
   quest::depop(166911);
   quest::depop(166912);
   quest::depop(166913);
   quest::depop(166914);
   quest::depop(166915);
   quest::depop(166916);
   quest::depop(166917);
   quest::depop(166918);
   quest::depop(166919);
   quest::depop(166920);
   quest::depop(166921);
   quest::depop(166922);
   quest::depop(166921);
   quest::depop(166923);
   quest::depop(166924);
   quest::depop(166925);
   quest::depop(166926);
   quest::depop(166927);
   quest::depop(166928);
   quest::depop(166929);
   quest::depop(166930);
   quest::depop(166931);
   quest::depop(166932);
   quest::depop(166933);
   quest::depop(166934);
   quest::depop(166935);
   quest::depop(166936);
   quest::depop(166937);
   quest::depop(166938);
   quest::depop(166939);
   quest::depop(166940);
   quest::depop(166941);
   quest::depop(166942);
   quest::depop(166943);
   quest::depop(166944);
   quest::depop(166945);
   quest::depop(166946);
   quest::depop(166947);
   quest::depop(166948);
   quest::depop(166949);
   quest::depop(166441);
   quest::depop(166412);
   quest::depop(166495); #a grimling guard
   quest::depop(166792); #Prince Garzemort
   quest::depop(166594); #a grimling guard
   quest::depop(166782); #a grimling conqueror
   quest::depop(166780); #a grimling manacrafter
   quest::depop(166778); #a grimling thief
   quest::depop(166781); #a grimling soldier
   quest::depop(166779); #a grimling spiritist
   quest::depop(166156); #a sonic guardian
   quest::depop(166784); #a grimling chieftan
   quest::depop(166147); #a sonic guardian
   quest::depop(166785); #a sonic ravager
   quest::depop(166786); #a sonic defender
   quest::depop(166787); #a sonic defender
   quest::depop(166788); #a sonic tracker
   quest::depop(166789); #a sonic warwolf
   quest::depop(166794);
   quest::spawn2(166078,498128,0,1980,979.7,31.2,57.4); #Guard Kliknaw
   quest::spawn2(166079,0,0,2011.9,991.9,34.2,111.5); #Guard Rawkkun
   quest::spawn2(166080,0,0,1921.9,1009.9,29.4,33.5); #Sergeant Tylah
   quest::spawn2(166081,0,0,2026.6,1071,25.6,93.2); #Rittuh Purrik
   quest::spawn2(166083,0,0,1924.8,1096.9,25.6,170.4); #Kerr Tiparr
   quest::spawn2($a,0,0,1735.3,3406.9,,166.4,145.1); #an_owlbear_ravager an_owlbear_razorbeak an_owlbear_razorclaw an_owlbear_grappler
   quest::spawn2($b,0,0,1793.5,3391.5,,160.8,159); #an_owlbear_ravager an_owlbear_razorbeak an_owlbear_razorclaw an_owlbear_grappler
   quest::spawn2($c,0,0,1751.2,2885.1,,163.7,199.6); #an_owlbear_ravager an_owlbear_razorbeak an_owlbear_razorclaw an_owlbear_grappler
   quest::spawn2($d,0,0,1526.7,2495.8,164.7,112); #an_owlbear_ravager an_owlbear_razorbeak an_owlbear_razorclaw an_owlbear_grappler
   quest::spawn2($e,4,0,1376.4,2167.1,163.3,48.2); #an_owlbear_ravager an_owlbear_razorbeak an_owlbear_razorclaw an_owlbear_grappler
   quest::spawn2($f,0,0,1827.9,3639.5,158.3,127.1); #an_owlbear_ravager an_owlbear_razorbeak an_owlbear_razorclaw an_owlbear_grappler
   quest::spawn2($g,0,0,1800.5,3166.7,160.1,222.5); #an_owlbear_ravager an_owlbear_razorbeak an_owlbear_razorclaw an_owlbear_grappler
   quest::spawn2($h,0,0,1729.7,3499.8,158.7,65); #an_owlbear_ravager an_owlbear_razorbeak an_owlbear_razorclaw an_owlbear_grappler
   quest::spawn2($i,0,0,1983,3537,158.3,178); #an_owlbear_fleshrender an_owlbear_denmaster Skrietat Chu
   quest::spawn2($j,498009,0,1443.9,1270,150.5,246.7); #an_owlbear_cub
   quest::spawn2($k,498093,0,1498.9,936,53.1,279.5); #an_owlbear_ravager an_owlbear_razorclaw an_owlbear_grappler
   quest::spawn2($l,9,0,904.7,2337.1,245.3,142.5); #an_owlbear_ravager an_owlbear_razorbeak an_owlbear_razorclaw an_owlbear_grappler
   quest::spawn2($m,0,0,1893.1,3077.7,162.3,248.5); #an_owlbear_ravager an_owlbear_razorbeak an_owlbear_razorclaw an_owlbear_grappler
   quest::spawn2($n,0,0,1534.5,2806.4,164.1,81.6); #an_owlbear_ravager an_owlbear_razorbeak an_owlbear_razorclaw
   quest::spawn2($o,0,0,2072.1,3100.1,154.5,219.6); #an_owlbear_ravager an_owlbear_razorbeak Skrietat Cha
   quest::spawn2($p,3,0,1126.5,2150,143.9,63); #an_owlbear_spurclaw a_patrolling_owlbear
   quest::spawn2($q,,0,0,1542.4,2979.7,171.5,98); #an_owlbear_spurclaw a_patrolling_owlbear
   quest::spawn2($r,0,0,2002,3027,152.5,243.4); #an_owlbear_spurclaw a_patrolling_owlbear
   quest::spawn2($s,498126,0,1271.8,2131.7,171.6,244.8); #an_owlbear_gatherer
   quest::spawn2(166660,0,0,1996.1,3623.3,166.6,124.4); #an_owlbear_grappler
   quest::spawn2(166438,0,0,1318.9,1109.9,115.5,194.1); #an_owlbear_razorbeak
   quest::spawn2(166425,0,0,398,1942,133.1,25.5); #an_owlbear_ravager
   quest::spawn2(166665,0,0,1618.4,2593.6,162.9,153.1); #an_owlbear_razorbeak
   quest::spawn2(166492,0,0,516,1019,35.5,112.5); #an_owlbear_hunter
   quest::spawn2(166493,0,0,1265.8,2169.8,172,371); #an_owlbear_hunter
   quest::spawn2(166411,5,0,-1212,795.5,9.9,197.6); #Skrietat_Chet
   quest::spawn2(166439,0,0,1573.9,2270.5,160,208.8); #an_owlbear_razorbeak
   quest::spawn2(166440,0,0,1525.1,2349.4,163.1,99.8); #an_owlbear_razorbeak
   quest::spawn2(166441,0,0,1671.1,2462.,158,189.8); #an_owlbear_razorbeak
   quest::spawn2(166442,0,0,1698.9,2795.8,167.1,212.9); #an_owlbear_razorbeak
   quest::spawn2(166443,0,0,1697.9,3060.4,159.8,180.1); #an_owlbear_razorbeak
   quest::spawn2(166444,0,0,2063.2,3211,159.1,170.1); #an_owlbear_razorbeak
   quest::spawn2(166468,0,0,1975.4,3275.8,159.8,135.4); #an_owlbear_grappler
   quest::spawn2(166469,0,0,1949.6,3431.9,150.5,211.8); #an_owlbear_grappler
   quest::spawn2(166412,498125,0,1949.6,3431.9,150.5,211.8); #Prince_Skriatat
   quest::spawn2($t,0,0,-2969.5,466.9,55.6,24.6); #Ghowlik The_Fabled_Ghowlik a_sonic_packleader
   quest::spawn2($u,0,0,-2887.4,627.4,71.1,132.4); #a_sonic_hunter a_sonic_devourer a_sonic_warwolf a_sonic_stalker
   quest::spawn2($v,0,0,-2997.8,516.2,54,35); #a_sonic_devourer a_sonic_hunter a_sonic_ravager a_sonic_stalker
   quest::spawn2($w,0,0,-3002.5,573.7,52.7,63.5); #a_sonic_tracker a_sonic_devourer a_sonic_warwolf
   quest::spawn2($x,0,0,-3113.2,618.4,64.2,99.1); #a_sonic_ravager a_sonic_hunter a_sonic_devourer
   quest::spawn2($y,0,0,-2991.5,666.2,62,109); #a_sonic_devourer a_sonic_hunter a_sonic_ravager a_sonic_stalker
   quest::spawn2($z,0,0,-2942.5,670.6,64.7,129.9); #a_sonic_tracker a_sonic_devourer a_sonic_warwolf a_sonic_ravager
   quest::spawn2($aa,0,0,-2906.6,663.3,71.5,153.9); #a_sonic_tracker a_sonic_warwolf a_sonic_devourer a_sonic_hunter
   quest::spawn2($ab,0,0,-2867.1,582.8,69.9,159); #a_sonic_stalker a_sonic_ravager a_sonic_hunter a_sonic_warwolf
   quest::spawn2($ac,0,0,-2832.2,572.7,70.1,89.4); #a_sonic_ravager a_sonic_stalker a_sonic_warwolf
   quest::spawn2($ad,0,0,-2929.9,409,71.2,5.1); #a_sonic_hunter a_sonic_ravager a_sonic_stalker  
   quest::spawn2($af,0,0,-2845.7,507.9,70.4,20.9); #a_sonic_ravager a_sonic_hunter a_sonic_devourer
   quest::spawn2($ag,0,0,-2932.2,525,55.8,62.2); #a_sonic_hunter a_sonic_ravager a_sonic_stalker  
   quest::spawn2($ah,0,0,-2931.4,584.7,54.5,77.4); #a_sonic_tracker a_sonic_devourer a_sonic_warwolf
   quest::spawn2($dn,0,0,-620.4,-2810.5,85.5,255.1); #a grimling spiritist a grimling chieftan a grimling priest
   quest::spawn2($do,0,0,-493.6,-2737.4,54.8,195.9); #a grimling thief a grimling soldier
   quest::spawn2($dp,0,0,-506.8,-2754.4,46.1,196.9); #a grimling spiritist a grimling mystic
   quest::spawn2($dq,0,0,-505.1,-2719.4,37.7,213.5); #a grimling overseeer a grimling conqueror
   quest::spawn2($dr,0,0,-668.2,-2681.7,34,212.1); #a grimling recruit a grimling soldier
   quest::spawn2($ds,0,0,-675,-2663.7,17.7,205.8); #a grimling spiritist a grimling mystic
   quest::spawn2($dt,0,0,-685.1,-2694.8,22.7,214.9); #a grimling conqueror a grimling soldierr
   quest::spawn2($du,0,0,-590.1,-2659.5,26.1,205.1); #a grimling overseeer a grimling recruit
   quest::spawn2($dv,0,0,-604.4,-2671.7,24.4,208.6); #a grimling spiritist a grimling mystic 
   quest::spawn2($dw,0,0,-591,-2642.5,14.8,211.2); #a grimling overseeer a grimling conqueror
   quest::spawn2($dx,0,0,-579.2,-2616.6,10.7,183.9); #a grimling recruit a grimling soldier
   quest::spawn2($dy,0,0,-569.2,-2601.2,20.7,185.9); #a grimling  spiritist a grimling mystic
   quest::spawn2($dz,0,0,-579.3,-2584.3,9.2,190.5); #a grimling overseeer a grimling conqueror 
   quest::spawn2($ea,0,0,-537,-2581,13.8,211.6); #a grimling thief a grimling soldier 
   quest::spawn2($eb,0,0,-521.1,-2566.1,21.3,194.1); #a grimling soldier a grimling recruit
   quest::spawn2($ec,0,0,-533.1,-2548.8,6.4,187.9); #a grimling recruit a grimling soldier 
   quest::spawn2($ed,0,0,-444.8,-2514.3,28,203.9); #a grimling thief a grimling soldier 
   quest::spawn2($ef,0,0,-448.2,-2497.1,20.7,199.5); #a grimling  spiritist a grimling mystic
   quest::spawn2($eg,0,0,-458.8,-2528,15.8,198.5); #a grimling overseeer a grimling conqueror 
   quest::spawn2(166170,0,0,-626.6,-2842.3,82.8,253.4); #a grimling spiritist
   quest::spawn2(166210,0,0,-631.3,-2801.4,68.1,254.5); #a grimling bodyguard
   quest::spawn2(166209,0,0,-610.7,-2801.4,68.1,254.5); #a grimling bodyguard
   quest::depop();
   }

