# Shaman Epic 1.0
# Froham_the_Forgotten (98047)
# Erud's Crossing (erudxing)
# Aramid September 2006

sub EVENT_SAY {
    my $waitin = quest::saylink("waitin", 1);
    my $soon = quest::saylink("soon", 1);
    my $long = quest::saylink("long", 1);
    ...
    if($text=~/waitin/i) {
        quest::say(". just waitin' . . . it'll be here $soon, I s'pose");
    }
    if($text=~/soon/i) {
