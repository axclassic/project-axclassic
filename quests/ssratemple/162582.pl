...
my $BloodCoolDownTime = 0;
my $EmpRepopTime = 0;
my $EmpPrepTime = 0;
my $EmpPrep = 0;

sub EVENT_SPAWN {
    $BloodCoolDownTime = int(rand(60)) + 180; #Waiting time to reattempt Emp after failure (Current setting: 3-4 hours)
    $EmpRepopTime = int(rand(2880)) + 4320; #Respawn time forEmp after success (Current setting: 3-5 days)
    $EmpPrepTime = 150; #Seconds before Emp becomes targetable after killing Blood/Golem (Current setting: 2min 30sec)
    $EmpPrep = 0;
...
sub EVENT_TIMER {
	if($timer eq "EmpCycle") {
		if(!defined($qglobals{Emperor}) && !defined($qglobals{BloodCoolDown})) {
