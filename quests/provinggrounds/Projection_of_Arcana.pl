...
sub EVENT_SAY {
    ...
    my $specialization = quest::saylink("Specialization", 1);
    my $efficiency = quest::saylink("Efficiency", 1);
    ...
    if($text=~/hail/i) {
        ...
        if($client->IsGrouped()) {
            ...
            for($i=0; $i<$nMembers; $i++) {
                ...
                if($the_group->IsClient($member)) {
                    ...
                    my $QuestGlobalValue = $member->GetQGlobal("mpg_group_efficiency_lockout");
                    if($QuestGlobalValue) {
                        $flagged = 0;
                        my $qgduration = $member->GetQGlobalDuration("mpg_group_efficiency_lockout");
                        my $timeleft = plugin::GetTimeLeft($qgduration, "DHM");
                        $member->Message(15, "$name is locked out of the Efficiency trial for $timeleft.");
                    }
                    if($flagged == 1) {
            ...
            my $QuestGlobalValue = $client->GetQGlobal("mpg_group_efficiency_lockout");
            if($QuestGlobalValue) {
                $flagged = 0;
        ...
        if($flagged == 1) {
            ...
            my $instance_ID = quest::CreateInstance('chambersd', 1, 3600);
            my $arraySize = @groupclients;
            if($arraySize > 1) {
                quest::AssignGroupToInstance($instance_ID);
                quest::setgroupglobal("chambersd1", $instance_ID, 7, "H1");
            }
            else {
                quest::AssignToInstance($instance_ID);
                quest::setglobal(${name}."chambersd1", $instance_ID, 7, "H1");
            }
            quest::say("Mastery of a single armament of choice is not enough. The time will come when the flexibility gained from proficiency in multiple weapon styles will turn the tide of a battle and the alternative is failure. You must come to understand the value of efficency. If you have this ability, now is the opportunity to prove your worth!");
        }
    }
    elsif($text=~/specialization/i) {
        my $flagged = 1;
        my @clients;
        if($client->IsGrouped()) {
            my $the_group = $client->GetGroup();
            ...
            for($i=0; $i<$nMembers; $i++) {
                ...
                if($the_group->IsClient($member)) {
                    my $ClientName = $member->GetCleanName();
                    my $QuestGlobalValue = $member->GetQGlobal("mpg_raid_specialization_lockout");
                    if($QuestGlobalValue) {
                        $flagged = 0;
                        my $qgduration = $member->GetQGlobalDuration("mpg_raid_specialization_lockout");
                        my $timeleft = plugin::GetTimeLeft($qgduration, "DHM");
                        $member->Message(15, "$name is currently locked out of the Specialization trial for $timeleft.");
                    }
                    if($flagged == 1) {
                        push(@clients, $member);
                    }
        ...
        if($flagged == 1) {
            ...
            my $instance_ID = quest::CreateInstance('chambersd', 2, 10800);
            my $arraySize = @clients;
            if($arraySize > 1) {
                quest::AssignGroupToInstance($instance_ID);
                quest::setgroupglobal("chambersd2", $instance_ID, 7, "H3");
            }
            else {
                quest::AssignToInstance($instance_ID);
                quest::setglobal(${name}."chambersd2", $instance_ID, 7, "H3");
            }
            quest::say("At times it pays to be focused in your abilities, and at times it does not. This trial will test your skill at using your strengths when they are most effective, and otherwise relying on your companions. If you have this ability now is the time to prove your worth!");
        }
    }
