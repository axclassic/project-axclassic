...
sub EVENT_SAY {
    ...
    my $ingenuity = quest::saylink("Ingenuity", 1);
    my $adaptation = quest::saylink("Adaptation", 1);
    ...
    if($text=~/hail/i) {
        ...
        if($client->IsGrouped()) {
            ...
            for($i=0; $i<$nMembers; $i++) {
                ...
                if($the_group->IsClient($member)) {
                    ...
                    my $QuestGlobalValue = $member->GetQGlobal("mpg_group_ingenuity_lockout");
                    if($QuestGlobalValue) {
                        $flagged = 0;
                        my $qgduration = $member->GetQGlobalDuration("mpg_group_ingenuity_lockout");
                        my $timeleft = plugin::GetTimeLeft($qgduration, "DHM");
                        $member->Message(15, "$name is locked out of the Ingenuity trial for $timeleft.");
                    }
                    if($flagged == 1) {
            ...
            my $QuestGlobalValue = $client->GetQGlobal("mpg_group_ingenuity_lockout");
            if($QuestGlobalValue) {
                $flagged = 0;
        ...
        if($client->IsGrouped()) {
            ...
            for($i=0; $i<$nMembers; $i++) {
                ...
                if($the_group->IsClient($member)) {
                    my $ClientName = $member->GetCleanName();
                    my $QuestGlobalValue = $member->GetQGlobal("mpg_raid_adaptation_lockout");
                    if($QuestGlobalValue) {
                        $flagged = 0;
                        my $qgduration = $member->GetQGlobalDuration("mpg_raid_adaptation_lockout");
                        my $timeleft = plugin::GetTimeLeft($qgduration, "DHM");
                        $member->Message(15, "$name is currently locked out of the Adaptation trial for $timeleft.");
                    }
                    if($flagged == 1) {
                        push(@clients, $member);
                    }
        ...
        if($flagged == 1) {
            ...
            my $instance_ID = quest::CreateInstance('chamberse', 2, 10800);
            my $arraySize = @clients;
            if($arraySize > 1) {
                quest::AssignGroupToInstance($instance_ID);
                quest::setgroupglobal("chamberse2", $instance_ID, 7, "H3");
            }
            else {
                quest::AssignToInstance($instance_ID);
                quest::setglobal(${name}."chamberse2", $instance_ID, 7, "H3");
            }
