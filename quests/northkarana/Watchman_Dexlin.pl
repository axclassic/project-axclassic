...
sub EVENT_SAY {
  ...
  if($text=~/brave fighter/i){
    ...
    quest::signalwith(13118,2);                   # Signals Tiny Skeletons - "asks them" if any of them are spawned, if so they set $skelup = 1 (prevents triggering multiple spawns)
...
sub EVENT_TIMER{
  if($timer == 1){
    if($skelup == "0"){                                 # If there are no tiny skeletons currently spawned
      ...
      quest::spawn(13118,106,0,-1839.87,829.51,-9.38);  # Spawn tiny skeletons
      quest::spawn(13118,106,0,-2149.24,1035.51,18.42);
      quest::spawn(13118,106,0,-1713.59,1075.06,-10.47);
      quest::spawn(13118,106,0,-1690.69,363.48,3.60);
      quest::spawn(13118,106,0,-2604.56,222.71,10.02);
      quest::signalwith(13118,1);                       # Signal tiny skeletons - tell them to attack player $skelatk
    }
