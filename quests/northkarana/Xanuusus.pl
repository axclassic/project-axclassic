...
sub EVENT_ITEM {
    if(plugin::check_handin(\%itemcount, 20483 => 1, 20484 => 1)) {
        ...
        quest::shout2("Denizens of The Rathe. Please join me in congratulating $name for earning the Ranger Epic: Earthcaller!");
        if($class ne 'Ranger') {
        ...
        quest::ding(); quest::exp(350000);
        quest::shout2("Denizens of The Rathe. Please join me in congratulating $name for earning the Druid Epic: Nature Walkers Scimitar!");
        if($class ne 'Druid') {
            quest::shout2("...probably for a Bot since $name is a $class");
        }
    }
