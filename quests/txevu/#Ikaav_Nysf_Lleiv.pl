...
function event_timer(e)
	if (e.timer == "spawn_golem") then
		...
		if ($entity_list->GetMobByNpcTypeID(297089)) then stonemasters = stonemasters + 1 end
		if ($entity_list->GetMobByNpcTypeID(297088)) then stonemasters = stonemasters + 1 end
		if ($entity_list->GetMobByNpcTypeID(297087)) then stonemasters = stonemasters + 1 end
...
function event_hp(e)
    ...
    e.self:AddAISpell(0, 1236, 1, -1, 45, -350)
		if ($entity_list->GetMobByNpcTypeID(297089)) then
			eq.get_entity_list():MessageClose(e.self,0,120,0,"The shielding on one of the stonemasters falters as Ikaav Nysf Lliev focuses her energies inward, intensifying her own powers.")
			eq.signal(297089,1)
			eq.set_next_hp_event(50)
		end
	elseif (e.hp_event == 50) then
		eq.set_timer("malicious_intent",20000)
		if ($entity_list->GetMobByNpcTypeID(297088)) then
			eq.get_entity_list():MessageClose(e.self,0,120,0,"The shielding on one of the stonemasters falters as Ikaav Nysf Lliev focuses her energies inward, intensifying her own powers.")
			eq.signal(297088,1)
	...
	elseif (e.hp_event == 25) then
		...
		if ($entity_list->GetMobByNpcTypeID(297087)) then
			eq.get_entity_list():MessageClose(e.self,0,120,0,"The shielding on the final stonemaster falters as Ikaav Nysf Lliev focuses her energies inward, intensifying her own powers.")
			eq.signal(297087,1)
		end
	end
end
