...
# Note: $count will always be one word higher than what was typed due to junk word added
    ...
    if($firstword=~/getcondition/i){
      if($count==4){
        ...
        quest::say("I will tell the value for the spawn condition id for the specified zonename.");
      }
    }
    ...
    if($firstword=~/setcondition/i){
      if($count==5){
        ...
        quest::say("I will set the spawn condition for the given id to the given value in the zonename you specify.");
      }
