...
sub EVENT_SAY {
   ...
   if($text=~/hail/i && $ulevel >= 59) {
      $client->Message(14,"Hello $name , I am devistated. This morning I broke the only $mirror I had and I am now unable to look at myself after casting my illusions. I need your help..");
      $client->Message(12,"If you die during this quest, I can send you $back to your instance for as long as it is active. If it already expired your corpse will be available in Lesser Faydark and I can $send you there...");
   }
   if($text=~/hail/i && $ulevel < 59) {
   ...
   if($text=~/send/i && $ulevel >= 59 && !defined($qglobals{$name."mmca"})) {
      $client->Message(14,"I am going to send you to the graveyard in Lesser Faydark. Your corpse will be available there after your quest instance has expired, not before..");
      quest::movepc(57, 3832.2, -684.4, -1.3);
      return 1;
   }
   if($text=~/send/i && $ulevel >= 59 && defined($qglobals{$name."mmca"})) {
