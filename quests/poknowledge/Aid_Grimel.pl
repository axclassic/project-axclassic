...
sub EVENT_SAY {
	if($text=~/hail/i) {
		...
		if(defined($qglobals{pop_cod_final}) && ($qglobals{pop_cod_final} == 1)) {
			#Must have killed Bertox and rescued Mylik and Adler
			...
			if(($text=~/skill in brewing/i) && plugin::check_hasitem($client, 16249)) {
				#Hardened Leather Signet
				if($client->GetRawSkill(65) >= 220) {
					#Brewing
			...
			if(($text=~/put my jewel craft skills to the test/i) && plugin::check_hasitem($client, 16250)) {
				#Clay Signet
				if($client->GetRawSkill(68) >= 220) {
					#Jewelcraft
			...
			if(($text=~/ready to use some clay/i) && plugin::check_hasitem($client, 16251)) {
				#Wooden Signet
				if($client->GetRawSkill(69) >= 220) {
					#Pottery
			...
			if(($text=~/am skilled with the needle/i) && plugin::check_hasitem($client, 16252)) {
				#Metal Signet
				if($client->GetRawSkill(61) >= 220) {
					#Tailoring
			...
			if(($text=~/skills with a fletching knife/i) && plugin::check_hasitem($client, 32800)) {
				#Marked Signet
				if($client->GetRawSkill(64) >= 220) {
					#Fletching
			...
			if(($text=~/master chef/i) && plugin::check_hasitem($client, 16254)) {
				#Runed Signet
				if($client->GetRawSkill(60) >= 220) {
					#Baking
...
sub EVENT_ITEM {
	if(plugin::check_handin(\%itemcount, 15985 => 1)) {
		#Imbued Breastplate
	...
	elsif(plugin::check_handin(\%itemcount, 15993 => 1, 17179 => 1)) {
		#Portable Drink and Portable Drink Barrel
	...
	elsif(plugin::check_handin(\%itemcount, 15991 => 1)) {
	...
	elsif(plugin::check_handin(\%itemcount, 16246 => 1)) {
		#Filled Sacred Urn
	...
	elsif(plugin::check_handin(\%itemcount, 15986 => 1)) {
	...
	elsif(plugin::check_handin(\%itemcount, 16247 => 1)) {
	...
	elsif(plugin::check_handin(\%itemcount, 16248 => 1)) {
		#Food Satchel
