...
sub EVENT_ITEM {
    if(plugin::check_handin(\%itemcount, 8956 => 1) && $faction == 1) {
        #Idol of karana
    ...
    elsif (plugin::check_handin(\%itemcount, 8964 => 1, 8963 => 1, 8962 => 1 ) && $qglobals{galeweaver} == 1) {
        # Writ, sphere, and scroll turn in
    ...
    elsif (plugin::check_handin(\%itemcount, 8966 => 1) && $qglobals{galeweaver} == 2) {
        #Turn in orb of the gale
