sub EVENT_SAY {
 if($wizepicK == 1) {
  ...
  if($text =~ /what oil/i) {
	...
	quest::summonitem(16858); #WRONG! should be SPARE GEARS
...
sub EVENT_ITEM {
	...
	#oil Handin
	if($wizepicK == 1 && $itemcount{14349} == 1) {
	...
	if($wizepicK == 1 && $itemcount{14333} == 1 && $itemcount{14332} == 1) {
		quest::summonitem(14339); #Staff of Gabstik 
		...
		quest::faction(342, 30); #Truespirit
	#just Cazic's Skin
	} elsif($wizepicK == 1 && $itemcount{14332} == 1) {
