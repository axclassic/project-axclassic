...
sub EVENT_ITEM {
  if($itemcount{59035} == 2) {
  quest::delglobal("luggald");
  quest::setglobal("luggald","1","0","D255");
  $luggald=undef;
    ...
    if($class eq "Necromancer") {
      quest::summonitem(59019);
    }
    elsif ($class eq "Shadowknight") {
      quest::summonitem(59006);
    }
    else {
  ...
  if(($itemcount{59050} == 1) && ($itemcount{59036} == 1) && ($itemcount{59051} == 1) && ($luggald == 1)) {
