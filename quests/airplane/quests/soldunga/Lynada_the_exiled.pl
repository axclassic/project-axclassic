...
sub EVENT_ENTER{ #PC enters and what's there depops and repops required level
	if($ulevel >= 51){
	quest::depop;	
	quest::spawn2(31147,0,0,$x,$y,$z,$h);	#This is the new High level NPC
	}
	if($ulevel <= 50){ 
	quest::depop;	
	quest::spawn2(31001,0,0,$x,$y,$z,$h);	 #This is the original npc that's in the database - Changed its respawn time to 2000 so it won't spawn double while you're there, if its for quest - if not then not, needed.
	}	
}
