...
sub EVENT_ITEM { 
	...
	if (plugin::check_handin(\%itemcount, 1056 => 1)){ # Faded Cloak
		quest::say("You found our Linara! That is wonderous news! Tolkar was right to reward you, but this faded old cloak is not enough... Allow me to restore it to it's former grandeur!");
		quest::summonitem(1057); # Mystic Cloak
	} elsif (plugin::check_handin(\%itemcount, 1598 => 1)){ # Black Stone Candlestick
