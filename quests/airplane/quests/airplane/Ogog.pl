...
if(($text=~/smash/i) && ($class eq "Warrior")){
...
if(($text=~/bash/i) && ($class eq "Warrior")){
...
if ($class ne "Warrior") {
    quest::emote("shrugs.");
    quest::say("You so not smart, you not even warrior!");
    quest::depop();
    } 
...
sub EVENT_ITEM {
  ...
  elsif(plugin::check_handin(\%itemcount, 20980 => 1, 20979 => 1, 20956 => 1)) { #warrior test of smash using djinni war blade, virulent poison, mottled spiroc

feather
    quest::summonitem(11674); #dagas
     quest::ding(); quest::exp(841790);
    quest::say("Yuz do good! Take dis!");
    quest::depop();
    }
