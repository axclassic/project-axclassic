...
sub EVENT_SAY {
my $discovered = quest::saylink("discovered", 1);
my $materials = quest::saylink("materials", 1);
my $pen = quest::saylink("pen", 1);
my $ink = quest::saylink("ink", 1);
my $paper = quest::saylink("paper", 1);
  ...
  if ($text=~/discover/i) {
    quest::say("I was researching a new spell, and discovered new information concerning the history of the Serpent, but I need some $materials.");
  }
  if ($text=~/materials/i) {
...
sub EVENT_ITEM {
my $teacher = quest::saylink("teacher", 1);
  ...
  if (plugin::check_handin(\%itemcount, 10600 => 1, 10601 => 1, 10602 => 1)) {
    ...
    quest::summonitem(10603);
  	...
  	quest::summonitem(59016);#Spell: Scryer's Trespass
  	 quest::ding(); quest::exp(1750000);#1% of level 51 xp
  }
  else {
