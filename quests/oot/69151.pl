...
#no warnings 'all';
    if($event4==1) {
        #Pitch Black Event
        if(((($zonetime >= 1800)&&($zonetime <= 2359))||(($zonetime >= 0)&&($zonetime <= 500)))&&($sirens==24)) {
            quest::unique_spawn(69309,71,0,7558.7,-2090.5,1.0,61.4);
        }#Kidd Spawn

        if(((($zonetime >= 1800)&&($zonetime <= 2359))||(($zonetime >= 0)&&($zonetime <= 500)))&&($sirens==37)) {
            quest::depopall(69309);
        }#Kidd Spawn
    }
    #if($forceboats==1){if($debugpl)==1){quest::shout("Checking");}
    if($ootlastseen == $sirens) {
        #boat hasnt changed waypoints yet
        if(((($sirens >= 0)&&($sirens < 14))||($sirens == 100))&&($oottimer >= 32)) {
            #ifwaypoint hasnt changed in x spawns, and boat in Butcher
        ...
        elsif(($sirens >= 14)&&($sirens < 39)&&($oottimer >= 32)) {
            #ifwaypoint hasnt changed in x spawns, and boat in OOT -> FP
        ...
        elsif(($sirens >= 39)&&($sirens < 60)&&($oottimer >= 32)) {
            #ifwaypoint hasnt changed in x spawns, and boat in Freeport
        ...
        elsif(($sirens >= 60)&&($sirens < 100)&&($oottimer >= 32)) {
            #ifwaypoint hasnt changed in x spawns, and boat in OOT -> Butcher
    ...
    if($sirens==14) {
        ...
        $sirens = undef;
        if((($zonetime >= 1800)&&($zonetime <= 2359))||(($zonetime >= 0)&&($zonetime <= 500))) {
            #nighttime
            if($event4==1) {
                #Pitch Black Event
                quest::spawn2(69156,72,0,10784.3,1151.7,-19,183.9);
            }
            else {
                quest::spawn2(69134,62,0,10784.3,1151.7,-40,183.9);
            }
        }#daytime boat
        else {
            #daytime
