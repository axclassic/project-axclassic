# Oracle_of_K`Arnon (69310) / Oracle_of_K`Arnon (69044)
...
#this next script is to respawn a higher or lower  level NPC for trivial zone questing
#Angelox
...
sub EVENT_ENTER{ #PC enters what's there depops and repops required level
	if($ulevel >= 51) {
		quest::spawn2(69310,0,0,$x,$y,$z,$h);   #This is the new level 52 Oracle
        quest::depop();
	}
	if($ulevel <= 50) {
        #This is the level  36 Oracle that's in the database - Changed his respawn time to 2000 so he won't spawn double while you're there..
		quest::spawn2(69044,0,0,$x,$y,$z,$h);
        quest::depop();
	}
}

sub EVENT_EXIT {
    #have to depop when PC leaves (unless you killed him), else you get duped by the same NPC with the normal respawn.
	quest::depop();
}
...
sub EVENT_ITEM {
	if(plugin::check_handin(\%itemcount, 18302 => 1)) {
		# Book of Scale
	...
	if(plugin::check_handin(\%itemcount, 18202 => 1)) {
		# Book of Turmoil
	...
	if(plugin::check_handin(\%itemcount, 55274 => 1)) {
		# Letter forthe Oracle
