...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 18211 =>1 )) {
    quest::say("Ah!! Good to have you on board. We lack strong young recruits such as yourself. Take this box. Your task is to seek escaped frogloks near this gate. Apparently, the slaves have been breaking loose from their shackles and attempting to head home. Fill and combine within the box their broken shackles and return the filled box to me for your geozite tool.");  #not live text
  ...
  if (plugin::check_handin(\%itemcount, 12435 =>1, 12436 =>1, 12437 =>1, 5133 =>1 )) {
   quest::emote(' hands you plans for a troopers pike head.');
