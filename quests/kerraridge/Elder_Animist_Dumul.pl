# Elder_Animist_Dumul (74240)

sub EVENT_SAY {
    my $magic = quest::saylink("magic", 1);
    my $name = quest::saylink("name", 1);
    ...
    if($text=~/name/i) {
        quest::say("His name is Draz Nurakk. It appears that he has learned the ways of the Khati Sha to the point of mastery. His magic and will seem to defy even the darkest secular beliefs of his people. We harbor no ill will toward the Iksar, but this individual must be stopped. This leads us to your $mission.");
    }
    if($text=~/mission/i) {
        quest::say("'We need you to meet our scouts and assist them in releasing the spirits that have already been affected by this magic. None of the scouts have reported in for some time, so you will need to find them. Once you find them, show them the seal that I gave you. They will help you to release the spirits and capture the totems. Please have the scouts help you to place the totems in the case that I gave you. If you lose the seal or damage it in combat, please return to me for another. Good luck, $name. You must succeed if we are to maintain the balance of the spiritual realm.");
    }
    if($text=~/lost seal/i) {
...
sub EVENT_ITEM {
    my $situation = quest::saylink("situation", 1);
    my $briefed = quest::saylink("briefed", 1);
    ...
    elsif(plugin::check_handin(\%itemcount, 9060 =>1 )) {
