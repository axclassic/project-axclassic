...
public:
	...
	/**
	 * Creates new password for login server account with info from world server
	 */
	virtual void UpdateLSPasswordInfo(unsigned int id, string password) { }
	
	/**
	 * Makes the one time flag setting.
	 */
	virtual void UpdateLSFirstFlag(unsigned int id) { }
	
	/**
	 * Verifys and sets a one-time flag for password conversion
	 */
	virtual bool GetFirstFlagFromAccountName(string name, unsigned int &flag) { return false; }
