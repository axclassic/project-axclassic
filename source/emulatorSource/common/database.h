...
//	void	ExtraOptions();
	...
	int32	GetBotIDByCharLikeName(const char* charName);
	...
	void	SaveBotGroups(int32 groupid, int32 charid, int32 botid, int16 slot, int32 hp, int32 mana, int tableA, int tableB, int tableC);
	...
	void	SaveNewBotToTemp(int32 groupid, int32 charid, int32 botid, int16 slot, int32 hp, int32 mana);
	...
	int 	BotCanCombatLookUp(int32 zone_id);
	...
	int	GetFirstBotID(int32 charid);
	...
	int	CountNewBots(int32 leaderid); // Check for newbe spawn
	bool 	CountClients(int32 charID); //count clients in group
	...
	bool	ReserveBotName(char* botName);
#endif //EQBOTS
