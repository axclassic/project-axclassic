#ifndef HoT_STRUCTS_H_
#define HoT_STRUCTS_H_
...
struct Spawn_Struct_Bitfields
{
	...
	//
	unsigned   linkdead:1;			// 1	Toggles LD on or off after name. Correct for HoT
	...
	unsigned   trader:1;			// 0	Unk in HoT
	unsigned   buyer:1;			// 0	Unk in HoT
};
...
/*0000*/ char unknown22[26]; 	// Extra 25 bytes for HoT ?
};
...
/*0076*/ uint8	unknown0076[4]; // New for HoT
...
struct ItemBodyStruct
{
	uint32 id;
	uint8 weight;
	uint8 unknownHoT1[3];	// Maybe weight is now a uint32
	uint8 norent;
	uint8 nodrop;
	uint8 attune;
	uint8 size;
