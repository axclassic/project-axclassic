...
#ifdef EQBOTS            
            if (valid){
		int32 CharID = database.GetCharacterID(char_name);
		//clog(WORLD__CLIENT_ERR,"Running Bot creation...");
		database.ReserveBotName(char_name);
		int32 BotID = database.GetBotIDByCharLikeName(char_name);
		database.SetFirstBotOwner(BotID, CharID);
		//database.SaveNewBotToTemp(20,CharID,BotID,1,20,20);
	    }
#endif
			break;
		}
...
//			strncpy(ngs->name,"Notcreated",64);
		...
		case OP_EnterWorld: // Enter world
		{
			...
			if (zoneID == 0 || !database.GetZoneName(zoneID)) {
				...
				database.MoveCharacterToZone(charid, "load");
				clog(WORLD__CLIENT_ERR, "Zone not found in database zone_id=%i, moving char to loading zone character:%s", zoneID, char_name);
			}
...
#ifdef EQBOTS
	int32 BotCount = database.CountNewBots(charid);
	if (BotCount == 1){
	  int32 BoTId =  database.GetFirstBotID(charid);
	  database.SaveNewBotToTemp(20,charid,BoTId,1,24,20);
	}
#endif
...
//	strcpy(pp.servername, WorldConfig::get()->ShortName.c_str());
	...
	else
	{
		...
		{
			if((ClientVersionBit & BIT_TitaniumAndEarlier) && (!SoFClient))
