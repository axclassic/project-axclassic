...
#ifdef EQBOTS
		ReSendPosition_timer(0),
		ReSendManaCheck_timer(0),
		mana_barCheck_timer(0),
		check_attack_timer(0),
...
void Mob::Kill() {
	...
	if (IsBot()){
	   if (HasManaBar())
	     CastToClient()->mana_barCheck_timer.Start(60000);
	  int32 botnpcid = 0;
	  int16 botid = 0;
...
#ifdef EQBOTS
void Mob::BotWarp( float x, float y, float z )  
{ 
   x_pos = x; 
   y_pos = y; 
   z_pos = z; 
   ...
   Mob* target = GetTarget();
    if (target){
     ...
     else if (!ReSendPosition_timer.Enabled()){
       ReSendPosition_timer.Start(200, false);
       SendPosition();
     }
     else if(ReSendPosition_timer.Enabled()) {
	if(ReSendPosition_timer.Check(false)) {
	  ReSendPosition_timer.Start(300, false);
	  SendPosition();
	}
     }
   }
...
///TODO Fix bug that crashes zone after 2 disbands - this "Kill()" works for now.
void Mob::BotKill() {
	 if (IsBot()){
	    if (HasManaBar())
	      CastToClient()->mana_barCheck_timer.Start(60000);
	...
	Death(this, 0, SPELL_UNKNOWN, HIDE);
	 if ((IsBotRaiding()) && (!IsEngaged())){
	  SetAppearance(eaSitting);
	 }
}

void Mob::BotDepop(bool IsZoning) {
      if (IsBot()){
/*	  if (MakeRandomInt(1,100) < 20){
 	    SayRandomBotMessage(36);
 	  }*/
	 if (HasManaBar())
	   CastToClient()->mana_barCheck_timer.Start(60000);
	WipeHateList();
	...
	InterruptSpell();
	int32 ChaRID = database.GetBotOwner(GetNPCTypeID());
	int32 botnpcid = GetNPCTypeID();
	int32 groupID = database.GetBotGroupID(ChaRID);
	UpdateBotStats(GetHP(),GetMana(),groupID,botnpcid);
	database.CleanBotLeaderEntries(botnpcid);
	...
	if(isSitting) {
		...
		if(GetManaRatio() < 90.0f) {
			if(mana_timer.Check(true)) {
			   SetAppearance(eaSitting);
			     ...
			     else if ((!IsBotRaiding()) && (SetChat())){ //Medding Chat
			     bool outDoor = zone->CanCastOutdoor();
			      if (MakeRandomInt(1,320) < 3){
				if (GetRace() == IKSAR)
				  SayRandomBotMessage(10);
				else if ((GetClass() == ENCHANTER) && (GetGender() == 1) && (GetLevel() > 13))
				  ...
				  SayRandomBotMessage(39);
				else
				  SayRandomBotMessage(11); //Rest of Medders
				}
			      }
		...
		if(mana_timer.Check(true)) {
			...
			if((GetManaRatio() > 0) && (GetManaRatio() < 5)){ //Engaged OOM
			  ...
			  if (MakeRandomInt(1,320) < 3){ //Cleric
			    if (GetClass() == CLERIC){
			      if (GetHP() < 100)
			  ...
			  else if (MakeRandomInt(1,420) < 3){ //Classes
			     if (GetClass() == MAGICIAN)
			       SayRandomBotMessage(37);
			     else if (GetClass() == DRUID)
			       SayRandomBotMessage(40);
			     else
			       SayRandomBotMessage(26);//Rest of engaged casters
			  }
			}
		      }
		    }
...
void Mob::SpawnBotTemp(int32 groupId, Client* botOwner) {
	...
	database.CleanBotLeader(botOwner->CharacterID());
	...
	for(int i=0; i<bots; i++) {
		...
		if((tmp = database.GetNPCType(atoi(row[1])))) {
			...
			database.SetBotLeader(npc->GetNPCTypeID(), botOwner->CharacterID(), npc->GetCleanName(), zone->GetShortName(), zone->GetZoneID());
		...
		if((g->BotGroupCount() < 6) && (atoi(row[0]) == groupId)) {
			...
			if(mtmp) {
				...
				int HP = mtmp->GetBotHP(groupId, mtmp->GetNPCTypeID());
				int MANA = mtmp->GetBotMana(groupId, mtmp->GetNPCTypeID());
				...
				mtmp->SetSpellState(botOwner->CharacterID());
				mtmp->ArmorColorUpadate(npc, &errorMessage); //re-colors armor tint on zin

				if (database.CountNewBots(botOwner->CharacterID()) ==1){
				  int Bsay = MakeRandomInt(0, 100);
				  if (Bsay <5)
				    botOwner->Message(5, "%s tells you, \'Use '#bot mana' if you want to know my mana status.\'", mtmp->GetCleanName());
				  else if (Bsay <10)
				    botOwner->Message(5, "%s tells you, \'You can make more bots with #bot create, and Aediles Thrall gives quests for spawning them.\'", mtmp->GetCleanName());
				  else if (Bsay <15)
				    botOwner->Message(5, "%s tells you, \'Spider legs, spiderling legs, fine mosquito wing, fire beetle leg, Sabertooth Cub Canine, we need four of any of those and 2 pp for your level 5 second bot.\'", mtmp->GetCleanName());
				  else if (Bsay <20)
				    botOwner->Message(5, "%s tells you, \'Target yourself and and say '#bot skin' if you want my buff.\'", mtmp->GetCleanName());
				  else if (Bsay <25)
				    botOwner->Message(5, "%s tells you, \'Target another mob and say '#bot attack' to start with my mellee.\'", mtmp->GetCleanName());
				  else if (Bsay <30)
				    botOwner->Message(5, "%s tells you, \'There's much better experience in the dungeons.\'", mtmp->GetCleanName());
				  else if (Bsay <35)
				    botOwner->Message(5, "%s tells you, \'Talk to Ranger Band for some Moonstone quests.\'", mtmp->GetCleanName());
				  else if (Bsay <40)
				    botOwner->Message(5, "%s tells you, \'Norrath Adventurer has quests for the Druid and Wizard gates.\'", mtmp->GetCleanName());
				  else if (Bsay <45)
				    botOwner->Message(5, "%s tells you, \'If you're tired of me bugging you with hints, just make another bot.\'", mtmp->GetCleanName());
				  else if (Bsay <50)
				    botOwner->Message(5, "%s tells you, \'You can spawn additional bots at levels 5,7,8 and 10, but you got to do the quests first.\'", mtmp->GetCleanName());
				  else if (Bsay <55)
				    botOwner->Message(5, "%s tells you, \'You start questing additional raid bots at level 45.\'", mtmp->GetCleanName());
				  else if (Bsay <60)
				    botOwner->Message(5, "%s tells you, \'Anita Thrall has quests for bot spells.\'", mtmp->GetCleanName());
				  else if (Bsay <65)
				    botOwner->Message(5, "%s tells you, \'The further away we get from starter zone, the better experience is.\'", mtmp->GetCleanName());
				  else if (Bsay <70)
				    botOwner->Message(5, "%s tells you, \'If you don't like me, you can remove me and make and spawn your own bot with '#bot create','#bot spawn',and '#bot add' .\'", mtmp->GetCleanName());
				  else if (Bsay <75)
				    botOwner->Message(5, "%s tells you, \'I'm here to help get you started, AXCLassic also has a Wiki and forums with more help. Look for AXClassic.com\'", mtmp->GetCleanName());
				  else if (Bsay <80)
				    botOwner->Message(5, "%s tells you, \'You can download full, custom clients at AXCLassic forums - look for the torrent files.\'", mtmp->GetCleanName());
				  else if (Bsay <85)
				    botOwner->Message(5, "%s tells you, \'many quests are do-able for your bots even though you're not of the required class.\'", mtmp->GetCleanName());
				  else if (Bsay <90)
				    botOwner->Message(5, "%s tells you, \'Champion Darkwater is the ladder master, he has some cool rewards as you level.\'", mtmp->GetCleanName());
				  else if (Bsay <95)
				    botOwner->Message(5, "%s tells you, \''#bot help shows general commands and a targeted bot with '#bot help' will show class-specific commands.\'", mtmp->GetCleanName());
				  else if (Bsay <100)
				    botOwner->Message(5, "%s tells you, \'Anita Thrall's gate quest will also reward you with a bind and gate Moonstone .\'", mtmp->GetCleanName());


				}

				
				  if (botOwner->GetClientVersion() >= EQClientSoD){
...
void Mob::SpawnBotBank(int32 groupId, Client* botOwner) {
	...
	for(int i=0; i<bots; i++) {
		...
		if((tmp = database.GetNPCType(atoi(row[1])))) {
			...
			if(!errorMessage.empty()) {
				npc->Say("There was an error loading my equipment");
			}
		...
		if((g->BotGroupCount() < 6) && (atoi(row[0]) == groupId)) {
			...
			if(mtmp) {
				...
				mtmp->SetHP(HP);
				mtmp->SetMana(MANA);
				mtmp->ArmorColorUpadate(npc, &errorMessage);
...
void Mob::SaveBotGroup(int32 GroupID, Client* botOwner) {
	...
	int32 LiveGroupID = database.GetGroupID(botOwner->GetName());
	...
	for(int j=0; j<MAX_GROUP_MEMBERS; j++) {
		if(f->members[j]) {
			...
			if (botOwner->GroupHasClient()){
			    database.SaveToBotTemp(LiveGroupID, botOwner->CharacterID(), f->members[j]->GetNPCTypeID(), j, f->members[j]->GetHP(), f->members[j]->GetMana());
			    //LogFile->write(EQEMuLog::Debug, "Mob::SaveBotGroup Found additional clients saved to BotTemp only, GroupID is %i.",GroupID);
			}
			else{
			   ...
			   // LogFile->write(EQEMuLog::Debug, "Mob::SaveBotGroup Found no additional clients saved BotTemp and BotBank, GroupID is %i.",GroupID);
...
void Mob::UpdateBotStats(int32 hp, int32 mana, int32 groupid, int32 botid) {
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	int32 affected_rows = 0;

	if(!database.RunQuery(query, MakeAnyLenString(&query, "UPDATE botbank SET hp=%i, mana=%i WHERE groupid=%i AND botid=%i",hp, mana, groupid, botid), errbuf, 0, &affected_rows)) {
		cerr << "Error in UpdateBotHP query '" << query << "' " << errbuf << endl;
	}
	safe_delete_array(query);
}

int Mob::GetBotHP(int32 groupid,int32 botid) {
	char errbuf[MYSQL_ERRMSG_SIZE];
	char* query = 0;
	MYSQL_RES* result;
	MYSQL_ROW row;
	int32 BotHP = 0;
      ...
      if (groupid > 19){ 
	if(database.RunQuery(query, MakeAnyLenString(&query, "SELECT hp FROM bottemp WHERE groupid=%i AND botid=%i",groupid, botid), errbuf, &result)) {
	...
	if(BotHP == 0){
	  if(database.RunQuery(query, MakeAnyLenString(&query, "SELECT hp FROM botbank WHERE groupid=%i AND botid=%i",groupid, botid), errbuf, &result)) {
		if(mysql_num_rows(result) == 1) {
			row = mysql_fetch_row(result);
			BotHP = atoi(row[0]);
		}
	...
	if(database.RunQuery(query, MakeAnyLenString(&query, "SELECT hp FROM botgroups WHERE groupid=%i AND botid=%i",groupid, botid), errbuf, &result)) {
		if(mysql_num_rows(result) == 1) {
			row = mysql_fetch_row(result);
			BotHP = atoi(row[0]);
		}
...
int Mob::GetBotMana(int32 groupid,int32 botid) {
	char errbuf[MYSQL_ERRMSG_SIZE];
	char* query = 0;
	MYSQL_RES* result;
	MYSQL_ROW row;
	int32 BotMANA = 0;
      ...
      if (groupid > 19){
	if(database.RunQuery(query, MakeAnyLenString(&query, "SELECT mana FROM bottemp WHERE groupid=%i AND botid=%i",groupid, botid), errbuf, &result)) {
		if(mysql_num_rows(result) == 1) {
			row = mysql_fetch_row(result);
			BotMANA = atoi(row[0]);
		}
	...
	if(BotMANA == 0){
	  if(database.RunQuery(query, MakeAnyLenString(&query, "SELECT mana FROM botbank WHERE groupid=%i AND botid=%i",groupid, botid), errbuf, &result)) {
		if(mysql_num_rows(result) == 1) {
			row = mysql_fetch_row(result);
			BotMANA = atoi(row[0]);
		}
	...
	if(database.RunQuery(query, MakeAnyLenString(&query, "SELECT mana FROM botgroups WHERE groupid=%i AND botid=%i",groupid, botid), errbuf, &result)) {
		if(mysql_num_rows(result) == 1) {
			row = mysql_fetch_row(result);
			BotMANA = atoi(row[0]);
		}
...
uint32 Mob::GetBotEquipmentColor(int8 material_slot) const
{
	//Bot tints
	int32 botslotid = 0;
	uint32 returncolor = 0;
	uint32 npctypeid = this->GetNPCTypeID();
	
	//Translate code slot # to DB slot #
	botslotid = Inventory::CalcSlotFromMaterial(material_slot);
	...
	if(database.RunQuery(Query, MakeAnyLenString(&Query, "SELECT color FROM botinventory WHERE npctypeid = %u AND botslotid = %u", npctypeid, botslotid), 0, &DatasetResult)) {
...
void Mob::SendBotWearChange(int8 material_slot)
{
	int32 BotColorID = GetBotEquipmentColor(material_slot);
	
	//Angelox: Somehow, the old equipment color id (10 digit) gets caughtup in the the botinventory table,
	//this fix is tell I can figure out something else
	if ((this->IsBot()) && (((BotColorID) > 0) && ((BotColorID) < 90000000))){
	 
	  EQApplicationPacket* outapp = new EQApplicationPacket(OP_WearChange, sizeof(WearChange_Struct));
	  WearChange_Struct* wc = (WearChange_Struct*)outapp->pBuffer;
	  
	  wc->spawn_id = GetID();
	  wc->color.color = GetBotEquipmentColor(material_slot);
	  wc->material = GetEquipmentMaterial(material_slot);
	  wc->wear_slot_id = material_slot;
	  entity_list.QueueClients(this, outapp);
...
void Mob::BotFaceTarget(Mob* MobToFace) {
	Mob* facemob = MobToFace;
	if(!facemob) {
		if(!GetTarget()) {
			return;
