				...
				if(BotRaidGroups[i]) {
					if(BotRaidGroups[i]->members[j]) {
						if(BotRaidGroups[i]->members[j]->IsBot()) {
							...
							BotRaidGroups[i]->members[j]->BotKill();
						}
				...
				if(BotRaidGroups[i]->members[j]) {
					if(BotRaidGroups[i]->members[j]->IsBot()) {
						if(BotRaidGroups[i]->members[j]->GetNPCTypeID() != *iter) {
							...
							entity_list.RezBotMemeber(iter, i, j, this, 0, c);
							i = MAX_BOT_RAID_GROUPS;
							break;
						}
						++iter;
...
//Angelox:Summon the bots to random positions around your character
void BotRaids::RandSummonRaidBots(Mob *m, bool wipe_hate) {
	int randmoveA = MakeRandomInt(-10, 14);
	int randmoveB = MakeRandomInt(-15, 19);
	int randmoveC = MakeRandomInt(-5, 9);
	int randmoveD = MakeRandomInt(-20, 25);
	for(int i=0; i<MAX_BOT_RAID_GROUPS; i++) {
		if(BotRaidGroups[i]) {
			for(int j=0; j<MAX_GROUP_MEMBERS; j++) {
				if(BotRaidGroups[i]->members[j]) {
...
void BotRaids::SaveGroups(Client *c, int tableA, int tableB, int tableC) {
		...
		if(BotRaidGroups[i]) {
			for(int j=0; j<MAX_GROUP_MEMBERS; j++) {
				if(BotRaidGroups[i]->members[j]) {
					...
					database.SaveBotGroups(i, c->CharacterID(), BotRaidGroups[i]->members[j]->GetNPCTypeID(), j, 1, 0, 0);
				}
			}
		}
	}
	...
	for(int x=0; x<MAX_BOT_RAID_GROUPS; x++) {
		if(BotRaidGroups[x]) {
			for(int y=0; y<MAX_GROUP_MEMBERS; y++) {
				if(BotRaidGroups[x]->members[y]) {
					if(BotRaidGroups[x]->members[y]->IsClient()) {
						continue;
					}
					database.SaveBotGroups(x, c->CharacterID(), BotRaidGroups[x]->members[y]->GetNPCTypeID(), y, 0, 1, 0);
				}
			}
		}
	}
	...
	for(int g=0; g<MAX_BOT_RAID_GROUPS; g++) {
		if(BotRaidGroups[g]) {
			for(int h=0; h<MAX_GROUP_MEMBERS; h++) {
				if(BotRaidGroups[g]->members[h]) {
					if(BotRaidGroups[g]->members[h]->IsClient()) {
						continue;
					}
					database.SaveBotGroups(g, c->CharacterID(), BotRaidGroups[g]->members[h]->GetNPCTypeID(), h, 0, 0, 1);
				}
			}
		}
	}
