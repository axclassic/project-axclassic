...
void Client::Handle_OP_ZoneChange(const EQApplicationPacket *app) {
		...
		if(g) {
			...
			for(int i=5; i>=0; i--) {
				if(g->members[i] && g->members[i]->IsBot()) {
					g->members[i]->UpdateBotStats(g->members[i]->GetHP(), g->members[i]->GetMana(),g->members[i]-> GetNPCTypeID());
					g->members[i]->UpdateBotBuffs(g->members[i]-> GetNPCTypeID());
					hasBots = true;
	...
	//Angelox: This disbands amy grouped clients - needed tell grouping can be fixed.
	if (GetGroup())
	  GetGroup()->DisbandGroup();
