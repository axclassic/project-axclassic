...
void Client::Handle_OP_ZoneChange(const EQApplicationPacket *app) {
		...
		Group *BotGroup = entity_list.GetGroupByMob(clientmob);
		//Group *ClientGroup = GetGroup();
		if(BotGroup) {
		    if(IsBotRaiding()){
			BotGroup->DisbandGroup(false);
			LogFile->write(EQEMuLog::Debug, "Client::ZoneChange Found raid group - disbanding raid...");
		    }
		    else{
			//bool hasBots = false;
			for(int i=5; i>=0; i--) {
				if(BotGroup->members[i] && BotGroup->members[i]->IsBot()) {
					//BotGroup->members[i]->UpdateBotStats(BotGroup->members[i]->GetHP(), 
					//BotGroup->members[i]->GetMana(),BotGroup->members[i]-> GetNPCTypeID());
					BotGroup->members[i]->UpdateBotBuffs(BotGroup->members[i]-> GetNPCTypeID());
					BotGroup->members[i]->SetBotIsZoning(true);
					BotGroup->DelMember(BotGroup->members[i]);
					//LogFile->write(EQEMuLog::Debug, "Client::ZoneChange Found Bots in group - removing Bots for zoning...");
					//hasBots = true;
				}
			}
			/*if(hasBots) {
					BotGroup->DisbandGroup(true);
					hasBots = false;
			}*/
		    //Angelox: This disbands everything (clients too) - needed tell grouping can be fixed.
		    BotGroup->DisbandGroup(true);
		    //hasBots = false;
		    }
		}
	...
	int8 canzone = 0; //Angelox4
	if(!database.GetSafePoints(target_zone_name, &safe_x, &safe_y, &safe_z, &minstatus, &minlevel, flag_needed, &canzone)) {
		//invalid zone...
	...
	if (GetLevel() > canzone){  //Angelox4
		//Message(13, "You are too high in level to enter this zone.");
		myerror = ZONE_ERROR_DENIEDENTRY;
	}
...
void Client::DoZoneSuccess(ZoneChange_Struct *zc, uint16 zone_id, int32 instance_id, float dest_x, float dest_y, float dest_z, float dest_h, sint8 ignore_r) {
	...
	//when they zone into it
	 ...
	 if((GetClientVersion() > EQClientTitanium) && (zone_id == 5)){
	    zone_id = 407;
...
void Client::ZonePC(int32 zoneID, int32 instance_id, float x, float y, float z, float heading, int8 ignorerestrictions, ZoneMode zm) {
	...
	if(ReadyToZone) {
		...
		if(zm == ZoneToBindPoint) {
		  ...
		  if((GetClientVersion() > EQClientTitanium) && (gmg->zone_id == 5))
		    zoneID = 407;
		  else
		    gmg->zone_id = zoneID;
...
void Client::SendZoneFlagInfo(Client *to) const {
	...
	for(; cur != end; cur++) {
		...
		char flag_name[128];
		int8 canzone = 0;  //Angelox4
		if(!database.GetSafePoints(short_name, &safe_x, &safe_y, &safe_z, &minstatus, &minlevel, flag_name, &canzone)) {
...
bool Client::CanBeInZone() {
	...
	char flag_needed[128];
	int8 canzone = 0; //Angelox4
	if(!database.GetSafePoints(zone->GetShortName(), &safe_x, &safe_y, &safe_z, &minstatus, &minlevel, flag_needed, &canzone)) { //Angelox4
	...
	if(GetLevel() > canzone) { //Angelox4
		_log(CLIENT__ERROR, "Character does not meet max level requirement (%d < %d)!", GetLevel(), canzone);
		return(false);
	}
	if(Admin() < minstatus) {
