...
void EntityList::CheckClientAggro(Client *around) {
	...
	for(iterator.Reset(); iterator.MoreElements(); iterator.Advance()) {
		...
		mob = iterator.GetData();
		if(!mob || mob->IsClient())	//also ensures that mob != around
			continue;

#ifdef EQBOTS
		//Angelox IsClassic Insert
		if(zone->IsClassic()&& (database.GetKarma(around->AccountID()) < 10000))
			if(around->GetLevel() >= 50)
				if(mob->GetLevel() < 50)
					continue;
#endif
...
// fez_ajer: Get a list of mobs that are actively attacking the specified mob
list<Mob*> EntityList::GetMobAggroList(Mob *target) {
		list<Mob*> result;

		// Make sure there is a valid mob to check
		if (target) {
			LinkedListIterator<NPC*> iterator(npc_list);
			for(iterator.Reset(); iterator.MoreElements(); iterator.Advance()) {
				Mob* mob = iterator.GetData()->CastToMob();

				// Flat out ignore invalid, non-engaged, mezzed and feared mobs
				if (!mob || !mob->IsEngaged() || mob->IsMezzed() || mob->IsFeared())
						continue;

				// Check aggro
				if (mob->CheckAggro(target))
...
// fez_ajer: Get a list of mobs that are actively attacking the specified group
list<Mob*> EntityList::GetGroupAggroList(Group *group) {
		list<Mob*> result;

		// Make sure there is a valid group to check
		if (group) {
			LinkedListIterator<NPC*> iterator(npc_list);
			for(iterator.Reset(); iterator.MoreElements(); iterator.Advance()) {
				Mob* mob = iterator.GetData()->CastToMob();
				...
				// Check group members aggro otherwise
				bool hasHate = false;
				for (int i=0; i < MAX_GROUP_MEMBERS; i++) {
					if (!group->members[i] || !mob->CheckAggro(group->members[i]))
						continue;
...
#ifdef EQBOTS
	//Angelox IsClassic Insert
	if((IsClient()) && (zone->IsClassic()) && (GetLevel() >= 50) && (database.GetKarma(CastToClient()->AccountID()) < 10000)) { //Angelox2
		if(target->GetLevel() <= 49) {
			CastToClient()->Message(15, "Targets level is trivial for you, it must be at least level 50.");
			CastToClient()->Message(15, "This zone is trivial for you, try a zone with higher level mobs.");
			return false;
		}
	}
	if(IsClient() && (target && target->IsBot())) {
		return false;
	}
#endif
