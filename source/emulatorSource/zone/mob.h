...
//	static	int		CheckEffectIDMatch(int8 effectindex, int16 spellid1, int8 caster_level1, int16 spellid2, int8 caster_level2);
	...
	virtual void SendBotWearChange(int8 material_slot);
	void BotWarp( float x, float y, float z );
	...
	int8 BotRaidGroup;
	void SetBotRaidGroup(int8 group) { BotRaidGroup = group; }
	int8 GetBotRaidGroup() { return BotRaidGroup; }
	...
	void SetBotGuarding(bool GuaRder) { BotGuarding = GuaRder; }
	void SetBotRaiding(bool v) { BotRaiding = v; }
	void SetHasClient(bool GotClient) { HasClient = GotClient; }
	void SetBotIsZoning(bool HasZoning) { IsZoning = HasZoning; }
	...
	void SetManaBarStatus(bool BarStatus) { botBarStatus = BarStatus; }
	...
	bool IsBotGuarding() const { return BotGuarding; }
	bool IsBotRaiding() const { return BotRaiding; }
	bool GroupHasClient() const { return HasClient; }
	bool BotIsZoning() const { return IsZoning; }
	//bool GetNpcMode() const { return botNpcMode; }
	bool HasManaBar() const { return botBarStatus; }
	...
	void UpdateBotStats(int32 hp, int32 mana, int32 groupid, int32 botid);
	void UpdateBotBuffs(int32 botid);
	void DeleteBotBuffs(int32 botid);
	int GetBotHP(int32 groupid, int32 botid);
	int GetBotMana(int32 groupid, int32 botid);
...
//	inline void SetEGNode(EGNode *s) { _egnode = s; }
  ...
  // HP Event
   ...
   void SetNextHPEvent( int hpevent );
	...
	Timer ReSendPosition_timer; //So as not to spam SendPostion packets to the client.  
	Timer mana_barCheck_timer;//Check for mana bar update in client window (for newer clients).
	Timer check_attack_timer; //checks if attacker bot arrived to its target.
	...
	Timer ReSendManaCheck_timer; //#bot mana can't be spammed
	...
	void SetBotArchery();
	void SetSpellState(uint32 CharID);
	void BotFaceTarget(Mob* MobToFace = 0);
...
protected:
	...
	virtual bool Bot_Command_CharisTarget(int charistype, Mob *target) { return(false); } //Angelox2
