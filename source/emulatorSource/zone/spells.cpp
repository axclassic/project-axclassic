...
// #ifdef EQBOTS //Bots spam too musch when raiding
//       if (!IsBotRaiding()){
// #endif
...
bool Mob::SpellOnTarget(int16 spell_id, Mob* spelltar)
{
	...
	if ((IsClient()) && (zone->IsClassic()) && (this->GetLevel() >= 50) && (database.GetKarma(CastToClient()->AccountID()) < 10000)){
	  if(spelltar->GetLevel() <= 49){
	    Message(13, "SOT: Trivial target in a classic zone.");
		return false;
	  }
	}
	if ((zone->IsClassic()) && (IsBot()) && (this->GetLevel() >= 50) && (database.GetKarma(BotOwner->CastToClient()->AccountID()) < 10000)){
	  if(spelltar->GetLevel() <= 49){
...
/*
	if((IsBot()) &&(IsDetrimentalSpell(spell_id))){ 
	    switch(spell_id){
	    case 87:    //LoH
		  return false;
	    case 88:    //HT
...
/*
	...
	int8 HTSpell = NULL;
		...
		if(spells[spell_id].id == HTSpell){
		mlog(SPELLS__CASTING_ERR, "Spell casting canceled(spells.cpp line 3706): HT, LoH types disabled for Bots.");
		return true;
		}
