...
void command_botmode(Client *c, const Seperator *sep) {
   ...
   else if (c->GetClientVersion() >= EQClientSoD){
     c->Message(15, "This command is for the Titanium client.");
    return;
   }
...
void command_bot(Client *c, const Seperator *sep) {
	...
	if(!strcasecmp( sep->arg[1], "help") && !strcasecmp( sep->arg[2], "\0")){
		...
		c->Message(15, "#bot add - make the targeted bot join your group.");
		...
		c->Message(15, "#bot summon [bot],[corpse],[food],or [drink] - Will summon a bot or corpse (needs Necro), or food/drink.");
		...
		c->Message(15, "#bot set [primary], [snare] [on] or [off], [root] [on] or [off], [chat] [on] or [off] - enable primary slot for range item, enable/disable root, snare, chat, [armorcolor] - restores your bots tinted armor color.");
		...
		c->Message(15, "#bot armorcolor - tint your bots armor with a different color..");
...
//BotHelp
	...
	if((!strcasecmp(sep->arg[1], "shaman")) || (!strcasecmp( sep->arg[1], "help") && !strcasecmp( sep->arg[2], "shaman"))) { 
	    ...
	    c->Message(15, "#bot levitate - Provides levitation for your target.");
	    c->Message(15, "#bot summon - [food/drink] for your target.");
	    c->Message(15, "#bot charisma - Cast charisma buff on target");
	    c->Message(15, "-----------------------------------------------------------------------");
	...
	if((!strcasecmp(sep->arg[1], "necromancer")) || (!strcasecmp( sep->arg[1], "help") && !strcasecmp( sep->arg[2], "necromancer"))) { 
	    ...
	    c->Message(15, "#bot invis - Will make you invisible");
	    c->Message(15, "#bot charm - Turns your target into the bots pet.");
	    c->Message(15, "#bot auto mez on/off - Will automatically stop incoming undead adds to the melee.");
	...
	if((!strcasecmp(sep->arg[1], "paladin")) || (!strcasecmp( sep->arg[1], "help") && !strcasecmp( sep->arg[2], "paladin"))) { 
	    c->Message(15, "--------- Additional Paladin Commands-------------------- ");
	    c->Message(15, "#bot LayOnHands - Completely heals  your target.");
	    c->Message(15, "----------------------------------------------------------------------");
	    entity_list.BotHelpWindow(c,14);
	    return;
	}
	if((!strcasecmp(sep->arg[1], "shadowknight")) || (!strcasecmp( sep->arg[1], "help") && !strcasecmp( sep->arg[2], "shadowknight"))) { 
	    c->Message(15, "--------- Additional ShadowKnight Commands-------------------- ");
	    c->Message(15, "#bot HarmTouch - Does serious damage to your target.");
	    c->Message(15, "----------------------------------------------------------------------");
	    entity_list.BotHelpWindow(c,13);
	    return;
	}
	if((!strcasecmp(sep->arg[1], "enchanter")) || (!strcasecmp( sep->arg[1], "help") && !strcasecmp( sep->arg[2], "enchanter"))) { 
	    ...
	    c->Message(15, "#bot slow - Slows your targets attack rate.");
	    c->Message(15, "#bot pacify - Calms your target.");
	    c->Message(15, "#bot rune - Buffs your target with the Rune shield.");
	    c->Message(15, "#bot charm - Turns your target into the bots pet.");
	...
	if((!strcasecmp(sep->arg[1], "magician")) || (!strcasecmp( sep->arg[1], "help") && !strcasecmp( sep->arg[2], "magician"))) { 
	    ...
	    c->Message(15, "#bot lore - cast Identify on the item on your mouse pointer.");
	    c->Message(15, "#bot summon - [food/drink] for your target.");
	    c->Message(15, "#bot magepic - Summons Magician epic orb.");
	    c->Message(15, "-------------------------------------------------------------------------");
...
//		bool Special = (RuleB(EQOffline, AXCustom));
		...
		if(database.CountBots(charID) >= RuleI(EQOffline, CreateBotCount)) {
		...
		if(!botName || (strlen(botName) < 4) || (strlen(botName) > 40)) {
			 if (strlen(botName) < 4) 
				c->Message(15, "%s is not enough characters.", botName);
			 else
				...
				if(atoi(row[0]) != 0) {
					...
					if(database.RunQuery(query, MakeAnyLenString(&query, "SELECT MAX(id) from npc_types where name='%s' and isBot=1", sep->arg[2]), errbuf, &result)) {
						if((row = mysql_fetch_row(result))) {
							database.SetBotOwner(atoi(row[0]), charID); 
	...
	if((!strcasecmp(sep->arg[1], "delete")) && (!strcasecmp(sep->arg[2], "now"))) {
		...
		else if(database.GetBotOwner(c->GetTarget()->GetNPCTypeID()) != charID) {
...
//List
	if(!strcasecmp(sep->arg[1], "list")) {
	 ...
	 if(database.RunQuery(query, MakeAnyLenString(&query, "SELECT id, name, class, race FROM botsowners AS p JOIN npc_types AS n ON p.botnpctypeid = n.id WHERE p.botleadercharacterid = %i",charID), errbuf, &result, &affected_rows))
	    {
            ...
            while((row = mysql_fetch_row(result)))
            {
				...
				if(listAll && database.GetBotOwner(atoi(row[0])) == charID) { 
					c->Message(15,"ID: %s -- Class: %s -- Name: %s -- Race: %s -- ", row[0], crow, row[1], rrrow);
				}
				else if((database.GetBotOwner(atoi(row[0])) == charID) && (irow == iClass)) { 
					c->Message(15,"ID: %s -- Class: %s -- Name: %s -- Race: %s -- ", row[0], crow, row[1], rrrow);
				}
			}				
		}
...
//SpawnOneBot by name or by id
	if ((!strcasecmp(sep->arg[1], "spawn") ) || (!strcasecmp(sep->arg[1], "call"))) {
		int botIdByName = (database.GetBotIDByBotName(std::string(sep->arg[2])));
		int botOwnerId1 = (database.GetBotOwner(botIdByName));
		//int botOwnerId2 = (database.GetBotOwner(atoi(sep->arg[2])));
		int botOwnerId3 = 0;
		int botId = 0;
		if ((botOwnerId1) != (charID))
		...
		if ((tmp = database.GetNPCType(botId))){
			...
			const unsigned int spawnedBots = database.SpawnedBotCount(charID);
			//int32 groupID = database.GetBotGroupID(charID);
			//bool HasGroup = database.BotHasGroup(botId, charID);
			bool IsSpawned = database.IsBotSpawned(charID, botId);
/*			if((HasGroup) && (groupID < 20 )){
			...
			if(spawnedBots && IsSpawned) {
				...
				MYSQL_RES* total = database.ListSpawnedBots(charID);
				MYSQL_ROW row;
				if(mysql_num_rows(total) == spawnedBots) {
					for(unsigned int i=0; i<spawnedBots; i++) {
			...
			if(RuleB(EQOffline, BotQuest)) {
				const unsigned int allowedBots = database.AllowedBotSpawns(charID);
				if(allowedBots == 0) {
			...
			unsigned int SpawnedBotRule = RuleI(EQOffline, SpawnBotCount);
			if(spawnedBots >= SpawnedBotRule) {
			...
			int combatID = database.BotCanCombatLookUp(zoneID);
			int32 remain = c->GetPTimers().GetRemainingTime(pTimerSpawnBotReuse);
			if(!c->GetPTimers().Expired(&database, pTimerSpawnBotReuse, false) && (!c->GetGM())) {
				c->Message(13,"You must wait %d minutes %d seconds before using the spawnbot command again.", ((remain)/60), (remain%60));
				return;
			}
			if(combatID == 0)
			  c->GetPTimers().Start(pTimerSpawnBotReuse, 5);
			else
			  c->GetPTimers().Start(pTimerSpawnBotReuse, 40);
			...
			database.SetBotLeader(npc->GetNPCTypeID(), charID, npc->GetCleanName(), zone->GetShortName(), zone->GetZoneID());
...
//bot armor colors (armorcolor)
	if(!strcasecmp(sep->arg[1], "armorcolor")) {
	    
		if (target == NULL) {
			c->Message(15, "Target your Bot");
			return;
		}
	    
		if((!target->IsBot()) || (!c->IsGrouped())) {
		    c->Message(15, "Target your grouped Bot first.");
		    return;
		}

		const int hasColor = (database.BotColorLookup(charID));
		if(hasColor <= 0){
		    c->Message(15, "You haven't completed the armorcolor quest yet.");
		    return;
		   }
	    ...
	    char *query = 0;
	    MYSQL_RES *result = 0;
	    unsigned int red = atoi(sep->arg[3]);
	    unsigned int green = atoi(sep->arg[4]);
	    unsigned int blue = atoi(sep->arg[5]);
	    uint32 setcolor = (red << 16) | (green << 8) | blue;
	    
			int setslot;	  
			if ((!strcasecmp(sep->arg[2], "head")) || (!strcasecmp(sep->arg[2], "helm"))){ 
				setslot = 2;}
			else if (!strcasecmp(sep->arg[2], "arms")){ 
				setslot = 7;}
			else if (!strcasecmp(sep->arg[2], "bracer") || (!strcasecmp(sep->arg[2], "wrist"))){ 
				setslot = 9;}
			else if (!strcasecmp(sep->arg[2], "hands")){ 
				setslot = 12;}
			else if ((!strcasecmp(sep->arg[2], "chest")) || (!strcasecmp(sep->arg[2], "robe"))){ 
				setslot = 17;}
			else if (!strcasecmp(sep->arg[2], "legs")){ 
				setslot = 18;}
			else if ((!strcasecmp(sep->arg[2], "feet")) || (!strcasecmp(sep->arg[2], "boots"))){ 
				setslot = 19;}
			else{
				setslot = 0;}
				//setslot = atoi(sep->arg[2]);}
				
		if(target && target->IsBot() && (database.GetBotOwner(target->GetNPCTypeID()) == charID)) {
		  
			if (setslot == 0){
				c->Message(15, "Usage: #bot armorcolor [slot] [red] [green] [blue] - use #bot help armorcolor for info");
				return;
			}
		...
		c->GetTarget()->CastToMob()->SendBotWearChange(slotmaterial);
		}
		else {
	...
	// Help for coloring bot armor
		...
		c->Message(15, "-----------------#bot armorcolor help-----------------------------");
		c->Message(15, "Armor:  [chest/robe], [arms], [bracer/wrists], [hands], [legs], [boots/feet], [helm/head].");
		c->Message(15, "------------------------------------------------------------------");
		c->Message(15, "Color: [red] [green] [blue]  (enter a number from 0-255 for each color.");
		c->Message(15, "------------------------------------------------------------------");
		c->Message(15, "Example: #bot armorcolor chest 0 255 0 - this would make the chest bright green");
		return;
	}
	...
	if((!strcasecmp(sep->arg[1], "showgroups"))||(!strcasecmp(sep->arg[1], "showgroup"))){
	    ...
	    if(!strcasecmp(sep->arg[2], "raid")){
	      c->Message(15, "Your list of stored raid groups are,");
		    if(database.RunQuery(query, MakeAnyLenString(&query, "SELECT botgroups.groupid, npc_types.name, npc_types.class, npc_types.race, npc_types.id FROM botgroups,npc_types WHERE botgroups.botid = npc_types.id AND npc_types.id > 5000000 ORDER by botgroups.groupid"), errbuf, &result, &affected_rows))
		    {
			while((row = mysql_fetch_row(result)))
			{
				int irow = atoi(row[2]);
			    ...
			    if(database.GetBotOwner(atoi(row[4])) == charID) { 
	      ...
	      c->Message(15, "Your list of stored single groups are,");
		    if(database.RunQuery(query, MakeAnyLenString(&query, "SELECT botbank.groupid, npc_types.name, npc_types.class, npc_types.race, npc_types.id FROM botbank,npc_types WHERE botbank.botid = npc_types.id AND npc_types.id > 5000000 ORDER by botbank.groupid"), errbuf, &result, &affected_rows))
		    {
			while((row = mysql_fetch_row(result)))
			{
				int irow = atoi(row[2]);
...
//Savegroup - Angelox: For saving individual groups under different groupIDs. Can be used for grouping or raid-groups. 
	if(!strcasecmp(sep->arg[1], "savegroup")){
	    int32 OldGroupID = database.GetBotGroupID(charID);
	    int32 NewGroupId = atoi(sep->arg[2]);
	    int32 GroupExists = database.BotGroupExists(NewGroupId, charID);
	    int32 remain = c->GetPTimers().GetRemainingTime(pTimerCoolDownTrigger);	    
		if(!c->IsGrouped()){
		    c->Message(15, "You need to group with a bot for this");
		    return;
		}
		else if (c->GroupHasClient()){
		   c->Message(13, "Can't save bot group while grouped with other players.");
		   return;
		}
		else if(c->IsBotRaiding()){
		    c->Message(13, "This works only while in single group mode.");
		    return;
		}
		else if(!c->GetPTimers().Expired(&database, pTimerCoolDownTrigger, false)) {
			c->Message(13,"Wait %d seconds cool-down before using the savegroup command again.", (remain%10));
			return;
		}
		  ...
		  if((GroupExists) && (NewGroupId < 20)){
		    ...
		    //database.DeleteFromBotBank(OldGroupID, charID);
		    database.UpdateBotGroupID(20, charID);
		    c->Message(15, "Group info saved to GroupID 20.");
		    c->Message(15, "For forming bot raid groups or saving multiple groups, specify a groupID. GroupIDs under 20 are reserved for raid groups and have restrictions.");
		    c->SaveBotGroup(20, c);
		    c->GetPTimers().Start(pTimerCoolDownTrigger, 10);
		    return;
		  }
		  else{
		    ...
		    database.UpdateBotGroupID(NewGroupId, charID); 
		      if(NewGroupId < 20){
...
//Bot  
	  if(!strcasecmp(sep->arg[2], "bot")) {
		...
		if(c->GetTarget()->IsMob() && !c->GetTarget()->IsPet()){
			...
			if(b && (database.GetBotOwner(b->GetNPCTypeID()) != charID)) 
			{
...
//Corpse
	...
	if(!strcasecmp(sep->arg[2], "corpse")) {
		...
		if(c->IsGrouped()) {
			...
			{
				switch(database.BotSummonLookup(charID))
				{
				case 1:
					summonClass2 = false;
					summonClass3 = false;
			...
			for(unsigned int i=0; i<MAX_GROUP_MEMBERS; ++i) {
				if((g && g->members[i]) && g->members[i]->IsBot() && (g->members[i]->GetClass() == NECROMANCER)) {
					...
					if(!t->IsClient()) {
						...
						if (CurMana < 100)
						  summoner->BotSay("Not enough mana!");
						else
...
//Group
	if(!strcasecmp(sep->arg[2], "group")) {
		c->Message(15, "Summon group temporarily disabled for zone crash issues.");
		c->Message(15, "#bot follow or #bot attack will also summon your bots to the action.");
		return;
	}	
/*
				...
				for(unsigned int i=0; i<MAX_GROUP_MEMBERS; i++)
				{
					if(g->members[i] && g->members[i]->IsBot()) {
					  botMember = g->members[i];
						botMember->SetTarget(botMember->BotOwner);
						botMember->InterruptSpell();
						botMember->BotWarp(c->GetX(), c->GetY(), c->GetZ());
						  if (c->IsBotGuarding()){
							botMember->SetTarget(c);
							botfollowid = botMember->GetID();
							botfollowname = botMember->GetName();
							botMember->SetFollowID(c->GetID());
						  }
						    
						if(botMember && botMember->GetPetID()) {
							botMember->GetPet()->SetTarget(botMember);
							botMember->GetPet()->BotWarp(c->GetX(), c->GetY(), c->GetZ());
						}
						if(botMember && botMember->IsBot()) {
							  if (c->IsBotGuarding()){
							    botMember->SetFollowID(0);
							  }
						}
					}
				}
		...
		if (c->IsGrouped()) {
			...
			if (g) {
				  for (unsigned int i=0; i<MAX_GROUP_MEMBERS; i++) {
					...
					c->Message(15, "Only the group leader can use this command.");
					return;
				      }
				  }
				  
				if (c->IsBotRaiding())
				  c->Message(15, "Disbanding raid...");
				else
				  c->Message(15, "Disbanding group...");
				
				g->DisbandGroup(false);
				database.CleanBotLeader(charID);
				database.DeleteFromBotTemp(charID);
			}
		}
		else {
			c->Message(15, "No group to disband...");
			c->Message(15, "Checking database caches...");
			  const int spawnedBots = database.SpawnedBotCount(charID);
			    if (spawnedBots >=1){
			      database.CleanBotLeader(charID);
			      database.DeleteFromBotTemp(charID);
			      c->Message(15, "Bots found and removed.");
			      c->Message(15, "If you had a spawned bot, let it camp out before reforming.");
			      c->Message(15, "If problems persist, please report.");
			    }
			    else{
			      c->Message(15, "Database caches are clean.");
			    }
		}
		return;
	}
...
//SpawnGroup
	if((!strcasecmp(sep->arg[1], "spawngroup")) || (!strcasecmp(sep->arg[1], "groupspawn"))) {
		const int spawnedBots = database.SpawnedBotCount(charID);
		if(c->IsBotRaiding()) {
			c->Message(15, "Not while raiding.");
			return;
		}

		if(spawnedBots >= 1) {
			c->Message(15, "You already have spawned bots.");
			c->Message(15, "If you do not have any spawned bots and still get this message, try #bot disbandgroup first.");
			return;
		}
		...
		if(c->IsGrouped()) {
			c->Message(15, "You can't spawn a group when you're already grouped.");
			c->Message(15, "If you do not have a group and still get this message, try #bot disbandgroup first.");
		...
		int32 GroupID = atoi(sep->arg[2]);
		 if(sep->arg[2][0] == '\0')
		   GroupID = 20;
		int32 GroupExists = database.BotGroupExists(GroupID, charID);
		if(!GroupExists){
		...
		if((!c->GetPTimers().Expired(&database, pTimerSpawnGroupReuse, false)) && (!c->GetGM())) {
...
//SpawnRaid
	if(!strcasecmp(sep->arg[1], "spawnraid")) {
		...
		if((!c->GetPTimers().Expired(&database, pTimerSpawnRaidReuse, false)) && (!c->GetGM())) {
		...
		if(combatID == 0)
		  c->GetPTimers().Start(pTimerSpawnRaidReuse, 20);
		else
...
//		} //Raid Timer end
		...
		MYSQL_RES* groups = database.LoadBotGroups(charID,1,0,0);
		MYSQL_ROW row;
		...
		for(int i=0; i<bots; i++) {
			...
			if((tmp = database.GetNPCType(atoi(row[1])))) {
				...
				std::string errorMessage = "";
				npc->EquipBot(npc->CastToNPC(), &errorMessage);
				if(!errorMessage.empty()) {
					npc->CastToMob()->Say("There was an error loading my equipment");
				}
		...
		MYSQL_RES* groups = database.LoadBotGroups(charID,0,1,0);
		MYSQL_ROW row;
		int bots = mysql_num_rows(groups);
		const NPCType* tmp = 0;
		NPC* npc = 0;
		...
		MYSQL_RES* groups = database.LoadBotGroups(charID,0,0,1);
		MYSQL_ROW row;
		int bots = mysql_num_rows(groups);
		const NPCType* tmp = 0;
		NPC* npc = 0;
...
//Groupraid
	if(!strcasecmp(sep->arg[1], "groupraid")) {
		...
		int32 RaidReuse = c->GetPTimers().GetRemainingTime(pTimerCoolDownTrigger);
		if(!c->GetPTimers().Expired(&database, pTimerCoolDownTrigger, false)) {
			c->Message(13,"Wait %d seconds for spawnraid cool-down before using the groupraid command.", (RaidReuse%10));
			return;
		}
	      //const int spawnedBots = database.SpawnedBotCount(charID);
	      RaidSpawned = c->GetBotRaidGroup();
	      if(!strcasecmp(sep->arg[2], "agroup")){
		 
		  if (RaidSpawned != agroup){
		    c->Message(15, "Your agroup is not spawned, spawn your agroup first.");
		    
			if  (RaidSpawned == bgroup)
			    c->Message(15, "If bgroup is spawned, you can group with bgroup.");
			else if (RaidSpawned == cgroup)
			    c->Message(15, "If cgroup is spawned, you can group with cgroup.");
			else
			    c->Message(15, "you have no groups spawned.");
			...
			if((g->BotGroupCount() < 6) && (atoi(row[0]) == group)) {
				mtmp = entity_list.GetMobByNpcTypeID(atoi(row[1]));
					...
					*/
					...
					//mtmp->Say("group count is at %i",group);
				
				    }
			}
			else {
				group++;
				bool InClientGroup = false;
				mtmp = entity_list.GetMobByNpcTypeID(atoi(row[1]));
				if(mtmp) {
					mtmp->SetFollowID(id);
					id = mtmp->GetID();
					...
					for(unsigned int k=0; k<MAX_GROUP_MEMBERS; ++k) {
						if(g->members[k]) {
							g->members[k]->SetBotRaiding(true);
							g->members[k]->SetBotRaidID(br->GetBotRaidID());
							 if (g->members[k]->IsClient())
							   InClientGroup = true;
							 if ((c->GetClientVersion() >= EQClientSoD) && (InClientGroup)){
							   g->SendHPPacketsFrom(g->members[k]);
							   g->members[k]->SetManaBarStatus(true);
							 }
							 else{
							   g->members[k]->SetManaBarStatus(false);
							 }
						}
					}
					br->AddBotGroup(g);
					...
					{
						for(unsigned int n=0; n<MAX_GROUP_MEMBERS; ++n) {
							if(g->members[n]) {
								g->members[n]->SetBotRaiding(true);
								g->members[n]->SetBotRaidID(br->GetBotRaidID());
								g->members[n]->SetManaBarStatus(false);
							}
						}
					}
					br->AddBotGroup(g);
		...
		return;
		  ...
		  if (RaidSpawned != bgroup){
		    c->Message(15, "Your bgroup is not spawned, spawn your bgroup first.");
		    
			if  (RaidSpawned == agroup)
			    c->Message(15, "If agroup is spawned, you can group with agroup.");
			else if (RaidSpawned == cgroup)
			    c->Message(15, "If cgroup is spawned, you can group with cgroup.");
			else
			    c->Message(15, "you have no groups spawned.");
			...
			if((g->BotGroupCount() < 6) && (atoi(row[0]) == group)) {
				mtmp = entity_list.GetMobByNpcTypeID(atoi(row[1]));
					...
					*/
					...
					if ((mtmp->IsGrouped()) && (c->GetClientVersion() >= EQClientSoD)){
					  g->SendHPPacketsFrom(mtmp);
					   mtmp->SetManaBarStatus(true);
					 }
					 else{
					   mtmp->SetManaBarStatus(false);
					 }
		...
		return;
		  ...
		  if (RaidSpawned != cgroup){
		    c->Message(15, "Your cgroup is not spawned, spawn your cgroup first.");
		    
			if  (RaidSpawned == agroup)
			    c->Message(15, "If agroup is spawned, you can group with agroup.");
			else if (RaidSpawned ==bgroup)
			    c->Message(15, "If bgroup is spawned, you can group with bgroup.");
			else
			    c->Message(15, "you have no groups spawned.");
...
//SaveRaid
	if(!strcasecmp(sep->arg[1], "saveraid")) {
		...
		if(br1) {
			database.DeleteBotGroups(charID, 1, 0, 0);
			...
			if(g) {
				...
				for(unsigned int j=0; j<MAX_GROUP_MEMBERS; j++) {
					if(g->members[j]) {
						...
						database.SaveBotGroups(0, charID, g->members[j]->GetNPCTypeID(), j, g->members[j]->GetHP(), g->members[j]->GetMana(), 1, 0, 0);
					}
				}
				c->Message(15, "Your agroup raid is saved.");
			}
		...
		if(br2) {
			database.DeleteBotGroups(charID, 0, 1, 0);
			...
			if(g) {
				...
				for(unsigned int k=0; k<MAX_GROUP_MEMBERS; k++) {
					if(g->members[k]) {
						...
						database.SaveBotGroups(0, charID, g->members[k]->GetNPCTypeID(), k, g->members[k]->GetHP(), g->members[k]->GetMana(), 0, 1, 0);
					}
				}
				c->Message(15, "Your bgroup raid is saved.");
			}
		...
		if(br3) {
			database.DeleteBotGroups(charID, 0, 0, 1);
			...
			if(g) {
				database.DeleteBotGroups(charID, 0, 0, 1);
				for(unsigned int m=0; m<MAX_GROUP_MEMBERS; m++) {
					if(g->members[m]) {
						if(g->members[m]->IsClient()) {
							continue;
						}
						database.SaveBotGroups(0, charID, g->members[m]->GetNPCTypeID(), m, g->members[m]->GetHP(), g->members[m]->GetMana(), 0, 0, 1);
					}
				}
				c->Message(15, "Your cgroup raid is saved.");
			}
...
/*		if(c->IsBotRaiding()) {
		...
		if(c->IsBotRaiding()) {
                    c->Message(15, "This is for forming groups while not in raid-mode.");
					return;
		}
		if(c->IsGrouped()){
		    Group *g = entity_list.GetGroupByMob(c);  //engaged
		...
		if ( c->IsGrouped() ){
		  Group *g = entity_list.GetGroupByClient(c);
			...
			if(g && (g->BotGroupCount() > 5)){
			      ...
			      kmob->check_grouped_timer.Start(10, false);
			      return;
			}
			if(g && (c->CastToMob() != g->members[0])) {
		...
		if(c->GetTarget()->IsMob() && !c->GetTarget()->IsPet()){
		  ...
		  int32 MygroupID = database.GetBotGroupID(charID);
		  int32 GroupID = database.GetBotGroupID(charID);
		  int32 LiveGroupID = database.GetGroupID(c->GetName());
		        if (LiveGroupID > 0){
			   c->SetHasClient(true);
			   //b->Say("Found a client, GroupID is %i", LiveGroupID);
			}
			else{
			   c->SetHasClient(false);
			   //b->Say("No client, GroupID is %i", GroupID);
			}
			...
			if(database.GetBotOwner(b->GetNPCTypeID()) != charID){
			...
			//bool HasGrouP = database.BotHasGroup(b->GetNPCTypeID(), charID);
			if (c->GroupHasClient())
			  MygroupID = LiveGroupID;
			
			else{
			  if (((MygroupID >= 1) && (MygroupID <= 19)) || 
			    (( MygroupID == 0) && (c->IsGrouped()))){ 
			    if(SavedGroupID != MygroupID){
				...
				b->check_grouped_timer.Start(10, false);
				return;
			    }
			  }
			}
			...
			//1: Set its leader
			if (b->HasManaBar())
			  b->CastToClient()->mana_barCheck_timer.Start(60000);
			  
			b->Say("I'm becoming %s\'s bot!", c->GetName());
			//c->Message(15, "GroupID is %i.", LiveGroupID);
			...
			if (c->GroupHasClient()){
			   database.SetGroupID(b->GetCleanName(), LiveGroupID, b->GetNPCTypeID());
			}
			...
			else{  // Angelox: regroups them if leader bot is disbanded.
			    Group *g = c->GetGroup();
			    if (g){
				int botfollowid = 0;
				for (unsigned int i=0; i<MAX_GROUP_MEMBERS; i++) {
...
*/
			...
			//TODO: Next two vars sets to half full no matter what.
			//int HP = b->GetBotHP(GroupID, b->GetNPCTypeID());
			//int MANA = b->GetBotMana(GroupID, b->GetNPCTypeID());
			b->CalcBotStats();
			//b->SetHP(HP);
			...
			//b->SendPosition();
			 ...
			 if (c->GetClientVersion() >= EQClientSoD){
			   b->SetManaBarStatus(true);
			 }
			 else{
			   b->SetManaBarStatus(false);
			 }
//BotHelp
			if(b->GetClass() == CLERIC) {
				...
				b->BotSay("I have the following additional commands available at your request: #bot invis, #bot rune, #bot slow, #bot pacify, #bot charm, #bot dire charm, #bot auto mez, #bot ai mez, #bot tash, #bot wtash, #bot levitate, #bot endureb, #bot charisma.");
			...
			else if (b->GetClass() == PALADIN){
				b->BotSay("I have the following additional command available at your request: #bot LayOnHands.");
				b->BotSay("Use #bot paladin help, if you wish to see my commands again");
			}
			else if (b->GetClass() == SHADOWKNIGHT){
				b->BotSay("I have the following additional command available at your request: #bot HarmTouch.");
				b->BotSay("Use #bot shadowknight help, if you wish to see my commands again");
			}
			if (c->GroupHasClient()){
				c->Message(13, "WARNING!! You have live player(s) in your group - group will be disbanded on zoning.");
			}
		}
		return;
	}
...
//Angelox: New Commands Start
	if(!c->IsGrouped()) {
				...
				for(unsigned int i=0; i<MAX_GROUP_MEMBERS; i++) {
					if(g->members[i] && g->members[i]->IsBot()) {
						...
						g->members[i]->WipeHateList();
						  if (database.BotSpamLookup(database.GetBotOwner( c->CharacterID())) < 2)
		...
		else if(c->IsGrouped())
		{
			...
			c->SetBotGuarding(true);
			}
		}
		return;
	}
	else if(!strcasecmp(sep->arg[1], "attack")) { //Bot attack
		if (((zone->IsClassic()) && (c->GetTarget() == NULL)) || ((!zone->CanRaid()) && (c->GetTarget() == NULL)) || 
		   ((!zone->CanRaid()) && (c->GetTarget() == c)) || ((!zone->CanRaid()) && (c->GetTarget()->IsBot()))){ 
			c->Message(15, "Target the mob and re-try the command.");
		}	  
		else if((!zone->CanRaid()) && (c->IsBotRaiding()) && ((BotLevel >= 50) && (target->GetLevel() <= 49))) {
			c->Message(15, "You cannot form raid groups in this zone.");
		}
		else if((!HasKarma) && (zone->IsClassic()) && (BotLevel >= 50) && (target->GetLevel() <= 49)){
			c->Message(15, "Targets level is trivial for you, must be at least level 50.");		  
			c->Message(15, "This zone is trivial for you, try a zone with higher level mobs.");
		}
		else if(!c->IsGrouped()){
		...
		else if((c->GetTarget() != NULL) && c->IsAttackAllowed(c->GetTarget()))
		{
			...
			if((target) && ((target->IsBot()) || (target->IsClient()))) {
			  c->Message(15, "You must target a monster(1).");
				return;
			}
			
			if(target && target->IsMezzed()) {
			  c->Message(15, "Not while target is mezzed.");
				return;
			}
			
			if(c->IsEngaged()){
			  c->Message(15, "You're already engaged.");
				return;
			}
		...
		{
			c->Message(15, "You must target a monster(2).");
		}
		return;
	}
	else if(!strcasecmp(sep->arg[1], "assist")) {
		if (((!HasKarma) && (zone->IsClassic()) && (c->GetTarget() == NULL)) || ((!zone->CanRaid()) && (c->GetTarget() == NULL)) || 
		   ((!zone->CanRaid()) && (c->GetTarget() == c)) || ((!zone->CanRaid()) && (c->GetTarget()->IsBot()))){ 
			c->Message(15, "Target the mob and re-try the command.");
			return;
		}	  
		...
		if((!HasKarma) && (zone->IsClassic()) && (BotLevel >= 50) && (target->GetLevel() <= 49)){
			c->Message(15, "Targets level is trivial for you, must be at least level 50.");		  
			c->Message(15, "This zone is trivial for you, try a zone with higher level mobs.");
			return;
		}
...
// Original Group Order commands
	if(!strcasecmp(sep->arg[1], "group") && !strcasecmp(sep->arg[2], "order")){
	    ...
	    else if(!strcasecmp(sep->arg[3], "attack")){ //Bot group order attack
			if (((!HasKarma) && (zone->IsClassic()) && (c->GetTarget() == NULL)) || ((!zone->CanRaid()) && (c->GetTarget() == NULL)) || 
				((!zone->CanRaid()) && (c->GetTarget() == c)) || ((!zone->CanRaid()) && (c->GetTarget()->IsBot()))){ 
					c->Message(15, "Target the mob and re-try the command.");
					return;
	...
	if((!strcasecmp(sep->arg[1], "inventory")) && (!strcasecmp(sep->arg[2], "remove"))){ //Angelox3
		if((c->GetTarget() == NULL) || (sep->arg[3] == '\0') || !c->GetTarget()->IsBot()){
			...
			if(c->GetTradeskillObject() || (c->trade->state == Trading))
			    ...
			    if(br) {
				if(br->GetBotRaidAggro()) {
				  c->Message(15, "Not while raid is engaged");
				  return;
				}
			    }
			}
			else if ((c->CheckAggro(c)) || (c->GetAggroCount() > 0)) {
				c->Message(15, "Not while you have aggro.");
				return;
			}
			else if(c->IsGrouped()){
				Group *g = entity_list.GetGroupByMob(c);  //engaged
				for (unsigned int i=0; i<MAX_GROUP_MEMBERS; i++) {
				    if(g && g->members[i] && g->members[i]->IsEngaged()){
				      c->Message(15, "Not while group is engaged.");
					return;
				    }
				}
			}
			
			int lock1ID = (database.BotLock1Lookup(charID));
			if (lock1ID >1){
			    database.UpdateBotLock1(1,charID);
			    target->SetBotPrimary(false);
			    c->Message(15, "Not while 'Set Primary' is active (Set Primary is now disabled)");
			    return;
			}
			...
			if(!hasAug) {
				if(itm) {
					...
					if(materialFromSlot != 0xFF) {
						...
						gearbot->CastToNPC()->SendBotWearChange(materialFromSlot);
					}
	...
	if((!strcasecmp(sep->arg[1], "unequip")) ||  (!strcasecmp(sep->arg[1], "remove")))  { //Angelox3
		...
		else if(c->GetTarget()->IsBot() && (c->GetTarget()->BotOwner == c->CastToMob()))
		{
			...
			int slotId = 255;	  
			if ((!strcasecmp(sep->arg[2], "primary")) || (!strcasecmp(sep->arg[2], "13"))){ 
				slotId = 13;}
			else if ((!strcasecmp(sep->arg[2], "secondary")) || (!strcasecmp(sep->arg[2], "14"))){ 
				slotId = 14;}
			else if ((!strcasecmp(sep->arg[2], "charm")) || (!strcasecmp(sep->arg[2], "0"))){ 
				slotId = 0;}
			else if ((!strcasecmp(sep->arg[2], "left") && (!strcasecmp(sep->arg[3], "ear"))) || (!strcasecmp(sep->arg[2], "1"))){ 
				slotId = 1;}
			else if ((!strcasecmp(sep->arg[2], "head")) || (!strcasecmp(sep->arg[2], "2"))){ 
				slotId = 2;}
			else if ((!strcasecmp(sep->arg[2], "face")) || (!strcasecmp(sep->arg[2], "3"))){ 
				slotId = 3;}
			else if ((!strcasecmp(sep->arg[2], "right") && (!strcasecmp(sep->arg[3], "ear"))) || (!strcasecmp(sep->arg[2], "4"))){ 
				slotId = 4;}
			else if ((!strcasecmp(sep->arg[2], "neck")) || (!strcasecmp(sep->arg[2], "5"))){ 
				slotId = 5;}
			else if ((!strcasecmp(sep->arg[2], "shoulders")) || (!strcasecmp(sep->arg[2], "6"))){ 
				slotId = 6;}
			else if ((!strcasecmp(sep->arg[2], "arms")) || (!strcasecmp(sep->arg[2], "7"))){ 
				slotId = 7;}
			else if ((!strcasecmp(sep->arg[2], "back")) || (!strcasecmp(sep->arg[2], "8"))){ 
				slotId = 8;}
			else if ((!strcasecmp(sep->arg[2], "left") && (!strcasecmp(sep->arg[3], "wrist"))) || (!strcasecmp(sep->arg[2], "9"))){ 
				slotId = 9;}
			else if ((!strcasecmp(sep->arg[2], "right") && (!strcasecmp(sep->arg[3], "wrist"))) || (!strcasecmp(sep->arg[2], "10"))){ 
				slotId = 10;}
			else if ((!strcasecmp(sep->arg[2], "range")) || (!strcasecmp(sep->arg[2], "11"))){ 
				slotId = 11;}
			else if ((!strcasecmp(sep->arg[2], "hands")) || (!strcasecmp(sep->arg[2], "12"))){ 
				slotId = 12;}
			else if ((!strcasecmp(sep->arg[2], "left") && (!strcasecmp(sep->arg[3], "finger"))) || (!strcasecmp(sep->arg[2], "15"))){ 
				slotId = 15;}
			else if ((!strcasecmp(sep->arg[2], "right") && (!strcasecmp(sep->arg[3], "finger"))) || (!strcasecmp(sep->arg[2], "16"))){ 
				slotId = 16;}
			else if ((!strcasecmp(sep->arg[2], "chest")) || (!strcasecmp(sep->arg[2], "17"))){ 
				slotId = 17;}
			else if ((!strcasecmp(sep->arg[2], "legs")) || (!strcasecmp(sep->arg[2], "18"))){ 
				slotId = 18;}
			else if ((!strcasecmp(sep->arg[2], "feet")) || (!strcasecmp(sep->arg[2], "19"))){ 
				slotId = 19;}
			else if ((!strcasecmp(sep->arg[2], "waist")) || (!strcasecmp(sep->arg[2], "20"))){ 
				slotId = 20;}
			else if ((!strcasecmp(sep->arg[2], "ammo")) || (!strcasecmp(sep->arg[2], "21"))){ 
				slotId = 21;}			
			...
			if(slotId > 21 || slotId < 0) {
				c->Message(15, "or by one of the bots 21 slot IDs in its inventory; you can alternately choose a slot between 0 and 21.");
				return;
			}
			const char* equipped[22] = {"Charm", "Left Ear", "Head", "Face", "Right Ear", "Neck", "Shoulders", "Arms", "Back",
										"Left Wrist", "Right Wrist", "Range", "Hands", "Primary Hand", "Secondary Hand",
...
//update
	...
	if(!strcasecmp(sep->arg[1], "update")) {
		...
		//Angelox fixed update bots outside your group exploit
			if(t && (database.GetBotOwner(t->GetNPCTypeID()) != charID)){
	...
	if(!strcasecmp(sep->arg[1], "bindme")) {
		  ...
		  if(g) {
			for(unsigned int i=0; i<MAX_GROUP_MEMBERS; i++){
				if(((g->members[i]) && (g->members[i]->IsBot()) && (g->members[i]->GetClass() == CLERIC))
				  || ((g->members[i]) && (g->members[i]->IsBot()) && (g->members[i]->GetClass() == MAGICIAN))
			...
			if (binder == NULL)
			{
			  c->Message(15, "No class match for this spell in your Bot group.");
			  c->Message(15, "Valid classes are: Cleric level 10 or higher,");
			  c->Message(15, "Druid, Wizard, Enchanter, Necromancer Level 12 or higher,");
			  c->Message(15, "Shaman Level 14 or higher.");
			  return;
			}
		...
		if(hasbinder) {
		  if (bindID < 1){
			...
			int CurMana = binder->GetMana();
			if (CurMana < 100)
			  binder->BotSay("Not enough mana!");
			else
...
//Rune
	...
	if(!strcasecmp(sep->arg[1], "rune")) {
		...
		if((t == NULL) || ((t->IsNPC()) && (!t->IsBot())) || (!t->IsGrouped()) ||
			((t->IsBot()) && (database.GetBotOwner(t->GetNPCTypeID()) != charID)) ||  
		...
		if(hasruneer) {
			...
			c->Message(15, "No class match for this spell in your Bot group.");
			c->Message(15, "You need an Enchanter in your group.");
			return;
		}
		return;
	}
...
//Skin (Druids Skin Like * buffs)
	...
	if(!strcasecmp(sep->arg[1], "skin")) {
		Mob *t = c->GetTarget();
		if (t == NULL){
			c->Message(15, "You must select a player, bot or pet in the zone.");
			return;
		}
		Mob *skiner = NULL;
		...
		if(hasskiner) {
		    if((target == c )||(target->IsBot())||((target->IsPet())&&(target->GetOwner()->IsBot()))
			||((target->IsPet())&&(target->GetOwner()->IsClient()))){
			
			skiner->InterruptSpell();
			int CurMana = skiner->GetMana();
			/* if      (skiner->GetLevel() <= 12) {
				skiner->BotSay("I need to be level 13 or higher for this...");
			} */
			if 	((skiner->GetLevel() >= 1) && (skiner->GetLevel() <= 13)) {
				if (CurMana < 10)
				  skiner->BotSay("Not enough mana!");
				else
...
//Cure
	...
	if ((!strcasecmp(sep->arg[1], "cure")) && (c->IsGrouped())) {
		...
		if(g) {
			...
			if(CurerClass == 0){
				  c->Message(15, "No class match for this spell in your Bot group.");
				  c->Message(15, "Valid classes are Cleric, Druid, Shaman.");
				  return;
			}
			  
			int CurMana = Curer->GetMana();
			switch(CurerClass) {
				case CLERIC:
					Curer->InterruptSpell();
					if	(!strcasecmp(sep->arg[2], "poison") && (BotLevel >= 1))  {
						if (CurMana < 20)
						  Curer->BotSay("Not enough mana!");
						else
		...
		if(g) {
			for(unsigned int i=0; i<MAX_GROUP_MEMBERS; i++){
				if(g->members[i] && g->members[i]->IsBot() && (g->members[i]->GetClass() == CLERIC)) {
				    ...
				    if (Special) {
					  const int hasTemp = (database.BotTempLookup(charID));
					  if((hasTemp == 0) && (BotLevel >= 40)){
						    Tempr->BotSay("We haven't completed the Bot Spell Scriber's request yet.");
						    return;
					    }
					  else if(hasTemp == 0){ //hasTemp == 1 for regular Temperance
				...
				||((target->IsPet())&&(target->GetOwner()->IsClient()))){
				  ...
				  if (BotLevel >= 40){
				      ...
				      int CurMana = Tempr->GetMana();
				      if (CurMana < 550)
					Tempr->BotSay("Not enough mana!");
				      else
...
//Magepic
	if((!strcasecmp(sep->arg[1], "magepic")) && (!c->IsGrouped())) {
		c->Message(15, "You need to group with a bot for this");
		return;
	}
	else if((!strcasecmp(sep->arg[1], "magepic")) && (c->IsGrouped())) {
		bool hasMage = false;
		Group *g = c->GetGroup();
		//int32 charid = (c->CharacterID());
		const int hasMagepic = (database.BotMagepicLookup(charID));
		if ((c->GetTarget() == NULL)  || (c->GetTarget()->IsNPC())){
		    c->Message(15, "Target yourself first.");
		    return;
		}
		if(g) {
			for(unsigned int i=0; i<MAX_GROUP_MEMBERS; i++){
			    if(g->members[i] && g->members[i]->IsBot() && (g->members[i]->GetClass() == MAGICIAN)) {
				      hasMage = true;
				      Mob *Mage = g->members[i];
				      if(hasMagepic <= 0){
					    Mage->BotSay("We haven't completed the Bot Spell Scriber's request yet.");
					    return;
					   }
				      else
					    Mage->InterruptSpell();
					    Mage->CastToClient()->CastSpell(1944, c->GetID(), 1, -1, -1); // MageEpic
					    Mage->SpellOnTarget(1944, t); //Latter for text, this one is the real Mcoy
					    return;
				      }
			       }
			}
			if(!hasMage) {
				c->Message(15, "You must have a Magician in your group.");
			}
		else {
			c->Message(15, "You must have a Magician in your group.");
		}
  ...
  if (!strcasecmp(sep->arg[1], "change")){ 
	 ...
	 // !strcasecmp(sep->arg[2], "heritage") || !strcasecmp(sep->arg[2], "tattoo") || !strcasecmp(sep->arg[2], "details")){
	  ...
	  int32 thisBot = t->GetNPCTypeID();
		if(c->GetTarget() && c->GetTarget()->IsBot()) {
			if  (sep->IsNumber(3)) {
				if (database.GetBotOwner(c->GetTarget()->GetNPCTypeID()) == charID) {
		...
		if(g) {
			for(unsigned int i=0; i<MAX_GROUP_MEMBERS; i++){
				if(g->members[i] && g->members[i]->IsBot() && (g->members[i]->GetClass() == ENCHANTER)) {
					...
					const int hasWtashr = (database.BotWtashrLookup(charID));
			...
			if(c->IsGrouped()) {
				...
				if (pacerClass !=0) {
					...
					if (!pacer->CheckLosFN(t)){
					     pacer->BotSay(" No line of sight", sep->arg[2]);
 						 if (c->IsBotGuarding()){
 						    pacer->SetFollowID(ClientID);
 						    pacer->BotSay(" Moving closer to you - try again.", sep->arg[2]);
 						 }
					     return;
					}
					if(pacer->CastToNPC()->Bot_Command_CalmTarget(target)) {
						if(target->FindType(SE_Lull) || target->FindType(SE_Harmony) || target->FindType(SE_Calm)){
							pacer->BotSay( "Successfully pacified %s.", target->GetCleanName());
							  if ((pacer->GetFollowID() ==ClientID) && (c->IsBotGuarding()))
							      pacer->SetFollowID(ClientID +1);
						}
						else{
...
//Remove Bot's Pet
	...
	if(!strcasecmp(sep->arg[1], "pet") && !strcasecmp(sep->arg[2], "remove")) {
        if(c->GetTarget() != NULL) {
			if (c->IsGrouped() && c->GetTarget()->IsBot() && (database.GetBotOwner(c->GetTarget()->GetNPCTypeID()) == charID) && 
...
//Title
	...
	if(!strcasecmp(sep->arg[1], "title")){
		if ((!t->IsBot()) || (t == NULL) || (database.GetBotOwner(t->GetNPCTypeID()) != charID)){
			c->Message(15, "You must select one of your bots.");
			c->Message(15, "Usage: #bot title <lastname, a phrase or tag> <Target>.");
			return;
		}
...
//Heal
	...
	if(!strcasecmp(sep->arg[1], "heal")){
		...
		if(g) {
			...
			if((((BotLevel >= 63) && (BotLevel <= 75)) && (CurMana < 600))||
			   (((BotLevel >= 58) && (BotLevel <= 62)) && (CurMana < 498))||
			   (((BotLevel >= 53) && (BotLevel <= 57)) && (CurMana < 355))||
			   (((BotLevel >= 51) && (BotLevel <= 52)) && (CurMana < 167))||
			   (((BotLevel >= 30) && (BotLevel <= 50)) && (CurMana < 185))||
			   (((BotLevel >= 20) && (BotLevel <= 29)) && (CurMana < 115))||
			   (((BotLevel >= 10) && (BotLevel <= 19)) && (CurMana < 65))||
			   (((BotLevel >= 4) && (BotLevel <= 9)) && (CurMana < 28)) ||
...
//HarmTouch
	if(!strcasecmp(sep->arg[1], "HarmTouch"))
	{
	
		int16 spellid = 88;
		int32 remain = c->GetPTimers().GetRemainingTime(pTimerHarmTouch);
		bool HasSK = false;
		Mob *HTCaster = NULL;
		Group *g = c->GetGroup();
		float dist2;
	    ...
	    if (g){
		for(unsigned int i=0; i<MAX_GROUP_MEMBERS; i++){
		    if(g->members[i] && g->members[i]->IsBot() && (g->members[i]->GetClass() == SHADOWKNIGHT)) {
			HasSK = true;
			HTCaster = g->members[i];
			dist2 = HTCaster->DistNoRoot(*target);
			
		       if ((target == NULL) || target->IsBot() ||  !target->IsNPC()){
			 c->Message(15, "Invallid target");
		       }
		       else if (!c->GetPTimers().Expired(&database, pTimerHarmTouch) && !c->GetGM()) {
			 c->Message(13, "Ability recovery time not yet met - %d minutes %d seconds left.", ((remain)/60), (remain%60));
			 c->InterruptSpell();
		       }
		       else{
			  if(dist2 > 1600){
			    HTCaster->BotSay("I'm too far away!");
			  }
			  else{
			    HTCaster->BotSay("Casting HT...");
			    HTCaster->CastToNPC()->CastSpell(spellid, t->GetID(), 10, 0);
			    c->GetPTimers().Start(pTimerHarmTouch, HarmTouchReuseTime);
			  }
		       }
		    }
		}
	    ...
	    if(!g) {
	     c->Message(15, "You're not grouped."); 
	    }
	    else if(!HasSK){
	     c->Message(15, "You need a ShadowKnight in your group");
	    }
	    return;
	}

//LayOnHands
	if(!strcasecmp(sep->arg[1], "LayOnHands"))
	{
	
		int16 spellid = 87;
		int32 remain = c->GetPTimers().GetRemainingTime(pTimerLayHands);
		bool HasPally = false;
		bool RightTarget = false;
		Mob *PallyCaster = NULL;
		Group *g = c->GetGroup();
		float dist2;
		
	    if (g){
		for(unsigned int i=0; i<MAX_GROUP_MEMBERS; i++){
		    if(g->members[i] && g->members[i]->IsBot() && (g->members[i]->GetClass() == PALADIN)) {
			HasPally = true;
			PallyCaster = g->members[i];
			dist2 = PallyCaster->DistNoRoot(*target);
			
		       if (target == NULL){
			 c->Message(15, "Invallid target");
		       }
		       else if(((target->IsNPC()) && (target->IsBot())) || (target->IsClient())){
			 RightTarget = true;
		       }
		       else{
			 c->Message(15, "Target a bot or player");
		       }
		       
		       if (!c->GetPTimers().Expired(&database, pTimerLayHands) && !c->GetGM()) {
			 c->Message(13, "Ability recovery time not yet met - %d minutes %d seconds left.", ((remain)/60), (remain%60));
			 c->InterruptSpell();
		       }
		       else if (RightTarget){
			  if(dist2 > 1600){
			    PallyCaster->BotSay("I'm too far away!");
			  }
			  else{
			    PallyCaster->BotSay("Casting LoH...");
			    PallyCaster->CastToNPC()->CastSpell(spellid, t->GetID(), 10, 0);
			    c->GetPTimers().Start(pTimerLayHands, LayOnHandsReuseTime);
			  }
		       }
		    }
		}
	    ...
	    if(!g) {
	     ...
	     c->Message(15, "You need a Paladin in your group");
	    }
	    return;
	}
...
//Slow
	...
	//Charisma
	if((!strcasecmp(sep->arg[1], "charisma")) &&(!c->IsGrouped())) {
		c->Message(15, "You need to group with a bot for this");
		return;
	}
	if(!strcasecmp(sep->arg[1], "charisma")){
	    if(target == NULL){
		c->Message(15, "You must select a player or bot in the zone");
		return;
	    }
	    ...
	    if(((target->IsNPC()) && (target->IsBot())) || (target->IsClient()))
	      RightTarget = true;
		...
		if(!RightTarget){
				...
				if(g->members[i] && g->members[i]->IsBot()) {
					switch(g->members[i]->GetClass()) {
						case SHAMAN:
							...
							break;
						case ENCHANTER:
							if(CharerClass != SHAMAN){
								Charer = g->members[i];
								CharerClass = ENCHANTER;
							}
							break;
						default:
							break;
			...
			switch(CharerClass) {
				case SHAMAN:
					if (BotLevel <= 9){
						Charer->BotSay("I don't have the needed level yet", sep->arg[2]);
					    break;
						}
					if (!Charer->CheckLosFN(t)){
						Charer->BotSay("I don't have line of sight", sep->arg[2]);
					    break;
						}
						Charer->InterruptSpell();
						Charer->BotSay("Casting Charisma...");
						Charer->CastToNPC()->Bot_Command_CharisTarget (1,target);
					break;
				...
				case ENCHANTER:
					...
					if (!Charer->CheckLosFN(t)){
						...
						Charer->InterruptSpell();
						Charer->BotSay("Casting Charisma...");
						Charer->CastToNPC()->Bot_Command_CharisTarget (2,target);
					break;
				default:
					c->Message(15, "You must have an Shaman or Enchanter in your group.");
					break;
			}
		}
		return;
...
//Sow
	...
	if ((!strcasecmp(sep->arg[1], "sow")) && (c->IsGrouped())) {
		...
		Mob *t = c->GetTarget();
		Mob *Sower = NULL;
		bool HasSmallSower = false; //This means level is too low.
		int32 SowerClass = 0;
		Group *g = c->GetGroup();
		if(g) {
			for(unsigned int i=0; i<MAX_GROUP_MEMBERS; i++){
				if(g->members[i] && g->members[i]->IsBot()) {
					switch(g->members[i]->GetClass()) {
						case DRUID:
							...
							else if ((!HasSmallSower)&&(SowerClass == 0)){
							  g->members[i]->BotSay("That's my level ten spell.");
							  HasSmallSower = true;
							}
							break;
						case BEASTLORD:
							if ((BotLevel >= 24) && (SowerClass != DRUID)){
							 ...
							 g->members[i]->BotSay("That's my level 24 spell.");
							 HasSmallSower = true;
							}
							break;
						case RANGER:
							if ((BotLevel >= 28) && (SowerClass != DRUID)){
							  ...
							  g->members[i]->BotSay("That's my level 28 spell.");
							  HasSmallSower = true;
							}
							break;
						case SHAMAN:
							if ((BotLevel >= 9) && (SowerClass == 0 )){
							  ...
							  g->members[i]->BotSay("That's my level nine spell.");
							  HasSmallSower = true;
							}
							break;
						default:
		    ...
		    if(SowerClass == 0){
			  if (HasSmallSower){
			    ...
			    c->Message(15, "No class match for this spell in your Bot group.");
			    c->Message(15, "Valid classes are Ranger, Beastlord, Druid, Shaman.");
			  }
			//return;
		    }
			  
		    else if ((target == c )||(target->IsBot())||((target->IsPet())&&(target->GetOwner()->IsBot()))
			||((target->IsPet())&&(target->GetOwner()->IsClient())))
		    {
		      int CurMana = Sower->GetMana();
			switch(SowerClass) {
				case DRUID:
...
c->Message(15, "The lowest possible sow is available with a level 9 Shaman, so you have to be level 9");
	...
	if ((!strcasecmp(sep->arg[1], "shrink")) && (c->IsGrouped())) {
		...
		if(g) {
			...
			if(ShrinkerClass == 0){
				  c->Message(15, "No class match for this spell in your Bot group.");
				  c->Message(15, "Valid classes are Shaman and Beastlord.");
				  return;
			}
			  
			int CurMana = Shrinker->GetMana();
			switch(ShrinkerClass) {
				case SHAMAN:
					...
					if (BotLevel >= 15) {
						...
						if (CurMana < 50)
						  Shrinker->BotSay("Not enough mana!");
						else
...
//Gate
	...
	if ((!strcasecmp(sep->arg[1], "gate")) && (c->IsGrouped())) {
		...
		Group *g = c->GetGroup();
		bool Special = (RuleB(EQOffline, AXCustom));
		const int canGate = (database.BotCircleLookup(charID)); //Druid Gate spells
		const int canPort = (database.BotPortalLookup(charID)); //Wizard Gate spells
	   if (classicStat > 0){
		...
		if(g) {
			...
			if(GaterClass == 0){
			    c->Message(15, "You need to group with a Druid or Wizard bot for this.");
			    return;
			}
			
			int CurMana = Gater->GetMana();
			switch(GaterClass) {
				case DRUID:
				...
				case WIZARD:
					...
					if ((!strcasecmp(sep->arg[2], "twilight")) && (BotLevel >= 33) && (BotLevel <= 65)) {
						...
						if (CurMana < 300)
						  ...
						  Gater->BotSay("Relocation to Skyfire...");
						...
						if (CurMana < 300)
						  ...
						  Gater->BotSay("Relocation to Emerald Jungle...");
					    ...
					    //const int canPort = (database.BotPortalLookup(charID));
					    if (BotLevel >= 66){
...
//Endure Breath
	...
	if ((!strcasecmp(sep->arg[1], "endureb")) && (c->IsGrouped())) {
		...
		if(g) {
			...
			if(EndurerClass == 0){
			    c->Message(15, "No class match for this spell in your Bot group.");
			    c->Message(15, "Valid classes are Druid, Shaman, Enchanter, Ranger.");
			    return;
			}
			
			int CurMana = Endurer->GetMana();
			switch(EndurerClass) {
				case DRUID:
					...
					if (Endurer-> IsMoving()){
						...
						if (CurMana < 35)
						  Endurer->BotSay("Not enough mana!");
						else
...
//Invisible
	...
	if ((!strcasecmp(sep->arg[1], "invis")) && (c->IsGrouped())) {
	  ...
	  /* if((t->IsBot()) && (database.GetBotOwner(t->GetNPCTypeID()) != charID))){
	     c->Message(15,"Choose a target in your group.");
	     return;
		}
	  if((mygroup) && (!mygroup->IsGroupMember(t))){
		...
		if ((g) && (t != NULL)) {
			...
			if((InviserClass == 0) && (UInviserClass == 0)){
			    c->Message(15, "No class match for this spell in your Bot group.");
			    return;
			}
...
//Set Commands
	 ...
	 if ((c->IsBotRaiding()) && (!strcasecmp(sep->arg[2], "armorcolor"))){
	    BotRaids *br = entity_list.GetBotRaidByMob(c->CastToMob());
	      if (br){
		  br->SetRaidBotArmorColor();
	      }
	  }
	Group *g = c->GetGroup();
	if(g) {
		...
		if(g->members[i] && g->members[i]->IsBot()) {
		...
		else if(!strcasecmp(sep->arg[2], "armorcolor")){
		   botMember->ArmorColorUpadate(botMember->CastToNPC(), &errorMessage);
		    if ((target) && (target->IsClient())){
			c->Message(15, "Armor color updated.");
		    }
		  }
		  ...
		  c->Message(15, "[armorcolor] will restore bots armor colors you tinted.");
		  return;
		}
		if (sep->arg[2][0] == '\0'){
		  c->Message(15, "Usage: #bot set [primary], [snare] [on] or [off], [root] [on] or [off], [chat] [on] or [off].");
...
//Levitate
	...
	if ((!strcasecmp(sep->arg[1], "levitate")) && (c->IsGrouped())) {
		...
		Mob *t = c->GetTarget();
		Mob *Lever = NULL;
		bool HasSmallLever = false; //This means level is too low.
		int32 LeverClass = 0;
		Group *g = c->GetGroup();
		if(g) {
			for(unsigned int i=0; i<MAX_GROUP_MEMBERS; i++){
				if(g->members[i] && g->members[i]->IsBot()) {
					switch(g->members[i]->GetClass()) {
						case NECROMANCER:
							...
							else if ((!HasSmallLever)&&(LeverClass == 0)){
							  g->members[i]->BotSay("That's my level 41 spell.");
							  HasSmallLever = true;
							}
						    break;
						case RANGER:
							if ((BotLevel >= 35) && (LeverClass != NECROMANCER)){
							  ...
							  g->members[i]->BotSay("That's my level 35 spell.");
							  HasSmallLever = true;
						    break;
						case BEASTLORD:
							if ((BotLevel >= 32) && (LeverClass != NECROMANCER)){
							  ...
							  g->members[i]->BotSay("That's my level 32 spell.");
							  HasSmallLever = true;
							}
						    break;
						case WIZARD:
							if ((BotLevel >= 22) && ((LeverClass != SHAMAN) || (LeverClass != NECROMANCER) ||
							  ...
							  g->members[i]->BotSay("That's my level 22 spell.");
							  HasSmallLever = true;
							}
						    break;
						case DRUID:
							if ((BotLevel >= 14) && ((LeverClass != SHAMAN) || (LeverClass != NECROMANCER) ||
							  ...
							  g->members[i]->BotSay("That's my level 14 spell.");
							  HasSmallLever = true;
							}
						    break;
						case ENCHANTER:
			...
			if(LeverClass == 0){
			  if (HasSmallLever){
			    //Say nothing 
				  ...
				  c->Message(15, "No class match for this spell in your Bot group.");
				  c->Message(15, "Valid classes are Necromancer, Ranger, Beastlord, Wizard, Druid, Enchanter, Shaman.");
			  }
			 //return; 
			}
...
// fez_ajer: Bot mana command
	if(!strcasecmp(sep->arg[1], "mana")) {
		Group *g = c->GetGroup();
		if (c->IsBotGuarding()){
		  c->Message(15, "Not while bots are guarding.");
		  return;
		}
		if (!c->ReSendManaCheck_timer.Enabled()){
		    c->ReSendManaCheck_timer.Start(5000, false);
		}
		else if (c->ReSendManaCheck_timer.Enabled()){
		    if (!c->ReSendManaCheck_timer.Check(false)) {
			c->Message(15, "Mana check not ready yet - wait a few seconds.");
			return;
		    }
		}
			...
			for(unsigned int i=0; i<MAX_GROUP_MEMBERS; i++){
				if(g->members[i] && g->members[i]->IsBot() && (g->members[i]->GetClass() != WARRIOR) &&
					...
					hasCasterBots = true;
					  ...
					  if (g->members[i]->GetManaRatio() > 90)
					    g->members[i]->SayRandomBotMessage(35);
					  else
					    g->members[i]->BotSay("I'm at %0.0f percent mana.", g->members[i]->GetManaRatio());
					  
					  if ((!g->members[i]->IsEngaged()) && (g->members[i]->HasManaBar())){
					    g->SendHPPacketsFrom(g->members[i]);
					    //c->Message(15, "---Debug---ManaBarUpdate---On-------.");
					  }
				}
			}
...
/*
	if(!strcasecmp(sep->arg[1], "battlerez")) {
	    int BotsSpawned = c->spawned_bots.size();
		if(c->GetAggroCount()) {
			if(database.SpawnedBotCount(charID) < BotsSpawned) {
								...
								if(g->members[j]) {
									if(g->members[j]->IsBot()) {
										if(g->members[j]->GetNPCTypeID() != *iter) {
											...
											entity_list.RezBotMember(iter, 0, j, 0, g, c);
											c->battle_rez_timer.Disable();
											break;
										}
										++iter;
...
// offtank
	if(!strcasecmp(sep->arg[1], "offtank")) {
		...
		if((!HasKarma) && (zone->IsClassic()) && (BotLevel >= 50)) { 
		      c->Message(15, "You are in a trivial zone!");
		...
		for(unsigned int j = 0; j < (br ? MAX_BOT_RAID_GROUPS : 1); j++) {
	 ...
	 if(!strcasecmp(sep->arg[2], "info"))
		...
		else if(!strcasecmp(sep->arg[2], "group") && !strcasecmp(sep->arg[3], "create"))
        {
			if((c->GetTarget() == NULL) || !c->GetTarget()->IsBot() || c->GetFeigned()) {
				c->Message(15, "You need to target the bot, and not be feigned.");
				return;
			}
        ...
        else if(!strcasecmp(sep->arg[2], "invite") && !strcasecmp(sep->arg[3], "bot"))
        {
			...
			if(c->GetTarget()->IsGrouped()) {
				...
				if(g && (g->BotGroupCount() > 5)) {
					...
					//inv->BotOwner = NULL;
					inv->check_grouped_timer.Start(10, false);
					return;
				}
				if(g && (g->BotGroupCount() < 6))
                {
					...
					c->Message(15, "Don't forget to save your changes with the saveraid command.");
				}
				else
		...
		else if(!strcasecmp(sep->arg[2], "disband"))
        {
			if (c->IsBotRaiding()) {
			  Group *g = c->GetGroup();
			    ...
			    c->Message(15, "You have no raid to disband.");
			}
			return;
	}
