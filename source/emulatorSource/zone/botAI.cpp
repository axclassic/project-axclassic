...
void Mob::BOT_Process() {
	...
	if(check_grouped_timer.Enabled()) {
		if (check_grouped_timer.GetRemainingTime() <= 10){
    ...
    if(!BotOwner || BotOwner->qglobal || BotOwner->AmIaBot || 
      GetAppearance() == eaDead){
	  return;
    }
	
    if (!BotOwner->CastToClient()->IsGrouped()){
	  BotKill();
	  return;
    }
	...
	/*BotSay("check grouped timer disabled.");
	  if (BotOwner->CastToClient()->GroupHasClient())
	    BotSay("We got another client in the group!");*/
    }
    
    if((botClass == CLERIC) || (botClass == SHAMAN) || (botClass == ENCHANTER) || 
      (botClass == DRUID) || (botClass == WIZARD) || (botClass == NECROMANCER) ||
      (botClass == MAGICIAN)){
	  IsFighter = false;
    }
...
///Engaged
	...
	if(target){
	  /*if ((!IsMoving()) && (randInt <10)){
	    BotSay("Face Target-10.");
	    FaceTarget();
	  }*/
	...
	if((target) && (!IsFighter)){	
	  if((!CheckLosFN(target)) && (!BotOwner->CheckLosFN(GetTarget()))){
	     if (!zone->pathing){
		   if (botClass == CLERIC){
			if ((MakeRandomInt(1, 5)) && (SetChat())) //TODO random number here is going nowhere.
		  ...
		  if (IsBotRaiding()){
		    ...
		    InterruptSpell();
		    BotWarp(BotOwner->GetX(), BotOwner->GetY(), BotOwner->GetZ());
	     }
	     else{
		float dist2 = DistNoRoot(*target);
		bool WaypointChanged, NodeReached;
		VERTEX Goal = UpdatePath(GetTarget()->GetX(), GetTarget()->GetY(), GetTarget()->GetZ(),
		  walkspeed, WaypointChanged, NodeReached);
		  if(WaypointChanged)
		    tar_ndx = 20;
		  if ((dist2 > 75)&&(!IsBotArcher())){
			...
			check_attack_timer.Start(5000, false); //Try to reach target for five seconds, then if not there, warp.
			 ...
			 if(check_attack_timer.Check(false)) {
			    ...
			    CalculateNewPosition2(target->GetX(), target->GetY(), target->GetZ(), speed, false);
			 }
			 else
			    CalculateNewPosition2(Goal.x, Goal.y, this->GetZ(), speed);
		    }
		}
		else if(ReSendPosition_timer.Check(false)){
		      SendPosition();
		}
	    }
	  }
	...
	if ((target) && ((target->IsMezzed()) || (!IsAttackAllowed(target)))){
	    WipeHateList();
	    CastToNPC()->SetTarget(BotOwner);
	    return;
	}
	...
	if((botLevel <= 12) || (IsFighter)){
	  ...
	  if(target){
		...
		if(MakeRandomInt(1, 100) > 95) {
          ...
          if(target)
	  {
            if(!IsBotArcher() && !IsStunned() && !IsMezzed() && (GetAppearance() != eaDead))
            {
				...
				if(botClass == BARD || botClass == RANGER || botClass == SHADOWKNIGHT 
					  ...
					  float dist2 = DistNoRoot(*target);
						...
						if (zone->pathing){
						    ...
						    if(!check_attack_timer.Enabled()){
						      ...
						      if(check_attack_timer.Check(false)) {
							  Warp(target->GetX(), target->GetY(), target->GetZ());
							  CalculateNewPosition2(target->GetX(), target->GetY(), target->GetZ(), this->GetRunspeed(), false);
						      }
						      else{
					      ...
					      CalculateNewPosition2(GetTarget()->GetX(), GetTarget()->GetY(), GetTarget()->GetZ(), speed);
					    }
					    
					    if ((IsMoving()) || (dist2 > 10000)){
					      if(!check_attack_timer.Enabled()){
						check_attack_timer.Start(5000, false); //Try to reach target for five seconds, then if not there, warp.
						FaceTarget(target);
					      }
...
///End Engaged
		...
		if (!check_Moved_timer.Enabled()){ //Angelox: Start the movement checks - this makes sure they are stopped and not lagged, also keeps them from trying to cast or med while moving.
		   check_Moved_timer.Start(MakeRandomInt(5000,14000), false);
		}
	    ...
	    {
			if(!target) {
			  if ((!IsBotGuarding()) && (!GetFollowID())){
				SetFollowID(CastToClient()->GetID());
			  }
			  else{
				SetFollowID(0);
			  }
				...
				if ((!target->IsClient()) && (dist2>30)){
				  if(!CheckLosFN(target) && (zone->pathing)){ //Angelox Node pathing
				    //float speed = GetRunspeed();
				    bool WaypointChanged, NodeReached;
				    VERTEX Goal = UpdatePath(GetTarget()->GetX(), GetTarget()->GetY(), GetTarget()->GetZ(),
				   ...
				   if ((!IsBotRaiding()) && (SetChat())){ //Follow Chat
				      if (MakeRandomInt(1,3000) < 2){
					if (zoNe == 58)
				  ...
				  if (check_Moved_timer.Enabled()) {
					...
					if(moved) {
						...
						if (ReSendPosition_timer.Check(false)){
						     SendPosition();
						     ReSendPosition_timer.Start(100, false);
						}
						...
						if ((!zone->pathing) && (target) &&(!IsEngaged())){
			...
			if (GetBotAutoMez()) {
			  Mob *mezMob = BotGetMobToMez(0,0,1);
				if (mezMob && CastToNPC()->Bot_Command_MezzTarget(mezMob,0,0,1)){
				   ...
				   if ((!IsBotRaiding()) && (SetChat())){
				      if (MakeRandomInt(1,100) < 60){
					SayRandomBotMessage(42); //TODO This is for chanter - need necro.
				      }
				      else{
					BotSay("Mezzing << %s >>", mezMob->GetCleanName());
				      }
...
bool EntityList::Bot_AICheckCloseBeneficialSpells(NPC* caster, int8 iChance, float iRange, int16 iSpellTypes) {
	...
	//Update bots mana bar (if SoD or greater)
	/*
	if(caster->IsGrouped()){
	  bool InClientGroup = false;
	  Group *g = entity_list.GetGroupByMob(caster);
		if (caster->HasManaBar()){ //Means is client is SoD or later.
		      if (((!caster->CastToClient()->mana_barCheck_timer.Enabled())||
			  (caster->CastToClient()->mana_barCheck_timer.Check(false)))&&
			  (caster->GetManaRatio() > 5) && (caster->GetManaRatio() < 99)){
			      //Check Caster is in client group (while raiding)
			      if (caster->IsBotRaiding()){
			  ...
			  if (InClientGroup){
			    caster->CastToClient()->SendHPUpdate();
			    //caster->BotSay(">>>Updating manabar .. >>>");
			      if (caster->IsEngaged())
				caster->CastToClient()->mana_barCheck_timer.Start(30000);
			      else
				caster->CastToClient()->mana_barCheck_timer.Start(20000);
			  }
		      }
		}
	}
	*/
	...
	if( botCasterClass == CLERIC || botCasterClass == DRUID || botCasterClass == SHAMAN || botCasterClass == PALADIN
	  || botCasterClass == BEASTLORD || botCasterClass == RANGER || botCasterClass == NECROMANCER /*|| botCasterClass == WIZARD*/)
	{
			...
			if(caster->IsGrouped())
			{
				...
				if(g) {
					...
					for(unsigned int i = 0; i < MAX_GROUP_MEMBERS; i++) {
						...
						if ((GroupMemberByHP[i]) && (!GroupMemberByHP[i]->CastToMob()->DivineAura()) && (!GroupMemberByHP[i]->qglobal) && ((GroupMemberByHP[i]->GetHPRatio() > 5) 
						...
						if ((!caster->CastToClient()->evac_hold_timer.Enabled()) && (GroupMemberByHP[i]) && 
						  ...
						  //if (battleID == 2){ //BattleHeal routine
						    uint32 SayCheck = MakeRandomInt(0,100);
					...
					case SpellType_Nuke: {
							...
							&& (tar->CanBuffStack(AIspells[i].spellid, botLevel, true) >= 0))
						{
							...
							if (botClass == ENCHANTER) {
							     // if (!IsBotRaiding()){
								uint32 randsay = MakeRandomInt(0,100);
								int Id = (AIspells[i].spellid);
			...
			if((iterator.GetData()->IsTrackable())&& (!iterator.GetData()->IsBot()) && 
			  ...
			  (!iterator.GetData()->IsClient()) && (!iterator.GetData()->IsFindable())){
				...
				if (NamedOnly) {
				   ...
				   for(unsigned int Index = 0; Index < MyArraySize; Index++) {
...
void EntityList::BotHelpWindow(Client* client, int MobClass) {
	...
	WindowText += "</c><br> &nbsp; #bot auto mez on/off - Will automatically stop incoming undead adds to the melee.";
	WindowText += "</c><br> &nbsp; #bot lore - cast Identify on the item on your mouse pointer.";
	...
	WindowText += "</c><br> &nbsp; #bot charisma - Cast charisma buff on target";
	...
	WindowText += "</c><br> &nbsp; #bot lore - cast Identify on the item on your mouse pointer.";
	WindowText += "</c><br> &nbsp; #bot summon food/drink - Food or drink to target.";
	WindowText += "</c><br> &nbsp; #bot magepic - Summons Magician epic orb.";
	}
	else if (MobClass == 13){ //SK
	WindowText += "</c><br> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; Additional ShadowKnight Commands";
	WindowText += "</c><br> &nbsp; #bot HarmTouch - Does serious damage to your target.";
	}
	else if (MobClass == 14){ //Paladin
	WindowText += "</c><br> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; Additional Paladin Commands";
	WindowText += "</c><br> &nbsp; #bot LayOnHands - Completely heals  your target.";
	}
      client->SendPopupToClient(WindowTitle, WindowText.c_str());
      return; 
}
...
//Charisma
bool NPC::Bot_Command_CharisTarget(int charistype, Mob *target) {
	if(target) {
		int charisid = 0;
		int charislevel = GetLevel();
	switch(charistype) {
	  
		case 1: // Shaman - best Charisma Spell
		if((charislevel >= 59) && (charislevel <= 75)) {
			charisid = 1589; //Unfailing Reverence
		}
		else if((charislevel >= 58) && (charislevel <= 58 )) {
			charisid = 1582; //Talisman of the Serpent
		}
		else if((charislevel >= 47) && (charislevel <= 57 )) {
			charisid = 156; //Charisma
		}
		else if((charislevel >= 37) && (charislevel <= 46 )) {
			charisid = 155; //Glamour
		}
		else if((charislevel >= 28) && (charislevel <= 36 )) {
			charisid = 150; //Alluring Aura
		}
		else if((charislevel >= 10) && (charislevel <= 27)) {
			charisid = 284; //Spirit of Snake
		}
			break;

		case 2: // Enchanter - next best Charisma Spell
		if((charislevel >= 56) && (charislevel <= 75)) {
			charisid = 1701; //Overwhelming Splendor
		}  
		else if((charislevel >= 46) && (charislevel <= 55)) {
			charisid = 647; //Adorning Grace
		}
		else if((charislevel >= 43) && (charislevel <= 45)) {
			charisid = 4099; //Bounce
		}
		else if((charislevel >= 31) && (charislevel <= 42 )) {
			charisid = 646; // Radiant Visage
		}
		else if((charislevel >= 18) && (charislevel <= 30)) {
			charisid = 489; // Sympathetic Aura
		}
			break;
		}
		if(charisid > 0) {
			/*int32 DontslowMeBeforeTime = 0;
			CastSpell(charisid, target->GetID(), 1, -1, -1, &DontslowMeBeforeTime);
			target->SetDontslowMeBefore(DontslowMeBeforeTime); */
			CastSpell(charisid, target->GetID(), 1, -1, -1);
			return true;
		} 
	}
	return false;
...
void Mob::ArmorColorUpadate(NPC* bot, std::string* errorMessage) {
	GetBotItems(bot, errorMessage);
	...
	for(int i=0; i<=21; ++i) {
		...
		if(inst) {
			//item = inst->GetItem();
			bot->SendBotWearChange(materialFromSlot); //Activates bot color
			if(!errorMessage->empty())
				return;
		}
	}
...
Mob* Mob::BotGetMobToMez(int32 chanter, int32 bard, int32 necro) {
	...
	if (aggro_list.size() > 1) {
		float hpRatio = 0.0;
		Mob* ClientTarget = CastToClient()->GetTarget();
		...
		for (it = aggro_list.begin(); it != aggro_list.end(); it++) {
			...
			// Skip this mob if client has it targeted
			if (curMob == ClientTarget){
				//BotSay("Ignored %s", GetTarget()->GetCleanName());
				continue;
			}
			
			//Necro mezzers can only mez undead
			if ((curMob->GetBodyType() != 3) && (GetClass()==NECROMANCER)){
				continue;
			}
...
void Mob::SetSpellState(uint32 CharID){
      if (database.BotSnareLookup(CharID) > 1)
        this->SetBotSnare(false);
      else 
	this->SetBotSnare(true);
      
      if (database.BotSelosLookup(CharID) > 1)
        this->SetBotSelos(false);
      else 
	this->SetBotSelos(true);
      
      if (database.BotRootLookup(CharID) > 1)
        this->SetBotRoot(false);
      else 
	this->SetBotRoot(true);
      
      if (database.BotLock1Lookup(CharID) > 1)
        SetBotLock1(false);
      else 
	this->SetBotLock1(true);
      
      if (database.BotSpamLookup(CharID) > 1)
        this->SetBotChat(false);
      else 
	this->SetBotChat(true);
/*
      if (database.BotClientModeLookup(CharID) > 1)
        this->SetbotNpcMode(false);
      else 
	this->SetbotNpcMode(true);*/

      this->SetBotTashLevel(database.BotTashrLookup(CharID));
      this->SetBotTauntLevel(database.BotTauntLookup(CharID));
