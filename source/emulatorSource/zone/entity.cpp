...
void EntityList::RezBotMember(std::list<int32>::iterator iter, int group_id, int group_member_id, BotRaids *br, Group *g, Client *c) {
	...
	while(iterator.MoreElements())
	{
		if (iterator.GetData() && ((MobDistance = iterator.GetData()->DistNoZ(*client))<=distance))
		{
			if((iterator.GetData() != client) && iterator.GetData()->IsTrackable() 
			  ...
			  (!((iterator.GetData()->IsClient())||(iterator.GetData()->IsBot())||
			  (iterator.GetData()->IsFamiliar())||(iterator.GetData()->IsPet())))
			  || //OR, if it's these but too close to you...
			  (((iterator.GetData()->IsClient())||(iterator.GetData()->IsBot())||
			  (iterator.GetData()->IsFamiliar())||(iterator.GetData()->IsPet()))
			  &&(iterator.GetData()->DistNoZ(*client)>=200)))
#endif	
