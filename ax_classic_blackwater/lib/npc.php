...
//  if ($npc_faction_id != $_POST['npc_faction_id']) $fields .= "npc_faction_id=\"" . $_POST['npc_faction_id'] . "\", ";
  ...
  if ($isbot != $_POST['isbot']) $fields .= "isbot=\"0" . $_POST['isbot'] . "\", ";
  ...
  if ($private_corpse != $_POST['private_corpse']) $fields .= "private_corpse=\"0" . $_POST['private_corpse'] . "\", ";
  if ($unique_spawn_by_name != $_POST['unique_spawn_by_name']) $fields .= "unique_spawn_by_name=\"0" . $_POST['unique_spawn_by_name'] . "\", "; 
  $fields =  rtrim($fields, ", ");
...
function suggest_npcid() {
  ...
  $zidstart = $result['zoneidnumber'] . "001";

  $query = "SELECT MAX(id) as id FROM npc_types WHERE id like \"$zid\"";
  $result = $mysql->query_assoc($query);

  return (($result['id'] == 0) ? $zidstart : $result['id'] + 1);
}
