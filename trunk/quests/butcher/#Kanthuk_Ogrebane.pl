...
sub EVENT_SAY {
  ...
  # Only responds with text if hand-in was completed or if $client already has quest item.
    ...
    if($text=~/amstaf/i) {
      ...
      quest::say("I'll never forget what he did for me that day. He gave me the courage to continue. No one's ever done anything like that for me. I vowed that day that I'd follow him into any battle. I'd be there along side him, no matter what. As time went on we came across an elven woman named Ambreen.");
    }
    elsif($text=~/ambreen/i) {
      quest::say("Ambreen was younger than he and I. She knew of the powers of nature. She was able to control the rain which fell from the heavens and would call upon the spirits to aide her. It was obvious that Amstaf took a liking to Ambreen. She ended up coming with us on crusades. The last I saw of Ambreen was when we went to the snow covered lands of Everfrost. We were wandering the lands when a freak snowstorm hit us. We were seperated from one another for quite some time.");
    }
    elsif($text=~/snowstorm/i) {
