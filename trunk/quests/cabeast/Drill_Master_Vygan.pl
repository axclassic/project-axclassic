...
my $geozite = quest::saylink("geozite", 1);
my $want = quest::saylink("want", 1);
my $legionnaire = quest::saylink("legionnaire", 1);
my $welfare = quest::saylink("welfare", 1);
if($text=~/Hail/i){
...
my $issue = quest::saylink("issue", 1);
 ...
 if (plugin::check_handin(\%itemcount, 48006 =>1 )) {
   quest::say("Hmm. . . even if the writing wasn't as smudged as it is, I still would not be able to decipher the hidden message. You must now take this and find someone with the ability to read it. I hear there is such a person on patrol with that $calling but I cannot recall where.");
