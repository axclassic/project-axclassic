sub EVENT_SAY {
my $whiff = quest::saylink("whiff", 1);
my $second = quest::saylink("second", 1);
my $cleansing = quest::saylink("cleansing", 1);
my $Gandan = quest::saylink("Gandan", 1);
  if($text=~/hail/i){
    quest::say("Hmmph!! Expect to be a great master such as I? Maybe. You still appear to be a $whiff.");
  }
  elsif($text=~/whiff/i){
    quest::say("A whiff is the sound of a tail striking air. A whiff isss you. Inexperienced. I can help you, if you want to earn the $second shackle.");
  }
  elsif($text=~/second/i){
    quest::say("Then you shall perform the first rite. Take this pack. To the Outlandsss you will go. You will fill and combine the pack with one of each of the following: goblin scout beads, a large scorpion pincer, a froglok skipper skipping stone, and a sabertooth cub canine. Bring these to me and you shall prove yourself a monk.");
...
sub EVENT_ITEM {
my $stone = quest::saylink("stone", 1);
 if(plugin::check_handin(\%itemcount, 12427 => 1)){
  ...
  quest::say("Thisss is all that was ever found of Gandan Tailfist. I wish you luck in your quest. May you fare better than he did."); #Unable to find the true text; made this up
  ...
  quest::say("I don't need thisss, Take it back.");
 }
}
