...
sub EVENT_SAY {
my $suffering = quest::saylink("suffering", 1);
my $sent = quest::saylink("sent", 1);
my $accept = quest::saylink("accept", 1);
 if($text=~/hail/i){
 ...
 elsif($text=~/accept/i){ #Test of Suffering (Greenmist Quest 3/8)
...
sub EVENT_ITEM {
  ...
  quest::say("Excellent work! You are no squire. You are now a knight of the crusaders. Your next step shall be zealot. And every zealot wields a magical khukri. Speak with Lord Qyzar for further instruction.");
