...
my $ting = quest::saylink("ting", 1);
 ...
 if($text=~/ting/i){
  quest::say("You say you filled with da rage? Me have da rage too. My rage start when my luv, Hooly Bubblegut got killed by nasty brigands in da Rathe Mountains! Me want revenge!!!");
  quest::emote("'s face turns bright red.");
  quest::say("You go to doze mountains and kill bandits. Me want four bandit sashes. You bring me dem, and me give you reward.");
 }
}

sub EVENT_ITEM {
 # Handin: 4 Bandit sashes
 if(plugin::check_handin(\%itemcount, 12100 => 4)){
