...
######################################
sub EVENT_SAY { 
my $initiate = quest::saylink("initiate", 1);
my $willing = quest::saylink("willing", 1);

  if($text=~/hail/i) {
    $client->Message(14,"Hail, $name! I sense the gift of hatred in your aura. What status do you hold within these Spires? Are you an $initiate ?");
  }
  if(plugin::check_hasitem($client, 1369)) { #Initiate Symbol of Innoruuk
    if($text=~/initiate/i) {
      $client->Message(14,"Well met, Initiate $name. If you desire to raise your station in this temple, I have a task for you. Are you $willing to accept the task I am about to set before you?");
    }
    if($text=~/willing/i) {
      $client->Message(14,"As an initiate of the Spires, you have proven your hatred of the halflings who are desecrating our forest with their feeble shrines and camps. They persist in their hopeless task and seem to anticipate many of our tactics against them. Go out into the Nektulos and slay the one called Master Whoopal. Take his head to Draxiz N'Ryt so that he may probe the brain.");
    }
  }
  if(plugin::check_hasitem($client, 1370)) { #Disciple Symbol of Innoruuk
    if($text=~/disciple/i) {
      $client->Message(14,"I have an important task for a disciple of Hate such as yourself. It is no secret that King Naythox Thex wishes to expand the Teir'Dal Empire to the continent of Faydwer. We have a camp established in the Lesser Faydark near the stronghold of the vampire lord, Mayong Mistmoore. The camp is constantly under siege by the Fier'Dal and Koada'Dal and must rely on the support of Neriak for supplies necessary to survival. Are you willing to serve your King and Empire by delivering the provisions?");
    }
    if($text=~/deliver the provisions/i) {
      $client->Message(14,"The provisions are crated and prepared for shipment by Dran Slug Rembor. Go to his establishment in the Foreign Quarter and tell him Ithvol sent you. Make haste, young disciple of Hate! The fate of the Empire hangs upon our support of the Faydark Camp!");
    }
  }
}
