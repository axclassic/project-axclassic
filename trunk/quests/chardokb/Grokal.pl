...
my $box = quest::saylink("box",1);
 ...
 $client->Message(14,"Hello $class , I have been here a long time. I know many $secrets about this place and wish to share.");
	}
	if ($text =~ /secrets/i){
	...
	if ($text =~ /box/i){
	$client->Message(14,"Find the one named Nox. He is filled with Hate but he do enjoy a good melody. He might be willing for alittle payment if you ask him about the box.");
	}
	else{
	quest::say();
	}
