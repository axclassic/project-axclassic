...
sub EVENT_ITEM {
    ...
    if (plugin::check_handin(\%itemecount,13231=>1) && plugin::check_handin(\%itemcount,13232=>1)) {
...
sub EVENT_SPAWN {
	quest::settimer("repeat", 120); # First trigger after 120 seconds, and every 120 seconds after
}
