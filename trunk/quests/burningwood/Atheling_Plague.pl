...
sub EVENT_SAY {
my $obtain = quest::saylink("obtain", 1);
   if($text=~/hail/i) {
...
sub EVENT_ITEM
{
   ...
   if(plugin::check_handin(\%itemcount, 12764 => 1)) {
      quest::summonitem(12750);
	  quest::emote(' tosses the skull over the edge of the tower.');
      quest::say("Excellent. You show signs of a true Iksar slayer. Too, bad I have already given the skull of the Sister of Scale to another. Perhaps you would like to meet him before he departs. Say hello, Doval.");
      quest::unique_spawn(87161,0,0,$x + 5,$y,$z);
