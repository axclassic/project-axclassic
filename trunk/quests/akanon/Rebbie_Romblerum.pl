...
sub EVENT_SAY {
my $thread = quest::saylink("thread", 1);
...
my $Coif = quest::saylink("Coif", 1); 
  ...
  # the silk thread text Is made up ads i could Not find the real text reply.
  if ($text=~/thread/i) {
         quest::say ("Silk Thread is created by combining two spiderling silk in a sewing kit.");
  }
  if ($text=~/Boots/i) {
...
my $Coif = quest::saylink("Coif", 1); 
  if (plugin::check_handin(\%itemcount, 10988 => 1)) {
    quest::say("Hail $name! You must be one of Kaxon's new trainees. Kaxon has asked me to help get you outfitted in a suit of armor to protect you from the weapons of our foes. I have assembled a kit for you that will allow you to construct the armor pieces once you have gathered the necessary components. The required components vary according to which piece of Plague Rust Mail you are planning on assembling. Do you wish to craft, a Plague Rust $Bracer, Plague Rust $Gloves, Plague Rust $Boots, Plague Rust $Sleeves, Plague Rust $Leggings, Plague Rust $Coif, or a Plague Rust $Tunic.");
