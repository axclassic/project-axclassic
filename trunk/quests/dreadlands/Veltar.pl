...
my $virtues = quest::saylink("virtues", 1);
my $patience = quest::saylink("patience", 1);
my $agility = quest::saylink("agility", 1);
my $peace = quest::saylink("peace", 1);
if($text=~/Hail/i){
quest::emote('body has scars all about it, looking as though beaten with whips. One wrist bares a unique looking bracer, the other is bare.');
quest::say("Leave me be! I did not come up here because I wanted company. Or do you come to find information on the $Tynnonium shackle?");
}
if($text=~/Tynnonium/i){ 
