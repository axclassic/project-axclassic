...
sub EVENT_SAY {
  ...
  if ($text=~/started/i)  { #paladin test of spirit
...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 20943 => 1, 20869 => 1, 20868 => 1)) { #paladin test of spirit using silvery girdle, diaphanous globe, ivory sky diamond  
  quest::summonitem(2716);  #girdle of faith
     quest::ding(); 
     quest::exp(841790);
    ...
    quest::spawn2(71091,0,0,-579,767,175,63.5);
    quest::depop();
    }
   else {
