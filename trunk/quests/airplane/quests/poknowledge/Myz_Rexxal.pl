...
#hail
if ($text =~ /hail/i && $faction < 5){ # faction is above indifferent #
 $client->Message(14,"What's wrong $name , my sister is crazy. I am sorry she sent you to $betrayal.");
}
elsif ($text =~ /hail/i && $ulevel == 65 && $faction >= 5){ # 65 and indifferent #
...
if ($text =~ /hail/i && $ulevel == 64 && $faction >= 5){ #64 and indifferent #
