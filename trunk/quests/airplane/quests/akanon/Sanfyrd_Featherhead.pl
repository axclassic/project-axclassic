...
if ($text=~/Princess Joleena's wing/i) {
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 13198 => 4 )) { #Turn in 4 scrap metal
    ...
    quest::summonitem(quest::ChooseRandom(5013,5019,626)); #Various Items and a contraption, Kozyn Gigglephizz quest item.
   quest::ding();
   ...
   quest::faction(39,-3); 
	...
	if (plugin::check_handin(\%itemcount, 9426 => 1, 28618 => 1, 29906 => 1, 28165 => 1)) {#Bundle of Super Conductive Wires, Gold Tipped Boar Horn, Shard of Pure Energy, Silicorrosive Grease
  ...
  if(plugin::check_handin(\%itemcount, 13216 => 1, 13217 => 1)) { #text not live-like. Need original
    quest::say("Wow! You took care of those rogue models pretty easily. I bet Drekon told you I'd have a reward, huh? Well it's enough I don't have to worry about those models running around anymore. Here ya go!");
