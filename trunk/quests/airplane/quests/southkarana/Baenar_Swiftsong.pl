...
sub EVENT_SAY { 
  ...
  if ($text=~/doll/i) {
    ...
    quest::summonitem(20372); #Solusek Mining Company Invoice
  }
}

sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 20373 => 1)) { #Letter of Introduction
  ...
  elsif (plugin::check_handin(\%itemcount, 20371 => 1)) { #Note for Baenar
    ...
    quest::summonitem(20368); #Note to Maligar
  }
  if (plugin::check_handin(\%itemcount, 20367 => 1)) { #Maligar's Head
    ...
    quest::summonitem(20366); #Mahlin's Mystical Bongos
  }
  else {
