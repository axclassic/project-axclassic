...
sub EVENT_SAY {
  if ($text=~/hail/i) {
    quest::say("Greetings, faithful one. Your faith shall be tested this day.  Shall you take the test of courage, skill, or protection?");
    }
...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 20933 => 1, 20807 => 1, 20806 => 1)) { #cleric test of courage using ochre tessera, silver hoop, sky emerald
    ...
    quest::summonitem(27716); #aegis of the wind
     quest::ding();
     quest::exp(841790);
    quest::say("Wonderful! Take this Aegis of the Wind as your reward!");
    quest::depop();
    }
