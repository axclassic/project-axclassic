...
sub EVENT_TIMER 
{
	if($timer == 30) {
    		...
    		if (!defined($qglobals{dust_event_start}) && !defined($qglobals{dust_done}) && !$entity_list->GetMobByNpcTypeID(215043) && !$entity_list->GetMobByNpcTypeID(215485) && !$entity_list->GetMobByNpcTypeID(215045) && !$entity_list->GetMobByNpcTypeID(215060)) 
    		{
	...
	if ($signal == 1 && defined($qglobals{dust_event_start}) && !defined($qglobals{dust_done})) 
	{
		...
		quest::depop(215046);
     	...
     	elsif ($signal == 5 && !defined($qglobals{dust_event_start}) && !defined($qglobals{dust_done})) 
     	{
     		...
     		if(!$entity_list->GetMobByNpcTypeID(215043) && !$entity_list->GetMobByNpcTypeID(215485) && !$entity_list->GetMobByNpcTypeID(215045) && !$entity_list->GetMobByNpcTypeID(215060))
     		{
			quest::spawn2(215460,0,0,-356.8,591.3,436.3,209.4); 
			quest::spawn2(215460,0,0,-282.1,663.4,442.4,229.8); 
			quest::spawn2(215460,0,0,-168.5,782.1,434.2,170.2); 
