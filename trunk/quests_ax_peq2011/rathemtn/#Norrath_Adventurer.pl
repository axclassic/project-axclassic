...
my $rebellion = quest::saylink("Rebellion", 1);
my $send = quest::saylink("send", 1);
my $gloomingdeep = quest::saylink("Mines of Gloomingdeep", 1);
  if($text=~/Hail/i){
    ...
    if ($ulevel<=1){
    $client->Message(14,"I also have news of a $rebellion.");}
  }
  if(($text=~/rebellion/i) && ($ulevel<=1)){
    $client->Message(14,"The inhabitants of Gloomingdeep have started a rebellion, if you wish to help the revolt, I can $send you there.");
  }
  if(($text=~/Mines of Gloomingdeep/i) && ($ulevel<=1)){
    $client->Message(14,"Good luck with the revolt!.");
    quest::rebind(189,-255.7,33.2,15.3);
    quest::movepc(189,-255.7,33.2,15.3);
  }
  if(($text=~/send/i) && ($ulevel<=1)){
    $client->Message(14,"You wish to be ported over to the $gloomingdeep? Once i do this, there is no turning back.");
  }
  if($text=~/travels/i){
