...
int command_init(void) {
	...
	(
		...
		command_add("setkarma","[karma amount] - Sets targeted Clients Karma in the database",240,command_setkarma) ||
		...
		command_add("castspell"," Cast a spell",50,command_castspell) ||
		command_add("HarmTouch","Underfoot Ability",50,command_harmtouch) ||
		command_add("LayOnHands","Underfoot Ability",50,command_layhands) ||
		...
		//command_add("botmode", "- change bots from NPC to Client mode or back.", 0, command_botmode) ||
#endif //EQBOTS
...
void command_setkarma(Client *c, const Seperator *sep)
{
	if((sep->arg[1][0] == 0 || strcasecmp(sep->arg[1],"*")==0) || ((c->GetTarget()==0) || (!c->GetTarget()->IsClient())))
		c->Message(0, "Usage: #setkarma [karma amount]");
	else
	{
		char errbuf[MYSQL_ERRMSG_SIZE];
		char *query = 0;
		c->Message(15,"Setting Client %u to Karma %i",c->GetTarget()->CastToClient()->AccountID(),atoi(sep->argplus[1]));
		database.RunQuery(query, MakeAnyLenString(&query, "update account set karma=%i where id=%i",atoi(sep->argplus[1]),c->GetTarget()->CastToClient()->AccountID()), errbuf);
		c->LogSQL(query);
		safe_delete_array(query);
	}
}
  ...
  Group *g = c->GetGroup();
	...
	for(int i=0; i<MAX_GROUP_MEMBERS; i++){
	  if(g->members[i] && g->members[i]->IsBot()) {
	    hasBots = true;}
	    c->Message(15, "You have bots in your group, remove them first!");
	    c->Message(15, "WARNING! Be sure there are no other players with bots in the zone before you initiate this command.");
	    return;
	}
    }
	char errbuf[MYSQL_ERRMSG_SIZE];
	...
	MYSQL_RES *result;
	MYSQL_ROW row;
	if(database.RunQuery(query, MakeAnyLenString(&query, "SELECT count(*) FROM botleader WHERE zoneid = %i",zone->GetZoneID()), errbuf, &result)) {
	 row = mysql_fetch_row(result);
	  if(atoi(row[0]) != 0) {
	    c->Message(15, "There are other players with Bots in this Zone!");
	    c->Message(15, "This command is not allowed with Bots in zone.");
	...
	Group *g = c->GetGroup();
	  if(g) {
		...
		if(g->members[i] && g->members[i]->IsBot()) {
		  hasBots = true;}
		  c->Message(15, "You have bots in your group, remove them first!");
		  c->Message(15, "WARNING! Be sure there are no other players online before you initiate this command.");
		  return;
	      }
	  }
	      char errbuf[MYSQL_ERRMSG_SIZE];
	      ...
	      MYSQL_RES *result;
	      MYSQL_ROW row;
	      if(database.RunQuery(query, MakeAnyLenString(&query, "SELECT count(*) FROM botleader"), errbuf, &result)) {
	      row = mysql_fetch_row(result);
		if(atoi(row[0]) != 0) {
		  c->Message(15, "There are other players online!");
		  c->Message(15, "This command is not allowed while players are online.");
		  mysql_free_result(result);
		  safe_delete(query);
		  return;
	    }
...
void command_harmtouch(Client *c, const Seperator *sep)
{
	...
	if (c->GetClientVersion() <= EQClientSoF){
	   c->Message(15, "This skill works with post SoD clients");
	}
	else if (c->GetClass() != SHADOWKNIGHT) {
	  c->Message(15, "This is a ShadowKnight ability");
	}
	else if ((c->GetTarget() == NULL) || c->GetTarget()->IsBot() ||  !c->GetTarget()->IsNPC()){
...
void command_layhands(Client *c, const Seperator *sep)
{
	...
	else if (c->GetClass() != PALADIN) {
	  c->Message(15, "This is a Paladin ability");
	}
	/*else if ((c->GetTarget() == NULL) || c->GetTarget()->IsBot() ||  !c->GetTarget()->IsNPC()){
            c->Message(15, "Invallid target");
	}*/
	else if (!c->GetPTimers().Expired(&database, pTimerLayHands) && !c->GetGM()) {
	    c->Message(13, "Ability recovery time not yet met - %d minutes %d seconds left.", ((remain)/60), (remain%60));
	    c->InterruptSpell();
	}
	else{
