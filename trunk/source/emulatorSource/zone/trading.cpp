	...
	for(int y=0; y < 8; y++) {
		...
		if (item2) {
			...
			if((GetGM() && !with->IsBot()) || ((item2->NoDrop != 0) && !with->IsBot()))
			    ...
			    BotRaids *br = entity_list.GetBotRaidByMob(CastToMob());
			    if(br) {
				if(br->GetBotRaidAggro()) {
				   with->BotSay("I can't while raid is engaged");
				}
			    }
			}
			// franck-add: you can give nodrop items to bots
			else if(with->IsBot() && botCanWear[y] && !with->IsEngaged()) {
				with->BotSay("Thank you for the %s, %s.", item2->Name,  this->GetName());
			}
			else if(with->IsBot() && with->IsEngaged()) {
				with->BotSay("I can't while engaged");
			}
