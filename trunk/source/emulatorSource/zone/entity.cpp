...
void EntityList::RezBotMemeber(std::list<int32>::iterator iter, int group_id, int group_member_id, BotRaids *br, Group *g, Client *c) {
	const NPCType* tmp = database.GetNPCType(*iter);
	...
	c->SetTarget(npc);
	Mob *rezzed_bot = c->GetTarget();
	rezzed_bot->SetFollowID(c->GetID());
	rezzed_bot->BotOwner = c->CastToMob();
	rezzed_bot->SetOwnerID(0);
	uint32 itemID = 0;
	const Item_Struct* item2 = NULL;
	std::string errorMessage = "";
	rezzed_bot->EquipBot(rezzed_bot->CastToNPC(), &errorMessage);
	if(!errorMessage.empty()) {
		rezzed_bot->CastToMob()->Say("There was an error loading my equipment");
	}
	rezzed_bot->CalcBotStats(false);
	bool same_group = false;
	Group *rbg = 0;
	if(group_member_id == 0) {
		rbg = new Group(rezzed_bot);
		rezzed_bot->SetBotRaidID(br->GetBotRaidID());
		rezzed_bot->SetBotRaiding(true);
		++iter;
		int32 groupLeaderID = br->BotRaidGroups[group_id]->members[0]->GetNPCTypeID();
		for(int j=0; j<4; ++j) {
			if(groupLeaderID != *iter) {
				++iter;
			}
			else {
				same_group = true;
		...
		if(same_group) {
			for(int n=0; n<5; ++n) {
				if(br->BotRaidGroups[group_id]->members[n]) {
					rbg->AddMember(br->BotRaidGroups[group_id]->members[n]);
				}
			}
		}
		c->Message(5, "%s tells you, \'I am rezzed into my group\'", rezzed_bot->GetCleanName());
		rbg->SetID(br->BotRaidGroups[group_id]->GetID());
		rbg->disbandcheck = false;
		if(same_group) {
			*br->BotRaidGroups[group_id] = *rbg;
		}
		else {
			br->RemoveEmptyBotGroup();
			rbg->SetID(br->BotRaidGroups[group_id]->GetID()-1);
			br->InsertBotGroup(rbg);
		}
	}
	else {
		if(br) {
			for(int g=1; g<=group_member_id; ++g) {
				iter--;
			}
			int32 groupLeaderID = br->BotRaidGroups[group_id]->members[0]->GetNPCTypeID();
			for(int j=0; j<4; ++j) {
			...
			if(same_group) {
				rbg = new Group(br->BotRaidGroups[group_id]->members[0]);
				bool regrouped = false;
				for(int k=1; k<=5; ++k) {
					if(k == group_member_id) {
						rbg->AddMember(rezzed_bot);
						rezzed_bot->SetBotRaidID(br->GetBotRaidID());
						rezzed_bot->SetBotRaiding(true);
						regrouped = true;
					}
						...
						if(regrouped) {
							if(br->BotRaidGroups[group_id]->members[k-1])
								rbg->AddMember(br->BotRaidGroups[group_id]->members[k-1]);
						}
						else {
							if(br->BotRaidGroups[group_id]->members[k])
								rbg->AddMember(br->BotRaidGroups[group_id]->members[k]);
						}
					}
				}
				c->Message(5, "%s tells you, \'I am rezzed into %s\'s group\'", rezzed_bot->GetCleanName(), br->BotRaidGroups[group_id]->members[0]->GetCleanName());
				rbg->SetID(br->BotRaidGroups[group_id]->GetID());
				rbg->disbandcheck = false;
				*br->BotRaidGroups[group_id] = *rbg;
			}
			else {
				c->Message(15, "Battlerez failed. Try again.");
			}
		}
		else if(g) {
			g->AddMember(rezzed_bot);
			c->spawned_bots.remove(*iter);
			c->spawned_bots.push_back(rezzed_bot->GetNPCTypeID());
		}
	}
}
...
void EntityList::RemoveAllNPCs(){
	...
	while(iterator.MoreElements()) {
		...
		if(iterator.GetData()->IsBot()) {
			int32 ThisBot = (iterator.GetData()->GetNPCTypeID());
			database.CleanBotLeaderEntries(ThisBot);
			database.DeleteBotTempEntry(ThisBot);			
			database.DeleteBotBuffs(ThisBot);
		}
...
char* EntityList::MakeNameUnique(char* name) {
	...
	for (int i=0; i < 300; i++) {
		if (!used[i]) {
			...
			snprintf(name, 64, "%s%03d", temp_name, i);//Angelox
			#endif
			return name;
		}
	}
...
{
	// Get the client test out of the way early
	if (!client)
		return false;

	float maxrange = 0.0, distance = 0.0;

	if (client->GetClass() == DRUID)
		maxrange = (client->GetSkill(TRACKING) * 10.0);
	else if (client->GetClass() == RANGER)
		maxrange = (client->GetSkill(TRACKING) * 12.0);
	else if (client->GetClass() == BARD)
		maxrange = (client->GetSkill(TRACKING) * 7.0);
	...
	while(iterator.MoreElements())
	{
		// Prevent overflow of the track_array buffer
		if (array_counter >= spe)
			break;

		// Get the pointer to the mob once
		Mob* cur_entity = iterator.GetData();

		// Check low-cost conditions first
		if ((cur_entity) && (cur_entity->IsTrackable()) 
#ifdef EQBOTS	    //Angelox: The idea here is to unclutter the tracking window of bots and their pets
		    && //If it's not any of these...
		    (!((cur_entity->IsClient())||(cur_entity->IsBot())||
		    (cur_entity->IsFamiliar())||(cur_entity->IsPet())))
		    || //OR, if it's these but away from yours...
		    (((cur_entity->IsClient())||(cur_entity->IsBot())||
		    (cur_entity->IsFamiliar())||(cur_entity->IsPet()))
		    &&(cur_entity->DistNoZ(*client)>=200)))
#endif	
		{
			// Check distance from tracking client to mob
			distance = cur_entity->DistNoZ(*client);
			if (distance <= maxrange) {
				memset(track_ent, 0, sizeof(Track_Struct));
				track_ent->entityid = cur_entity->GetID();
				track_ent->distance = distance;
				track_ent->level = cur_entity->GetLevel();
				//track_ent->NPC = (!cur_entity->IsClient() && !cur_entity->IsBot());
				track_ent->GroupMember = (g && cur_entity->IsClient() && g->IsGroupMember(cur_entity->GetName())) ? 1 : 0;
	...
	// We'll always send the tracking packet, it just may not have all of the mobs
	// That will only happen if more than 50 spawn between calculating spe and iteration
