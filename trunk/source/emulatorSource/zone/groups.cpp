...
bool Group::AddMember(Mob* newmember)
{
	...
	if (newmember->IsBot()){
	  int32 BotOwnerID = database.GetBotOwner(newmember->GetNPCTypeID());
	  hasGroupid = database.CountClients(BotOwnerID);
	  //groupid = database.CountClients(newmember->GetOwner()->CharacterID());
	}
	else{
	  hasGroupid = database.CountClients(newmember->CastToClient()->CharacterID());
	}

	if (hasGroupid){
	      //hasNoClients = false;
	      GetLeader()->SetHasClient(true);
	      LogFile->write(EQEMuLog::Debug, "Group::AddMember found another client in group.");
	}

	if((newmember->IsBot()) && (!hasGroupid)) {
		int i;
		...
		safe_delete(outapp);
		//LogFile->write(EQEMuLog::Debug, "Group::AddMember found no additional clients and did that dance.");
		return true;
	}
	else
	{
		...
		safe_delete(outapp);
		LogFile->write(EQEMuLog::Debug, "Group::AddMember jumped to client-only section (If bots, then Bot/client combos too).");
		return true;
#ifdef EQBOTS
	}
#endif
...
//#ifdef EQBOTS
		   //int32 GroupID = database.GetBotGroupID(members[i]->CastToClient()->CharacterID());
//		   if /*(*/(members[i]->CastToClient()->IsBotRaiding())/* && (GroupID > 0))*/
//		     continue;
//#endif
...
bool Group::DelMember(Mob* oldmember,bool ignoresender){
...
#ifdef EQBOTS //marko
	Group *grp = oldmember->GetGroup();
	if (grp){
	  if(oldmember->IsClient() && (oldmember == GetLeader())) {
	      grp->DisbandGroup(false);
	  }
	  /*else if (oldmember-> IsBot()){ 
	      LogFile->write(EQEMuLog::Debug, "Group::DelMember() Bot grouped set zoning.");
	      grp->MemberZoned(oldmember);
	  }*/
	}
#endif //EQBOTS
	...
	for (uint32 i = 0; i < MAX_GROUP_MEMBERS; i++) {
		if (members[i] == NULL) {
			//LogFile->write(EQEMuLog::Debug, "Group::DelMember() null member at slot %i", i);
			continue;
		}
		if (members[i] != oldmember) {
	...
	safe_delete(outapp);
	 ...
	 if(oldmember->IsClient())
	 {
		...
		Group *g = entity_list.GetGroupByMob(oldmember);//GetGroup()->MemberZoned(this);
		//Angelox: Bot should be removed from group by now - reset AI timer to something short
		if (oldmember->IsBot()){
		  if (oldmember->IsBotRaiding()){
		    oldmember->BotKill();
		  }
		  else if (oldmember->BotIsZoning()){ //Amd hopefully not grouped
		    //g->MemberZoned(oldmember);
		    oldmember->check_grouped_timer.Start(10, false);
		    //LogFile->write(EQEMuLog::Debug, "Group::DelMember() BotIsZoning2");
		  }
		  else if (!g){
		     oldmember->check_grouped_timer.Start(10, false); 
		     //LogFile->write(EQEMuLog::Debug, "Group::DelMember() Bot Not Grouped");
		  }
		  else{
		    LogFile->write(EQEMuLog::Debug, "Group::DelMember() Nothing Happened");
		  }
		}
#endif //EQBOTS
...
void Group::DisbandGroup(bool IsZoning) {
...
#ifdef EQBOTS
	    if(Leader->IsBotRaiding()){
	  
		  BotRaids *brd = entity_list.GetBotRaidByMob(Leader->CastToMob());
		  if(brd) {
			brd->botmaintank = NULL;
			brd->botsecondtank = NULL;
		        brd->botraidmaintarget = NULL;
		        brd->botraidsecondtarget = NULL;
		        brd->botgroup1target = NULL;
		        brd->botgroup2target = NULL;
		        brd->botgroup3target = NULL;
		        brd->botgroup4target = NULL;
		        brd->botgroup5target = NULL;
		        brd->botgroup6target = NULL;
		        brd->botgroup7target = NULL;
		        brd->botgroup8target = NULL;
		        brd->botgroup9target = NULL;
		        brd->botgroup10target = NULL;
		        brd->botgroup11target = NULL;
		        brd->botgroup12target = NULL;
			brd->RemoveRaidBots();
			brd->DisbandBotRaid();
			brd = NULL;
		  ...
		  int combatID = database.BotCanCombatLookUp(zoneID);
			if (combatID == 0){
			  Leader->GetPTimers().Start(pTimerSpawnGroupReuse, 10);
			  Leader->GetPTimers().Start(pTimerSpawnRaidReuse, 10);
			  Leader->GetPTimers().Start(pTimerSpawnBotReuse, 10);
			}
			else{
			  Leader->GetPTimers().Start(pTimerSpawnGroupReuse, 60);
			  Leader->GetPTimers().Start(pTimerSpawnRaidReuse, 80);
			  Leader->GetPTimers().Start(pTimerSpawnBotReuse, 30);
			}
	      ...
	      if (members[i]->IsBot()) {
		   if(IsZoning){
		      members[i]->check_grouped_timer.Start(10, false);
		   }
		   else{
		      members[i]->check_grouped_timer.Start(MakeRandomInt(100, 200), false);
		   }
		      
		   
		   safe_delete(outapp);
		}
	    //Leader->SetHasClient(false);
	  }
#endif //EQBOTS
		...
		if(g) {
			...
			for(int i=5; i>=0; i--) {
				if(g->members[i] && g->members[i]->IsBot()) {
					hasBots = true;
					//g->members[i]->check_grouped_timer.Start(10, false);
				}
			}
			if(hasBots) {
				hasBots = false;
