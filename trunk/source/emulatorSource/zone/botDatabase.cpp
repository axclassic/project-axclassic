...
//Not used
void Database::SetBotColorNull(int32 npctypeid, int32 botslotid) {
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	int32 affected_rows = 0;

	if(!RunQuery(query, MakeAnyLenString(&query, "UPDATE botinventory SET color=0 WHERE npctypeid = %u AND botslotid = %u", npctypeid, botslotid), errbuf, 0, &affected_rows)) {
		cerr << "Error in SetBotColorNull '" << query << "' " << errbuf << endl;
	}
...
bool Database::DeleteBot(int32 mobid) {
	...
	if(!RunQuery(query, MakeAnyLenString(&query, "DELETE FROM bottemp WHERE botid=%i", mobid), errbuf)) {
		success = false;
	}
	if(!RunQuery(query, MakeAnyLenString(&query, "DELETE FROM botbank WHERE botid=%i", mobid), errbuf)) {
		success = false;
	}
	if(!RunQuery(query, MakeAnyLenString(&query, "DELETE FROM botgroups WHERE botid=%i", mobid), errbuf)) {
		success = false;
	}
	safe_delete_array(query);
	return success;
...
void Database::SaveBotGroups(int32 groupid, int32 charid, int32 botid, int16 slot, int32 hp, int32 mana, int tableA, int tableB, int tableC) {
      if (tableA == 1){
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	int32 affected_rows = 0;
	...
	if(!RunQuery(query2, MakeAnyLenString(&query2, "INSERT into botgroups_b (groupid, charid, botid, slot, hp, mana) values (%i, %i, %i, %i, %i, %i)", groupid, charid, botid, slot, hp, mana), errbuf2, 0, &affected_rows2)) {
	...
	if(!RunQuery(query3, MakeAnyLenString(&query3, "INSERT into botgroups_c (groupid, charid, botid, slot, hp, mana) values (%i, %i, %i, %i, %i, %i)", groupid, charid, botid, slot, hp, mana), errbuf3, 0, &affected_rows3)) {
...
void Database::CopyBotBankToBotTemp(int32 groupid, int32 charid) { 
	...
	if (groupid > 19){
	  if(!RunQuery(query, MakeAnyLenString(&query, "REPLACE INTO bottemp SELECT * FROM botbank WHERE groupid=%i AND charid=%i;",groupid, charid), errbuf, 0, &affected_rows)) {
		cerr << "Error in CopyBotBankToBotTemp query '" << query << "' " << errbuf << endl;
	  }
	}
	else{
	  if(!RunQuery(query, MakeAnyLenString(&query, "REPLACE INTO bottemp SELECT * FROM botgroups WHERE groupid=%i AND charid=%i;",groupid, charid), errbuf, 0, &affected_rows)) {
		cerr << "Error in CopyBotBankToBotTemp query '" << query << "' " << errbuf << endl;
	  }
	}
	safe_delete_array(query);
...
int Database::BotCanCombatLookUp(int32 zone_id) {
    char errbuf[MYSQL_ERRMSG_SIZE];
    char *query = 0;
    MYSQL_RES *result;
    MYSQL_ROW row;
	...
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT cancombat FROM zone WHERE zoneidnumber='%u'", zone_id), errbuf, &result))
	{
		if (mysql_num_rows(result) == 1) {
			row = mysql_fetch_row(result);
			combatID = atoi(row[0]);
	...
	{
		cerr << "Error in BotCanCombatLookUp query '" << query << "' " << errbuf << endl;
	}
	safe_delete_array(query);
	return combatID;
}
...
//Bot magepic
int Database::BotMagepicLookup(int32 id) {
	char errbuf[MYSQL_ERRMSG_SIZE];
	char* query = 0;
	MYSQL_RES* result;
	MYSQL_ROW row;
	...
	if(RunQuery(query, MakeAnyLenString(&query, "SELECT value FROM quest_globals WHERE name='mage_epic_1' and charid=%i", id), errbuf, &result)) {
		if(mysql_num_rows(result) == 1) {
			row = mysql_fetch_row(result);
			spellActive = atoi(row[0]);
		}
		...
		cerr << "Error in BotMagepicLookup query '" << query << "' " << errbuf << endl;
	}
	mysql_free_result(result);
    safe_delete_array(query);
    return spellActive;
...
//Armorcolor quest
int Database::BotColorLookup(int32 id) {
	char errbuf[MYSQL_ERRMSG_SIZE];
	char* query = 0;
	MYSQL_RES* result;
	MYSQL_ROW row;
	...
	if(RunQuery(query, MakeAnyLenString(&query, "SELECT value FROM quest_globals WHERE name='armorcolor' and charid=%i", id), errbuf, &result)) {
		if(mysql_num_rows(result) == 1) {
			row = mysql_fetch_row(result);
			colorStatus = atoi(row[0]);
		}
	}
	else {
		cerr << "Error in BotColorLookup query '" << query << "' " << errbuf << endl;
	}
	mysql_free_result(result);
    safe_delete_array(query);
    return colorStatus;
...
///Bot armorcolor is done - this might be used elsewhere so left it. 
//Get the bot armor color is in Mob.cpp (GetBotEquipmentColor())
