...
/*Angelox: Will break group while feigned
			if((IsBotRaiding()) || (isgrouped)) {
				database.CleanBotLeader(CharacterID());
				BotRaids *brd = entity_list.GetBotRaidByMob(CastToMob());
				if(brd) {
					brd->RemoveRaidBots();
					brd = NULL;
				}
				Group *g = entity_list.GetGroupByMob(CastToMob());
				if(g) {
					bool hasBots = false;
					for(int i=5; i>=0; i--) {
				...
				Message(15, "You've been disbanded while you are feigned.");
				SetBotRaidID(0);
				SetBotRaiding(false);
				database.CleanBotLeader(CharacterID());
			}
