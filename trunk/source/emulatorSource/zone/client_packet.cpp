...
void Client::Handle_OP_AutoAttack(const EQApplicationPacket *app)
{
	...
	if((database.GetKarma(AccountID()) < 10000) && (zone->IsClassic()) && (GetLevel() >= 50) && (GetTarget() != NULL)){   //Angelox2
	    if (GetTarget()->GetLevel() <= 49){
		Message(13,"CP: Trivial target in a classic zone.");
		return;
	 }
	}
	if (app->pBuffer[0] == 0) 
...
#ifdef EQBOTS			  
			 ((nt->IsBot()) || (nt->IsPet() && nt->GetOwner() && nt->GetOwner()->IsBot()) && 
			 (nt->CastToClient()->GetClientVersion() >= EQClientSoD)) ||
#endif
...
*/
	...
	Mob* tmp = entity_list.GetMob(aps->npcid);
		if (tmp == 0 || !tmp->IsNPC() || ((tmp->GetClass() != ADVENTUREMERCHANT) && (tmp->GetClass() != DISCORD_MERCHANT) && (tmp->GetClass() != NORRATHS_KEEPERS_MERCHANT) && (tmp->GetClass() != DARK_REIGN_MERCHANT)))
		return;
...
#ifdef EQBOTS
	//BotInspect
	if (tmob ->IsBot()){ //Angelox4
	Mob::ProcessBotInspectionRequest(tmob->CastToMob(), this);
	}
#endif //EQBOTS
...
void Client::Handle_OP_GroupDisband(const EQApplicationPacket *app)
{
	...
	if(!group->IsLeader(this)) {
		Message(15,"Only the group leader can do this.");
		return;
	}
	
	if((group->IsLeader(this) && (GetTarget() == 0 || GetTarget() == this)) || (group->GroupCount()<3)) {
		group->DisbandGroup(false);
	} else {
		Mob* memberToDisband = NULL;
		memberToDisband = GetTarget();

#ifdef EQBOTS //Angelox
	  int32 otherGroupid = database.GetGroupID(memberToDisband->GetName());
	  int32 myGroupid = database.GetBotGroupID(CharacterID());
          if((group) && (memberToDisband->IsPet())) {
             Message(15,"You can't disband a pet.");
             return;
	  }
	  else if(memberToDisband->IsBot()){
	      if (database.GetBotOwner(memberToDisband->GetNPCTypeID()) == CharacterID()){
		int32 groupID = database.GetBotGroupID(CharacterID());
		database.DeleteBotGroupEntry(groupID, memberToDisband->GetNPCTypeID());
		database.DeleteBotTempEntry(memberToDisband->GetNPCTypeID());
	      }
	      else{
		Message(15,"Disband your own bot!");
		return;
	      }
	  }
          else if(otherGroupid != myGroupid) {
             Message(15,"Not in your group!");
             return;
	  }
...
#ifdef EQBOTS
	if((tmp != 0) && tmp->IsBot())
	//Mob::ProcessBotInspectionRequest(tmp->CastToMob(), this);
	Message(13, "Inspecting Bot Packet worked.");
#endif //EQBOTS
...
void Client::Handle_OP_Petition(const EQApplicationPacket *app)
{
	...
	else
	{
		...
		{
		Message(0,"Petition sent - You have prior petition(s) in the queue, use /deletepetition to delete if not needed anymore...");
		//return;
		}
...
#ifdef EQBOTS
	int32 CharID = CharacterID();
	int32 groupID = database.GetBotGroupID(CharID);
	
	/*
	const int spawnedBots = database.SpawnedBotCount(CharacterID());
	Group* grp = GetGroup();
	if ((IsGrouped()) || (IsBotRaiding())){
	  grp->DisbandGroup(false);
	}
	
	if (spawnedBots >=1){ // spawned bots after a zin and none spawned yet means trouble.
	  database.CleanBotLeader(CharacterID());
	  database.DeleteFromBotTemp(CharacterID());
	}
	 */ 
	  
	Mob::SpawnBotTemp(groupID,this); //Angelox: SpawnGroup (SpawnBotTemp spawns already spawned bots)
	Mob::CalculateChatListMetrics(); //Angelox: Start random chat method (can only be called once)
	Mob::SetBotGuarding(false);
	Mob::SetHasClient(false); //Angelox: for checking for more clients in bot group
#endif //EQBOTS
...
void Client::Handle_OP_RaidCommand(const EQApplicationPacket *app)
{
	DumpPacket(app);
	 ...
	 Message(13, "You can only raid with bots - try your #bot raid commands.");
	 return;
/*	
	const int spawnedBots = database.SpawnedBotCount(CharacterID());
	if ((spawnedBots >=1) || (IsBotRaiding())){
	 Message(13, "You can't mix players with bots during raid.");
	 return;
	}
*/
#endif
	...
	case RaidCommandInvite: {
		...
		if(i){
			Group *g = i->GetGroup();
			if(g){
				if((g->IsLeader(i) == false) || (i->IsBotRaiding()) ||( i-> IsGrouped())) //EQBOTS
					Message(13, "You can only invite an ungrouped player with no bots to join your raid.");
				else{
