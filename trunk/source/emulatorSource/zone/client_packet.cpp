...
void Client::Handle_OP_AutoAttack(const EQApplicationPacket *app)
{
	...
	if((zone->IsClassic()) && (GetLevel() >= 50) && (GetTarget() != NULL)){   //Angelox2
		...
		if(g) {
			...
			for(int i=5; i>=0; i--) {
				if(g->members[i] && g->members[i]->IsBot()) {
					hasBots = true;
					g->members[i]->DeleteBotBuffs(clientmob->GetNPCTypeID());
...
void Client::Handle_OP_GroupDisband(const EQApplicationPacket *app)
{
	...
	if((group->IsLeader(this) && (GetTarget() == 0 || GetTarget() == this)) || (group->GroupCount()<3)) {
		...
		memberToDisband = GetTarget();
          ...
          /*else if((memberToDisband->IsBot()) && (group) && (memberToDisband->IsBotRaiding())) {
             Message(15,"Target-disband members while raiding not allowed. Don't target anything for raid disband");
             return;
	  }*/
          else if((memberToDisband->IsBot()) && (group) && (!group->IsGroupMember(memberToDisband))) {
             Message(15,"Target a bot in your group.");
             return;
	  }
...
void Client::CompleteConnect()
{
	...
	Mob::CalculateChatListMetrics(); //Start random chat method (can only be called once)
#endif //EQBOTS
