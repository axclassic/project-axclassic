...
void Doors::HandleClick(Client* sender, int8 trigger)
{
	...
	if((haskey2 != SLOT_INVALID) && (haskey2 > 0))
	{
		playerkey = keyneeded2;
	}
	
	else if((haskey != SLOT_INVALID) && (haskey > 0))
	{
		playerkey = keyneeded;
	}
	...
	{ 
		...
		{	// they have something they are trying to open it with
			...
			{	// key required and client is using the right key
				if ((!keepoffkeyring) && (playerkey > 1)) //save regular key to keyring
				{
				  sender->KeyRingAdd(playerkey);
				}
				
				else if (keyneeded2 == playerkey) //save master key to keyring
				{
				  sender->KeyRingAdd(playerkey);
				}
