...
bool LoginServer::Process() {
	...
	if (statusupdate_timer.Check()) {
		this->SendStatus();
//		InitLoginServer(); // Angelox: Throws error so I can time the process
	}
...
bool LoginServer::InitLoginServer() {
	if(Connected() == false) {
		if(ConnectReady()) {
			...
			_log(WORLD__LS_ERR, "Not connected but not ready to connect, this is bad: %s:%d",  LoginServerAddress,LoginServerPort);
		}
...
void LoginServer::SendStatus() {
	statusupdate_timer.Start(); //Angelox: Defaults to 15 secs 
	ServerPacket* pack = new ServerPacket;
	...
	//database.UpdateLSStatus(numzones, numplayers);	
}
