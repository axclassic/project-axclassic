...
bool Database::DBLoadDoors(int32 iDoorCount, int32 iMaxDoorID) {
	...
	if (RunQuery(query, strlen(query), errbuf, &result))
	{
		...
		if (row && row[0]) {
			...
			mysql_free_result(result);
			Door tmpDoor;
				MakeAnyLenString(&query, "SELECT id,doorid,zone,name,pos_x,pos_y,pos_z,heading,opentype,guild,lockpick,keyitem,keyitem2,triggerdoor,triggertype from doors");//WHERE zone='%s'", zone_name
				if (RunQuery(query, strlen(query), errbuf, &result))
				{
					safe_delete(query);
					while((row = mysql_fetch_row(result))) {
						...
						tmpDoor.keyitem2 = atoi(row[12]);
						tmpDoor.trigger_door = atoi(row[13]);
						tmpDoor.trigger_type = atoi(row[14]);
