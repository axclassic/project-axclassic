...
void DatabaseMySQL::UpdateLSPasswordInfo(unsigned int id, string password) //Angelox
{
	if(!db)
	{
		return;
...
bool DatabaseMySQL::GetFirstFlagFromAccountName(string name, unsigned int &flag)
{
	if(!db)
	{
		return false;
	...
	MYSQL_ROW row;
	stringstream query(stringstream::in | stringstream::out);
	query << "SELECT FirstFlag FROM " << server.options.GetAccountTable() << " WHERE AccountName = '";
	query << name;
	query << "'";
...
void DatabaseMySQL::UpdateLSFirstFlag(unsigned int id) //Angelox
{
	if(!db)
	{
		return;
	...
	stringstream query(stringstream::in | stringstream::out);
	query << "UPDATE " << server.options.GetAccountTable() << " SET FirstFlag = 1 " ;
	query << " WHERE LoginServerID = '";
	query << id;
	query << "'";
